...
describe('automatic reminder email delivery', () => {
  ...
  it('sends one email when the same reminder occurrence is retried', async () => {
    ...
    expect(mocks.sendMail).toHaveBeenCalledWith(expect.objectContaining({ to: 'alex@example.com' }));
  });
  ...
  it('passes a stable event scope from replayed watcher Telegram callsites', async () => {
    const user = makeUser('watcher_telegram_scope');
    mocks.sendTelegram.mockResolvedValue(true);
    const record = {
      id: 'watcher-telegram', userId: user.id, agentId: `${user.id}_jarvis`,
      label: 'Price alert', kind: 'price_watch', status: 'active', ticks: 4,
      onFire: { type: 'telegram' },
      state: { items: [{ id: 'mower', deliver: 'telegram' }] },
    };
    await handlerHelpers(record).fire({
      itemKey: 'mower', eventKey: 'price:mower:450', message: 'Mower is now $450',
    });
    await handlerHelpers({ ...record, ticks: 5 }).fire({
      itemKey: 'mower', eventKey: 'price:mower:450', message: 'Changed replay wording',
    });

    const firstOptions = mocks.sendTelegram.mock.calls[0][2];
    const replayOptions = mocks.sendTelegram.mock.calls[1][2];
    expect(firstOptions.idempotencyScope).toMatch(/^watcher:watcher-telegram:event:/);
    expect(replayOptions.idempotencyScope).toBe(firstOptions.idempotencyScope);
  });
