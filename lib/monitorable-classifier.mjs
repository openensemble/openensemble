...
function topicKey(userText, matched) {
  const base = String(matched || userText || '').toLowerCase()
 ...
 * @returns {Promise<{action:'ask'|'proposal'|'suppress', count:number, proposalId?:string|null, topicKey:string}>}
 */
export async function recordMonitorableHit({ userId, agentId, userText, hit }) {
  ...
  const key = topicKey(userText, hit.matched);
