 ...
 *     platform, caps }
 *
 * `platform`/`caps` (added 2026-07, Android TV integration) are optional and
 * absent (null / []) for every existing ESP32 device — the client reports
 * them on WS auth (see ws-handler.mjs) or, for platform only, at pairing
 * redeem time. `platform` is a free-form string ("android-tv" for the TV
 * app); `caps` is an array of capability strings (e.g. "tv_commands").
 */
...
import { deviceWakeCutoff } from './voice-calibration.mjs';

function userFile(userId) {
  return path.join(USERS_DIR, userId, 'voice-devices.json');
}

// mtime-keyed parse cache — getDevice/getSlotAssignment run ≥2× on every
// voice turn and were a fresh readFileSync+parse each time. Mutating callers
// follow the read-modify-writeFile pattern synchronously (single event loop),
// and writeFile refreshes the entry, so sharing the parsed array is safe.
const _devicesCache = new Map(); // userId -> { mtimeMs, devices }

function readFile(userId) {
  const file = userFile(userId);
  let mtimeMs;
  try { mtimeMs = fs.statSync(file).mtimeMs; }
  catch { _devicesCache.delete(userId); return []; }
  const hit = _devicesCache.get(userId);
  if (hit && hit.mtimeMs === mtimeMs) return hit.devices;
  try {
    const raw = JSON.parse(fs.readFileSync(file, 'utf8'));
    const devices = Array.isArray(raw) ? raw : [];
    _devicesCache.set(userId, { mtimeMs, devices });
    return devices;
  } catch (e) {
...
function writeFile(userId, devices) {
  ...
  try {
    ...
    try { _devicesCache.set(userId, { mtimeMs: fs.statSync(file).mtimeMs, devices }); }
    catch { _devicesCache.delete(userId); }
  } catch (e) {
...
function toWire(d) {
  return {
    ...
    // Conversation mode: after every reply the server re-arms a follow-up
    // listen window (8 s) so the user can keep talking without the wake word,
    // and (fw ≥ 0.2.65) speech during the reply pauses TTS for barge-in.
    conversation_mode: !!d.conversation_mode,
    // Android TV integration (2026-07): null/[] for every non-TV device —
    // see the file-header comment for the shape.
    platform: d.platform ?? null,
    caps: Array.isArray(d.caps) ? d.caps : [],
  };
}
...
/**
 * Return this user's paired devices that are Android TVs — platform ===
 * 'android-tv', or (platform not yet reported / other platform) caps
 * includes 'tv_commands'. Used by the tv-control tool executor to find a
 * target when the originating device isn't itself a TV. Combine with
 * isDeviceOnline() (ws-handler.mjs) to filter to reachable devices.
 */
export function listTvDevices(userId) {
  return readFile(userId)
    .filter(d => d.platform === 'android-tv' || (Array.isArray(d.caps) && d.caps.includes('tv_commands')))
    .map(toWire);
}
 ...
 *
 * @param {string} userId
 * @param {{token?: string, info?: {name?: string, fw_version?: string, platform?: string, caps?: string[]}}} [opts]
 */
export function registerDevice(userId, { token, info = {} } = {}) {
  ...
  const entry = {
    ...
    // Android TV integration: optional at redeem time (routes/devices/pairing.mjs
    // passes `platform` when the client sends it); `caps` normally arrives later
    // over WS auth (see touchDevice below) once the socket is up.
    platform: info.platform ? String(info.platform).slice(0, 32) : null,
    caps: Array.isArray(info.caps)
      ? info.caps.filter(c => typeof c === 'string' && c).slice(0, 20).map(c => c.slice(0, 32))
      : [],
    tts_voice: 'alloy',
    wake_word_slot: 0,
    ...
    // Snapshot of the slot→owner map this device last loaded (set by
    // markVoiceConfigPushed). null = never pushed → wake routing falls back to
    // live config. Drives getSlotAssignment so routing follows the device's
    // actually-loaded models, not a save-without-push repack.
    voice_config_pushed_assignments: null,
  };
  devices.push(entry);
...
export function updateDevice(userId, id, patch) {
  ...
  for (const [k, v] of Object.entries(patch || {})) {
    ...
    else if (k === 'conversation_mode') entry.conversation_mode = !!v;
 ...
 * @param {string} userId
 * @param {string} id
 * @param {{fw_version?: string, mute_state?: boolean, platform?: string, caps?: string[]}} [opts]
 */
export function touchDevice(userId, id, { fw_version, mute_state, platform, caps } = {}) {
  const devices = readFile(userId);
  const idx = devices.findIndex(x => x.id === id);
  if (idx < 0) return;
  const now = Date.now();
  ...
  // Android TV integration: WS auth re-reports platform/caps on every
  // reconnect (ws-handler.mjs), so refresh them here the same way fw_version
  // is refreshed. Absent for every existing ESP32/browser auth — no-op.
  if (typeof platform === 'string' && platform) devices[idx].platform = platform.slice(0, 32);
  if (Array.isArray(caps)) {
    devices[idx].caps = caps.filter(c => typeof c === 'string' && c).slice(0, 20).map(c => c.slice(0, 32));
  }
  writeFile(userId, devices);
}
 ...
 *
 * `assignments` (optional) is the exact slot→owner map the device just
 * loaded — stored as the device's pushed snapshot so wake-time routing
 * (getSlotAssignment) can resolve slots against what the device ACTUALLY has,
 * not a later save-without-push that repacked the live config. Deep-copied so
 * it can't alias (and later mutate with) the voice-config cache.
 */
export function markVoiceConfigPushed(userId, id, version, assignments) {
  const devices = readFile(userId);
  const idx = devices.findIndex(x => x.id === id);
  if (idx < 0) return;
  devices[idx].voice_config_pushed_version = Number.isInteger(version) ? version : 0;
  if (assignments && typeof assignments === 'object') {
    try { devices[idx].voice_config_pushed_assignments = JSON.parse(JSON.stringify(assignments)); }
    catch { /* leave the prior snapshot if the map isn't serializable */ }
  }
  writeFile(userId, devices);
}
...
export function getSlotAssignment(deviceOwnerId, deviceId, slot) {
  ...
  // Resolve against the config snapshot the DEVICE actually loaded (its last
  // successful push) rather than the live config. A save-without-push repacks
  // the live slot→owner map (contiguous-slot normalization) while the device
  // keeps running the previously-pushed models — reading live config here
  // would misroute one household member's wake word into another member's
  // account during that window. Only fall back to live config for a device
  // that has never been pushed a snapshot yet (its first-connect push resolves
  // this within seconds). A device pushed an EMPTY config has a snapshot of
  // `{}` (routes nothing) — distinct from never-pushed (null → live).
  let a = null;
  if (deviceId) {
    const dev = readFile(deviceOwnerId).find(x => x.id === deviceId);
    const snap = dev && dev.voice_config_pushed_assignments && typeof dev.voice_config_pushed_assignments === 'object'
      ? dev.voice_config_pushed_assignments : null;
    a = snap
      ? (snap[slot] ?? snap[String(slot)] ?? null)
      : getConfiguredSlot(deviceOwnerId, slot);
  } else {
    a = getConfiguredSlot(deviceOwnerId, slot);
  }
  ...
  return {
    ...
    avg_prob_cutoff: deviceWakeCutoff(deviceOwnerId, deviceId, slot, a) ?? (typeof a.avg_prob_cutoff === 'number' ? a.avg_prob_cutoff : null),
  };
}
