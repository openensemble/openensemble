...
const SKIP_TOOLS = new Set([
  ...
  'skill_delete', 'skill_list', 'skill_read_code', 'skill_read_drawer', 'skill_read_blueprint',
]);
...
export async function maybeProposeSkill({ userId, agentId, agentName, userMessage, assistantContent, toolsUsed }) {
  ...
  // Failure gate — the header promises "without obvious failure" but nothing
  // ever checked results. Bundling a turn where a tool errored proposes a
  // broken sequence. Canonical error results start "Error:" / "Tool error:";
  // for short results also match error-shaped words anywhere (long results
  // are documents where "error" is likely just content). Same philosophy as
  // routine-proposer's gate: silent passes are cheap.
  const looksFailed = (t) => {
    const text = String(t?.text ?? '');
    if (/^\s*(?:tool )?error\b/i.test(text)) return true;
    return text.length > 0 && text.length <= 400 &&
      /\b(?:error|failed|not found|unauthoriz|forbidden|unavailable|permission denied|timed? ?out)\b/i.test(text);
  };
  if (interesting.some(looksFailed)) return { skipped: 'tool_failure' };
  ...
  try {
    ...
    // warn, not debug: falling through means the duplicate-skill guard did not
    // run, so the user can be offered a skill they already have. Debug level
    // meant that never showed up anywhere.
    console.warn('[skill-proposer] custom-skill overlap check failed (duplicate guard skipped):', e.message);
  }
