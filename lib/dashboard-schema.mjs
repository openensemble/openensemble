...
export function dashboardLayoutForOrientation(layout, orientation = 'portrait') {
  return layout?.version === 7 ? layout[orientation] : layout;
}
...
export function validateLayout(value) {
  if (isRecord(value) && value.version === 7) {
    if (!hasExactKeys(value, ['version', 'portrait', 'landscape'])) {
      return invalid('Layout version 7 must contain exactly version, portrait, and landscape.');
    }
    const layouts = {};
    for (const orientation of ['portrait', 'landscape']) {
      if (!isRecord(value[orientation]) || value[orientation].version === 7) {
        return invalid(`${orientation} must contain a layout version 1 through 6.`);
      }
      const checked = validateLayout(value[orientation]);
      if (!checked.ok) return invalid(`${orientation}: ${checked.error}`);
      layouts[orientation] = checked.layout;
    }
    return { ok: true, layout: { version: 7, ...layouts } };
  }
  if (!isRecord(value)) {
