...
import { rootTaskGraphs } from '../background-tasks/state.mjs';
...
function taskProgressText(task, groupSize = 1) {
  ...
  if (task.status === 'paused') return `${name} is paused. Review the saved progress to continue.`;
  if (task.phase === 'resuming') return `${name} is continuing after restart.`;
...
function sanitizeTask(task) {
  ...
  return {
    _userId: cleanString(task.userId, 240),
    taskId,
    watcherId,
    rootWatcherId,
    ...
    provider: cleanString(task.provider, 100) || null,
    model: cleanString(task.model, 300) || null,
    ...taskRuntimeFields(task),
    reasoningEffort: cleanString(task.reasoningEffort, 40) || null,
    executionTargetExplicit: task.executionTargetExplicit === true,
    ...
    canCancel,
    recoveryAvailable: status === 'paused' && Boolean(task.checkpoint),
...
function childSnapshot(task) {
  return {
    ...
    sourceSessionEpoch: task.sourceSessionEpoch,
...
export function projectActiveTasksForWire(tasks = []) {
  ...
  for (const raw of Array.isArray(tasks) ? tasks : []) {
    ...
    if (raw?.originScheduledTaskId || raw?.originScheduledSilent === true) continue;
    const task = sanitizeTask(raw);
    if (!task) continue;
    const key = `${task._userId}\u0000${task.visibleAgentId}\u0000${task.rootWatcherId}`;
  ...
  for (const group of groups.values()) {
    ...
    const retainedChildren = coordinatedTasksSnapshot(primary.rootTaskId || primary.taskId)
      ...
      .filter(Boolean);
    // The live graph retains completed child rows with the runtime model and
    // source chat. Coordination reservations alone only know configured models.
    const graph = rootTaskGraphs.get(primary.rootTaskId || primary.taskId);
    const graphChildren = graph?.userId === primary._userId && Array.isArray(graph.completedChildren)
      ? graph.completedChildren.map(retainedChildSnapshot).filter(Boolean)
      : [];
    ...
    snapshots.push({
      ...
      model: primary.model,
      runtimeProvider: primary.runtimeProvider,
      runtimeModel: primary.runtimeModel,
      modelSource: primary.modelSource,
      activity: primary.activity,
      sourceSessionKey: primary.sourceSessionKey,
      sourceSessionEpoch: primary.sourceSessionEpoch,
      reasoningEffort: primary.reasoningEffort,
      executionTargetExplicit: primary.executionTargetExplicit === true,
      ...
      text,
      state: {
        recoveryAvailable: primary.recoveryAvailable,
        recoveryTaskId: primary.recoveryAvailable ? primary.taskId : null,
