 ...
 *
 * Service id is 'system' (one per node). Trust state defaults to 'unverified'
 * so the user explicitly onboards host-health checks from the Nodes drawer.
 * Approval verifies the built-in read-only diagnostics, then starts the
 * watcher. Auto-fix remains a separate confirmation step.
 *
...
export function buildSystemProfile({ nodeId, hostname }) {
  ...
  return {
    ...
    operations: [
      // ── Read-only diagnostics — coordinator + auto-apply path can fire freely ─
      {
        id: 'disk_detail',
 ...
 * path, returns { created: false }. Otherwise saves the template as Draft.
 * The watcher is registered later when the user approves Host health from the
 * Nodes drawer. Skips non-linux platforms.
 *
...
export function ensureNodeSystemProfile(userId, nodeId, { hostname, platform } = {}) {
  ...
  if (profileExists) {
    ...
    if (!isActiveTrust) {
      // User intentionally left monitoring off, or this is a new Draft profile
      // waiting for explicit onboarding — don't override.
  ...
  log.info('node-system-profile', 'created system profile pending user approval', { userId, nodeId });
  return { created: true, signal_count: profile.health_signals.length, watcher_id: null, reason: 'created as draft; awaiting approval' };
}
...
function upgradeExistingProfile(userId, nodeId, hostname, fromVersion, toVersion) {
  ...
  const upgraded = loadProfile(userId, nodeId, SERVICE_ID);
  if (!upgraded || (upgraded.trust_state !== 'reviewed' && upgraded.trust_state !== 'proven')) {
    return {
      created: false,
      upgraded: true,
      from_version: fromVersion,
      to_version: toVersion,
      signal_count: upgraded?.health_signals?.length ?? 0,
      watcher_id: null,
      reason: 'upgraded draft profile; awaiting approval',
    };
  }
