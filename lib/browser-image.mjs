...
function pinnedLookup(hostname, _options, callback) {
...
function requestOnce(url, { requestHttp = http.request, requestHttps = https.request, accept = '*/*' } = {}) {
 ...
 * @param {{maxBytes:number,mimePattern:RegExp,label:string,accept?:string}} policy
 * @param {any} [deps]
 */
export async function fetchBrowserPublicResource(rawUrl, policy, deps = {}) {
  ...
  for (let redirect = 0; redirect <= MAX_REDIRECTS; redirect++) {
    const response = await requestOnce(url, { ...deps, accept: policy.accept || '*/*' });
...
export const __test = Object.freeze({ validateUrl, pinnedLookup });
