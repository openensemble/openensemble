...
export async function observeTurnAndLearn(userId, userText, scopedSessionKey) {
  ...
  try {
    ...
    // Two formats coexist in the session jsonl:
    //   - Modern: lastAssistant.tool_calls = [{function: {name, arguments}}, ...]
    //   - Legacy (what chat.mjs:persist() actually saves today):
    //       lastAssistant.toolsUsed = ["name({json args})", "name", ...]
    //     The args are the strings from chat.mjs:62, truncated at 120 chars
    //     but with the JSON envelope intact for the short args we care about
    //     (channel_id, account_id, agent_id, node_id, project name — all <30
    //     chars). Try JSON.parse; if truncation broke it, accept empty args
    //     and the learner just won't fire for that call.
    let calls = [];
    if (Array.isArray(lastAssistant.tool_calls) && lastAssistant.tool_calls.length) {
      calls = lastAssistant.tool_calls.map(tc => ({
        name: tc.function?.name,
        argsStr: tc.function?.arguments || '{}',
      }));
    } else if (Array.isArray(lastAssistant.toolsUsed) && lastAssistant.toolsUsed.length) {
      calls = lastAssistant.toolsUsed.map(_parseToolsUsedEntry);
    }
    ...
    for (const tc of calls) {
      ...
      let argsObj = {};
      try { argsObj = JSON.parse(tc.argsStr || '{}'); } catch { /* truncation — skip args */ }
...
async function _learnSkillAliasFromOwningTool(userId, userText, toolName, fw) {
  ...
  if (!candidate) return;
  try {
    if (!(await fw.setAliasExternal(userId, 'skill', candidate, skillId))) return;
...
export async function maybeConsumeAffirmation(userId, userText) {
  ...
  try {
    const fw = await import('./skill-alias-framework.mjs');
    if (!(await fw.setAliasExternal(userId, pending.entityKind, pending.origPhrase, pending.entityId))) return false;
...
/**
 * Parse one entry from chat.mjs:persist()'s `toolsUsed` array.
 *
 * Format: `email_purge_sender({"account":"<label>","sender":"uber"})` or just
 * `email_purge_sender` (no args). Args are truncated at 120 chars in the
 * persist layer — for our use case (alias learning on id-shaped args) that's
 * always far longer than needed.
 */
function _parseToolsUsedEntry(entry) {
  if (typeof entry !== 'string') return { name: '', argsStr: '{}' };
  const open = entry.indexOf('(');
  if (open < 0) return { name: entry, argsStr: '{}' };
  const close = entry.lastIndexOf(')');
  if (close <= open) return { name: entry.slice(0, open), argsStr: '{}' };
  return {
    name:    entry.slice(0, open),
    argsStr: entry.slice(open + 1, close),
  };
}
...
/**
 * Read the just-finished turn's tool calls from the session jsonl. Shared by
 * observeTurnAndLearn (alias learning) and lib/intent-learner.mjs (Phase-3
 * intent learning). Returns [{ name, argsObj }] (args parsed best-effort; {} on
 * truncation). Purely additive — observeTurnAndLearn's own path is unchanged.
 */
export async function getLastTurnToolCalls(userId, scopedSessionKey) {
  const lastAssistant = await _readLastAssistantMessage(userId, scopedSessionKey);
  if (!lastAssistant) return [];
  let calls = [];
  if (Array.isArray(lastAssistant.tool_calls) && lastAssistant.tool_calls.length) {
    calls = lastAssistant.tool_calls.map(tc => ({ name: tc.function?.name, argsStr: tc.function?.arguments || '{}' }));
  } else if (Array.isArray(lastAssistant.toolsUsed) && lastAssistant.toolsUsed.length) {
    calls = lastAssistant.toolsUsed.map(_parseToolsUsedEntry);
  }
  return calls.filter(c => c.name).map(c => {
    let argsObj = {};
    try { argsObj = JSON.parse(c.argsStr || '{}'); } catch { /* truncation — empty args */ }
    return { name: c.name, argsObj };
  });
}
