...
vi.mock('./execution-model-policy.mjs', () => ({
  ...
  listExecutionCatalog: async () => [],
  canUseExecutionProvider: () => ({ ok: false, reason: 'mocked-off' }),
}));
...
describe('runtime execution-profile authorization', () => {
  it('fails closed when a once-valid provider grant has been revoked', async () => {
    ...
    // Revoked pins must not apply the saved model. Auto may still emit a
    // structure/hint contender; effort-only auto that matches the baseline
    // does not count as applied.
    expect(result.applied).toBe(false);
    expect(result.effective.provider).toBe('openai-oauth');
    expect(result.effective.model).toBe('base-model');
    expect(result.contenders).toEqual(expect.arrayContaining([
      expect.objectContaining({
        skillId: 'deep_research', source: 'user', eligible: false,
        reason: 'oauth-provider-not-allowed',
      }),
    ]));
  });
