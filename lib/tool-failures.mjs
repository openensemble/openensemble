...
/**
 * Drop failure history for the given tools — called on skill deletion so a
 * removed skill's tools stop feeding flaky-tool proposals. Returns count removed.
 */
export async function purgeToolFailures(userId, toolNames = []) {
  if (!userId) return { ok: false, removed: 0 };
  const drop = new Set((toolNames || []).filter(Boolean));
  if (!drop.size) return { ok: true, removed: 0 };
  const data = loadFailures(userId);
  let removed = 0;
  for (const name of drop) { if (data[name]) { delete data[name]; removed++; } }
  if (removed) {
    try { await saveFailures(userId, data); }
