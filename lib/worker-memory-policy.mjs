...
const COORDINATED_CHILD_FORBIDDEN_TOOLS = new Set([
  'parallel_work',
  'redirect_worker',
]);

function isCoordinatedWorkstream(agent) {
  return agent?.ephemeral === true
    && typeof agent?.id === 'string'
    && agent.id.startsWith('ephemeral_workstream_');
}
...
export function filterWorkerLeafTools(tools, agent, turnOpts = {}) {
  ...
  const coordinatedChild = isCoordinatedWorkstream(agent);
  const parallelAuthorized = agent?.parallelWorkRequested === true
    || (
      Number.isSafeInteger(agent?.requestedWorkstreams)
      && agent.requestedWorkstreams >= 2
    );
  return tools.filter(tool => {
    const name = tool?.function?.name;
    if (WORKER_LEAF_FORBIDDEN_TOOLS.has(name)) return false;
    if (!coordinatedChild && name === 'parallel_work' && !parallelAuthorized) return false;
    return !coordinatedChild || !COORDINATED_CHILD_FORBIDDEN_TOOLS.has(name);
  });
}
