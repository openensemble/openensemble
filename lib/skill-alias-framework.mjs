 ...
 */
export async function resolveFromMessage(userId, text, opts = {}) {
  if (!userId || typeof text !== 'string' || text.length < 4) return null;
  const suppressLearning = opts?.suppressLearning === true || opts?.readOnly === true;
  ...
  for (const reg of visibleRegistrations(userId)) {
    ...
    for (const phrase of candidates) {
      ...
      if (stored[normPhrase]) {
        ...
        // Catalog says the entry is gone — stale alias. Normal turns clean it
        // up, while read-only/verifier resolution leaves storage untouched.
        if (!suppressLearning) deleteAlias(userId, reg.spec.entity_kind, phrase, noun);
      }
