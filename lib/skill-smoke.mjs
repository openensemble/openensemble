 ...
 */
import { normalizeSkillDashboardPayload } from './dashboard-widgets.mjs';
 ...
 * @param {{ userId?: string|null, readOnlyFs?: boolean, forceNoNetwork?: boolean }} [opts]
 * owner — required for the jailed path
 * @returns {Promise<SmokeReport>}
 */
export async function runSkillSmoke(
  skillDir,
  manifest,
  { userId = null, readOnlyFs = false, forceNoNetwork = false } = {},
) {
  ...
  const dashboardDescriptors = Array.isArray(manifest?.dashboardWidgets)
    ? manifest.dashboardWidgets : [];
  // Never exercise an unattended dashboard tool inside the ordinary RW/network
  // smoke. Split widget-bearing skills into an ordinary phase with those tools
  // removed and an exact widget phase whose bwrap mounts are read-only and whose
  // native network namespace is disconnected.
  if (dashboardDescriptors.length && !readOnlyFs) {
    const dashboardToolNames = new Set(
      dashboardDescriptors.map(widget => widget?.tool).filter(Boolean),
    );
    const ordinaryTools = tools.filter(tool => !dashboardToolNames.has(tool?.function?.name));
    const ordinary = ordinaryTools.length
      ? await runSkillSmoke(skillDir, {
        ...manifest,
        tools: ordinaryTools,
        dashboardWidgets: [],
      }, { userId, forceNoNetwork: false, readOnlyFs: false })
      : { ok: true, results: [] };
    if (ordinary.setupError) return ordinary;
    const dashboard = await runDashboardWidgetSmoke(skillDir, manifest, { userId });
    if (dashboard.setupError) return dashboard;
    return {
      ok: ordinary.ok && dashboard.ok,
      results: [...ordinary.results, ...dashboard.results],
    };
  }
  ...
  if (_shouldSandboxSmoke(manifest, uid, skillDir)) {
    ...
    try {
      ...
      const r = await runSandboxedJob({
        ...
        net: forceNoNetwork ? false : manifest?.sandbox?.network === true,
        readOnlyFs,
        timeoutMs,
      });
...
/**
 * Exercise only tools bound to dashboard widgets, including each descriptor's
 * real static args plus the empty config a newly-added card starts with. Widget
 * code is always smoked in the sandbox even when ordinary chat execution was
 * explicitly opted out of isolation. `toolName` narrows a contract update to
 * the exact affected dashboard tool and every widget descriptor bound to it.
 * @param {string} skillDir
 * @param {any} manifest
 * @param {{userId?:string|null, toolName?:string|null}} [opts]
 */
export async function runDashboardWidgetSmoke(
  skillDir,
  manifest,
  { userId = null, toolName = null } = {},
) {
  const descriptors = (Array.isArray(manifest?.dashboardWidgets) ? manifest.dashboardWidgets : [])
    .filter(widget => !toolName || widget?.tool === toolName);
  if (!descriptors.length) return { ok: true, results: [] };
  const names = new Set(descriptors.map(widget => widget?.tool).filter(Boolean));
  const tools = (Array.isArray(manifest?.tools) ? manifest.tools : [])
    .filter(tool => names.has(tool?.function?.name));
  return runSkillSmoke(skillDir, {
    ...manifest,
    tools,
    dashboardWidgets: descriptors,
    sandbox: { ...(manifest?.sandbox || {}), isolate: true },
  }, { userId, readOnlyFs: true, forceNoNetwork: true });
}
...
export async function smokeModule(mod, manifest) {
  const tools = Array.isArray(manifest?.tools) ? manifest.tools : [];
  const dashboardWidgetTools = new Set((Array.isArray(manifest?.dashboardWidgets)
    ? manifest.dashboardWidgets : [])
    .map(widget => widget?.tool)
  ...
  for (const t of tools) {
    ...
    // Widget tools are invoked below with their exact descriptor args and the
    // reduced read-only ctx. Avoid a second schema-generated invocation.
    if (dashboardWidgetTools.has(toolName)) continue;
  ...
  // A schema-generated smoke can accidentally hide a bad widget descriptor
  // (for example it supplies a required query that dashboard args omit). Run
  // every widget once with the exact arguments a newly-added card receives.
  // Use the same reduced capability shape the production dashboard host gives
  // a read-only tool. This catches a mislabeled handler that tries to register
  // a watcher, write a credential, or provision a runtime during polling.
  const dashboardStub = _makeDashboardStubCtx();
  for (const widget of (Array.isArray(manifest?.dashboardWidgets) ? manifest.dashboardWidgets : [])) {
    const toolName = widget?.tool;
    const tool = tools.find(candidate => candidate?.function?.name === toolName);
    if (!tool || tool.destructive === true || tool.readOnly !== true) continue;
    const label = `${toolName} [dashboard:${widget.id || 'widget'}]`;
    const args = { ...(widget.args || {}), config: {} };
    const startedAt = Date.now();
    try {
      const value = await _raceWithTimeout(
        fn(toolName, args, 'test-user', 'test-agent', dashboardStub),
        PER_TOOL_TIMEOUT_MS,
        label,
      );
      const durationMs = Date.now() - startedAt;
      if (value === null || value === undefined) {
        results.push({
          tool: label, outcome: 'fail', reason: 'returned-null',
          message: 'returned null with the widget\'s actual static args and initial config.',
          durationMs,
        });
        continue;
      }
      const checked = normalizeSkillDashboardPayload(value);
      if (checked.ok === true) {
        results.push({
          tool: label, outcome: 'pass',
          message: `valid dashboard payload (${JSON.stringify(checked.data).slice(0, 140)})`,
          durationMs,
        });
      } else {
        results.push({
          tool: label, outcome: 'fail', reason: 'wrong-type',
          message: `invalid dashboard widget payload: ${checked.error}`,
          durationMs,
        });
      }
    } catch (e) {
      results.push({
        tool: label,
        outcome: 'fail',
        reason: /smoke timeout/.test(e.message) ? 'timeout' : 'crashed',
        message: e.message,
...
// Keep this in lockstep with makeCtx(..., readOnlyCtx=true) in skill-host.mjs.
// Dashboard polling intentionally gets no credential writes, watcher methods,
// runtime provisioning, media helpers, or other side-effect capabilities.
function _makeDashboardStubCtx() {
  return {
    userId: 'test-user',
    agentId: 'test-agent',
