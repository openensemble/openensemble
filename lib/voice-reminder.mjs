...
function pickReminderVoice(userId, provider) {
  ...
  const looksValid = (v) => {
    ...
    if (provider === 'piper') return /^\d+$/.test(v);
...
async function synthesizeReminderTts(text, userId) {
  ...
  if (provider === 'elevenlabs') {
    ...
    const r = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(voice)}`, {
      ...
      body: JSON.stringify({ text, model_id: elModel, voice_settings: { stability: 0.5, similarity_boost: 0.75 } }),
  ...
  if (provider === 'piper') {
    ...
    const r = await fetch(piperUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ text, speaker_id: Number.isFinite(speakerId) ? speakerId : 0 }),
...
/**
 * Speak `text` (prefixed with "Reminder: ") on `deviceIds` paired to userId.
 * Synthesizes once, caches per-device markers, pushes token+done pairs.
 * Skips offline devices silently — they can't fetch a one-shot marker.
 *
 * Returns the deviceIds that were actually pushed to (for the fireReminder
 * delivery summary).
 */
export async function speakReminder({ userId, deviceIds, text }) {
  ...
  let ttsMp3;
  try {
    ttsMp3 = await synthesizeReminderTts(`Reminder: ${text}`, userId);
  } catch (e) {
    console.warn(`[reminder-voice] TTS synthesis failed: ${e.message}`);
    return [];
  }
  const chimeMp3 = await getChimeMp3();

  const delivered = [];
  for (const deviceId of targets) {
    // Markers are one-shot, so each device needs its own pair. Cheap — the
    // MP3 buffers themselves are shared by reference inside the cache Map.
    if (chimeMp3.length) {
