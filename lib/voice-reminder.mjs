...
import { getVoiceRef } from './voice-refs.mjs';
import fs from 'fs';
import os from 'os';
...
function pickReminderVoice(userId, provider) {
  const defaultVoice =
    ...
    provider === 'piper' ? '' :
    // Empty for pocket-tts too → the synth branch falls back to the bundled OE
    // default voice-state (or the 'george' preset).
    provider === 'pocket-tts' ? '' :
    // KittenTTS baked-in default preset (matches KITTENTTS_DEFAULT_VOICE).
    provider === 'kittentts' ? 'expr-voice-2-f' :
  ...
  const looksValid = (v) => {
    ...
    if (provider === 'pocket-tts') {
      // A cloned voice-ref (ref_<hex>) or a preset catalog name.
      return /^ref_[a-f0-9]+$/.test(v) || /^[a-zA-Z0-9_-]+$/.test(v);
    }
    if (provider === 'kittentts') {
      // One of the 8 baked-in presets (expr-voice-<n>-<m|f>).
      return /^expr-voice-\d+-[mf]$/.test(v);
    }
...
async function synthesizeReminderTts(text, userId) {
  ...
  const ALLOWED = ['openai', 'piper', 'elevenlabs', 'pocket-tts', 'kittentts'];
  ...
  if (provider === 'kittentts') {
    // KittenTTS speaks {text, voice} → WAV (24 kHz); voice is one of 8 baked-in
    // presets, and the server falls back to its own default for anything else.
    const kittenttsUrl = cfg.kittenttsUrl || 'http://127.0.0.1:5153/';
    const r = await fetch(kittenttsUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ text, voice }),
      signal: AbortSignal.timeout(30000),
    });
    if (!r.ok) throw new Error(`KittenTTS returned ${r.status}`);
    return ffmpegToDeviceMp3(Buffer.from(await r.arrayBuffer()), 'wav');
  }

  if (provider === 'pocket-tts') {
    // Mirror /api/tts: zero-shot clone from the user's uploaded ref (ref_<hex>,
    // owned by this user), an explicit preset catalog name, or the bundled OE
    // default voice-state — then resample WAV → 16 kHz device MP3.
    const pocketBody = { text };
    const isRef = typeof voice === 'string' && voice.startsWith('ref_');
    const pref = isRef ? getVoiceRef(userId, voice) : null;
    ...
    else if (voice && !isRef && voice !== 'default-en' && voice !== 'default') pocketBody.voice = voice;
    ...
    const r = await fetch(pocketUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(pocketBody),
      signal: AbortSignal.timeout(60000),
    });
    if (!r.ok) throw new Error(`Pocket TTS returned ${r.status}`);
    return ffmpegToDeviceMp3(Buffer.from(await r.arrayBuffer()), 'wav');
  }
...
/** ffprobe an in-memory MP3 buffer for its duration in ms (0 on failure). */
function probeMp3DurationMs(buf) {
  return new Promise((resolve) => {
    let out = '';
    let ff;
    try {
      ff = spawn('ffprobe', ['-v', 'error', '-show_entries', 'format=duration',
        '-of', 'default=nw=1:nk=1', '-i', 'pipe:0']);
    } catch { return resolve(0); }
    ff.stdout.on('data', d => { out += d; });
    ff.on('error', () => resolve(0));
    ff.on('close', () => { const s = parseFloat(out.trim()); resolve(Number.isFinite(s) ? Math.round(s * 1000) : 0); });
    ff.stdin.on('error', () => {});
...
/**
 * Speak a routine's reply on a device via the MP3-marker path and return how
 * long the audio is, so the caller can start ambient sound only AFTER the
 * announcement finishes. Same delivery as speakReminder (no prefix/chime) but
 * reports the measured duration.
 * @returns {Promise<{delivered:string[], durationMs:number}>}
 */
export async function speakRoutineTts({ userId, deviceIds, text }) {
  const ids = Array.isArray(deviceIds) ? deviceIds : (deviceIds ? [deviceIds] : []);
  if (!ids.length || !text) return { delivered: [], durationMs: 0 };
  const owned = new Set(listDevices(userId).map(d => d.id));
  const targets = ids.filter(id => owned.has(id) && isDeviceOnline(id));
  if (!targets.length) return { delivered: [], durationMs: 0 };
  let ttsMp3;
  try {
    ttsMp3 = await synthesizeReminderTts(text, userId);
  } catch (e) {
    console.warn(`[routine-tts] synthesis failed: ${e.message}`);
    return { delivered: [], durationMs: 0 };
  }
  const durationMs = await probeMp3DurationMs(ttsMp3);
  const delivered = [];
  for (const deviceId of targets) {
    const m = cacheOneShotMp3(ttsMp3);
    sendToDevice(deviceId, { type: 'token', text: m, agent: 'system' });
    sendToDevice(deviceId, { type: 'done', agent: 'system' });
    delivered.push(deviceId);
  }
  return { delivered, durationMs };
}
