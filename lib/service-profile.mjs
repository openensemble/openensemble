...
export function loadProfile(userId, nodeId, serviceId) {
  const p = profilePath(userId, nodeId, serviceId);
...
export function saveProfile(userId, nodeId, profile) {
  ...
  ensureProfilesDir(userId, nodeId);
  const p = profilePath(userId, nodeId, validated.service_id);
  ...
  fs.writeFileSync(profileMdPath(userId, nodeId, validated.service_id), renderProfileMd(validated), 'utf8');
  return validated;
}

export function listProfilesForNode(userId, nodeId) {
  const dir = profilesDir(userId, nodeId);
  if (!fs.existsSync(dir)) return [];
  const out = [];
  for (const entry of fs.readdirSync(dir)) {
    if (!entry.endsWith('.json')) continue;
    const serviceId = entry.replace(/\.json$/, '');
    const prof = loadProfile(userId, nodeId, serviceId);
...
export function deleteProfile(userId, nodeId, serviceId) {
  for (const p of [
    profilePath(userId, nodeId, serviceId),
    profileMdPath(userId, nodeId, serviceId),
    profileResearchPath(userId, nodeId, serviceId),
  ]) {
