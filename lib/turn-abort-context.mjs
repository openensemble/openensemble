...
/** Consume an async iterable with every iterator operation in one turn context. */
export async function* iterateInTurnContext(ctx, source) {
  const iterator = runWithTurnContext(ctx, () => {
    const iterable = typeof source === 'function' ? source() : source;
    if (!iterable || typeof iterable[Symbol.asyncIterator] !== 'function') {
      throw new TypeError('iterateInTurnContext requires an async iterable');
    }
  ...
  try {
    while (true) {
      const step = await runWithTurnContext(ctx, () => iterator.next());
      if (step.done) {
        finished = true;
        return step.value;
      }
    ...
    if (!finished && typeof iterator.return === 'function') {
      try { await runWithTurnContext(ctx, () => iterator.return()); }
