...
// Bounded, known modifiers preserve an exact count without treating arbitrary
// adjectives or model names as authorization to spawn a team.
const AGENT_QUALIFIERS_PATTERN = '(?:(?:parallel|background|child)\\s+){0,3}';
const MALFORMED_AGENT_COUNT_RE = new RegExp(
  `\\b${AGENT_COMMAND_PATTERN}\\s+(?:exactly\\s+)?(?:[+\\-]\\d+|\\d+(?:[.,]\\d+)+)\\s+${AGENT_QUALIFIERS_PATTERN}(?:[a-z0-9][a-z0-9._:/+@-]*\\s+)?(?:agents?|sub[- ]?agents?|workers?|workstreams?|lanes?)\\b`,
  'i',
);
const TARGET_QUALIFIED_COUNT_RE = new RegExp(
  `\\b${AGENT_COMMAND_PATTERN}\\s+(?:exactly\\s+)?(\\d+|zero|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|hundreds?|thousands?)\\s+${AGENT_QUALIFIERS_PATTERN}[a-z0-9][a-z0-9._:/+@-]*\\s+(?:agents?|sub[- ]?agents?|workers?|workstreams?|lanes?)\\b`,
  'gi',
);
...
const LANE_COUNT_RE = new RegExp(
  `\\b${AGENT_COMMAND_PATTERN}\\s+(?:exactly\\s+)?(${AGENT_COUNT_TOKEN})\\s+${AGENT_QUALIFIERS_PATTERN}(?:agents?|lanes?|workstreams?|sub[- ]?agents?|workers?)\\b|\\b(${AGENT_COUNT_TOKEN})\\s+${AGENT_QUALIFIERS_PATTERN}(?:agents?|lanes?|workstreams?|sub[- ]?agents?|workers?)\\s+(?:to|for|on)\\b`,
  'gi',
);
...
const TARGET_BEFORE_AGENT_GROUP_RE = new RegExp(
  `\\b(${AGENT_COUNT_TOKEN})\\s+${AGENT_QUALIFIERS_PATTERN}([a-z0-9][a-z0-9._:/+@-]*)\\s+(?:agents?|sub[- ]?agents?|workers?|workstreams?|lanes?)\\b`,
  'gi',
);
...
const EFFORT_BEFORE_TARGET_GROUP_RE = new RegExp(
  `\\b(${AGENT_COUNT_TOKEN})\\s+${AGENT_QUALIFIERS_PATTERN}(${EFFORT_VALUE_PATTERN})(?:-|\\s+)effort\\s+([a-z0-9][a-z0-9._:/+@-]*)\\s+(?:agents?|sub[- ]?agents?|workers?|workstreams?|lanes?)\\b`,
  'gi',
);
const AGENT_FOR_TARGET_GROUP_RE = new RegExp(
  `\\b(${AGENT_COUNT_TOKEN})\\s+${AGENT_QUALIFIERS_PATTERN}(?:(?:agents?|sub[- ]?agents?|workers?|workstreams?|lanes?)\\s+)?(?:for|with|using|on)\\s+([a-z0-9][a-z0-9._:/+@-]*)`,
  'gi',
);
...
const DECLARED_AGENT_ALLOCATION_HEADER_RE = new RegExp(
  `^${AGENT_COMMAND_PATTERN}\\s+(?:exactly\\s+)?(${AGENT_COUNT_TOKEN})\\s+${AGENT_QUALIFIERS_PATTERN}(?:agents?|sub[- ]?agents?|workers?|workstreams?|lanes?)(?:\\s+(?:with\\s+)?${EFFORT_EXPRESSION_PATTERN})?\\s*[:,]`,
  'i',
);
    ...
    if (!groups.length) {
      ...
      if (allMatch) {
        const leading = clause.match(new RegExp(
          `^(?:\\S+\\s+){0,3}(?:exactly\\s+)?(${AGENT_COUNT_TOKEN})\\s+${AGENT_QUALIFIERS_PATTERN}(?:agents?|sub[- ]?agents?|workers?|workstreams?|lanes?)\\b`,
          'i',
        ));
