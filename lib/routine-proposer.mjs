 ...
 *   user: "sydney, turn off kitchen"
 *   sydney: (HA fast-path misses on ambiguous "kitchen", router routes to Helen)
 *   helen:  calls ha_call_service { domain:'homeassistant', service:'turn_off',
 ...
 */
import { proposeRoutine, proposeAlias } from './proposals.mjs';
...
export async function maybeProposeRoutine({ userId, agentId, agentName, userMessage, toolsUsed, voiceCtx = null }) {
  ...
  if (voiceCtx?.source === 'voice-device' && voiceCtx.deviceId) {
    try {
      ...
      sendToDevice(voiceCtx.deviceId, { type: 'token', text: ' ' + sentence, agent: agentName || 'sydney' });
      sendToDevice(voiceCtx.deviceId, { type: 'done', agent: agentName || 'sydney' });
