...
export async function maybeProposeRoutine({ userId, agentId, agentName, userMessage, toolsUsed, voiceCtx = null }) {
  ...
  if (voiceCtx?.source === 'voice-device' && voiceCtx.deviceId) {
    try {
      const [{ sendToDevice, armFollowupAfterDrain }, { DISPOSITION_WINDOW_MS }] = await Promise.all([
        import('../ws-handler.mjs'),
        import('./voice-policy.mjs'),
      ]);
      ...
      // Use the central armer so this required reply is tracked at stt_begin
      // and cannot bypass fleet/self-audio policy bookkeeping.
      armFollowupAfterDrain(voiceCtx.deviceId, {
