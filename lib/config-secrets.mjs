...
/** Per-user profile.json secret paths. Lives here (next to SECRET_PATHS) so
 *  the same envelope / master-key infrastructure is reused across config
 *  and profile files. Telegram bot tokens were nested + missed by the
 *  top-level regex, leaking via /api/me + plaintext on disk. */
export const PROFILE_SECRET_PATHS = [
  'telegram.botToken',
...
/**
 * Resolve the OE master key. Throws loudly when the key is missing on a
 * populated install (see history below the function). Exported so sibling
 * modules (lib/encrypted-file.mjs) reuse the same key + guard rails — do
 * NOT inline `getUserKey('_system')` elsewhere.
 */
...
// ── Profile.json secret handling ────────────────────────────────────────────
// Same primitives, different path list. Profile secrets are explicit
// (PROFILE_SECRET_PATHS) — no regex scan, because profile.json has many
// nested objects we don't want to fingerprint by name.

/**
 * Encrypt every PROFILE_SECRET_PATH in `profile` that's currently plaintext.
 * Mutates in place. Idempotent.
 * @param {any} profile
 */
export function encryptProfileSecrets(profile) {
  const stats = { encrypted: 0, alreadyEncrypted: 0, missing: 0 };
  if (!profile || typeof profile !== 'object') return stats;
  for (const p of PROFILE_SECRET_PATHS) {
    const v = getAtPath(profile, p);
    if (v == null || v === '') { stats.missing++; continue; }
    if (isEncryptedField(v)) { stats.alreadyEncrypted++; continue; }
    if (typeof v !== 'string') { stats.missing++; continue; }
    setAtPath(profile, p, encryptValue(v));
    stats.encrypted++;
  }
  return stats;
}
...
/**
 * Return a NEW profile object with every PROFILE_SECRET_PATH decrypted.
 * Use this everywhere a profile is loaded for runtime use (telegram send,
 * webhook validation, etc.) so callers continue to see plaintext.
 * @param {any} profile
 */
export function decryptedProfileView(profile) {
  ...
  for (const p of PROFILE_SECRET_PATHS) {
    ...
    if (isEncryptedField(v)) {
      try { setAtPath(out, p, decryptValue(v)); }
        ...
        console.warn(`[config-secrets] profile decrypt failed for ${p}:`, e.message);
        setAtPath(out, p, '');
      }
    }
  }
...
/**
 * Read one profile secret by dotted path. Handles both encrypted envelopes
 * and legacy plaintext.
 * @param {any} profile
 * @param {string} dottedPath
 * @returns {string|null}
 */
export function getProfileSecret(profile, dottedPath) {
  const v = getAtPath(profile, dottedPath);
  if (v == null || v === '') return null;
  if (isEncryptedField(v)) {
    try { return decryptValue(v); }
    catch (e) { console.warn(`[config-secrets] profile decrypt failed for ${dottedPath}:`, e.message); return null; }
  }
...
/**
 * Boot migration for per-user profile.json files. Walks every directory
 * under USERS_DIR that has a profile.json and encrypts any
 * PROFILE_SECRET_PATH that's still plaintext. Same _allowKeyCreation
 * dance as the config bootstrap — if any profile already has encrypted
 * entries, we must not silently regen the key.
 *
 * @param {{ atomicWriteSync: (p:string, s:string)=>void, log?: any }} args
 */
export async function bootstrapProfileEncryption({ atomicWriteSync, log = null }) {
  const fs = await import('fs');
  const stats = { scanned: 0, encrypted: 0, alreadyEncrypted: 0, errored: 0 };
  if (!fs.existsSync(USERS_DIR)) return stats;
  // First pass: detect whether the system already has any encrypted profile
  // secrets (so we know whether to allow key creation if it's missing).
  let anyEncrypted = false;
  let anyPlaintext = false;
  const userDirs = [];
  for (const entry of fs.readdirSync(USERS_DIR, { withFileTypes: true })) {
    if (!entry.isDirectory()) continue;
    if (!entry.name.startsWith('user_')) continue; // skip _system, default, etc.
    const profilePath = path.join(USERS_DIR, entry.name, 'profile.json');
    if (!fs.existsSync(profilePath)) continue;
    userDirs.push(profilePath);
    try {
      const raw = JSON.parse(fs.readFileSync(profilePath, 'utf8'));
      for (const p of PROFILE_SECRET_PATHS) {
        const v = getAtPath(raw, p);
        if (v == null || v === '') continue;
        if (isEncryptedField(v)) anyEncrypted = true;
        else if (typeof v === 'string') anyPlaintext = true;
      }
    } catch { stats.errored++; }
  }
  stats.scanned = userDirs.length;
  if (!anyPlaintext) {
    (log?.info ?? (() => {}))(`[config-secrets] profile bootstrap: no plaintext profile secrets (encrypted=${anyEncrypted})`);
    return stats;
  }
  // Migrate. Allow key creation only if there are no encrypted secrets yet
  // anywhere (config OR profile) — otherwise we'd be racing the loud guard.
  const needsKeyCreation = anyPlaintext && !anyEncrypted;
  if (needsKeyCreation) _allowKeyCreation = true;
  try {
    for (const profilePath of userDirs) {
      try {
        const raw = JSON.parse(fs.readFileSync(profilePath, 'utf8'));
        const r = encryptProfileSecrets(raw);
        if (r.encrypted > 0) {
          atomicWriteSync(profilePath, JSON.stringify(raw, null, 2));
          stats.encrypted += r.encrypted;
        }
        stats.alreadyEncrypted += r.alreadyEncrypted;
      } catch (e) {
        stats.errored++;
        (log?.warn ?? console.warn)(`[config-secrets] profile bootstrap failed for ${profilePath}:`, e.message);
      }
    }
  } finally {
    if (needsKeyCreation) _allowKeyCreation = false;
  }
  (log?.info ?? console.log)(`[config-secrets] migrated ${stats.encrypted} plaintext profile secret(s) across ${stats.scanned} user(s)`);
  return stats;
}
