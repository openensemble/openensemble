...
// Source-version tracking lets the extension self-reload on mismatch.
...
// "last known good hash" in chrome.storage.local — when it reconnects
// (across either an OE restart OR an extension file change), it compares
// the new server hash to the stored one. Different → chrome.runtime.reload()
// which forces Chrome to re-read background.js etc. from disk.
//
