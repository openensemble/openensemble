...
function writeMirror(userId, mirror) {
  ...
  signalWorkEvent(userId, { kind: 'calendar' });
}
...
export function pruneEvent(e, calId) {
  ...
  if (Array.isArray(e.attendees)) out.attendeeCount = e.attendees.length;
  const self = e.attendees?.find(attendee => attendee.self);
  if (self?.responseStatus) out.selfResponse = self.responseStatus;
