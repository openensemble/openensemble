...
// Defense-in-depth — the parent_host fields below land interpolated into
// shell commands run on storage hosts (zfs/btrfs CLI). Validation also
// happens at write time in skills/nodes/execute.mjs:node_set_parent_host;
// repeating it here means exec sites refuse even if a record is poisoned by
// some other path or migrated forward from before the validator existed.
const _SHELL_SAFE = /^[A-Za-z0-9._/@:+-]+$/;
function assertShellSafe(label, value) {
  if (value == null) return value;
  const s = String(value);
  if (!_SHELL_SAFE.test(s)) {
    throw new Error(`refusing to interpolate "${label}" into shell command — disallowed characters`);
  }
  return s;
}
...
export async function captureHostSnapshot(parent_host, opId, ctx = {}) {
  ...
  if (parent_host.type === 'zfs') {
    ...
    assertShellSafe('parent_host.dataset', parent_host.dataset);
    const snapname = snapshotNameFromOpId(opId);
  ...
  if (parent_host.type === 'btrfs') {
    ...
    assertShellSafe('parent_host.subvolume', parent_host.subvolume);
    assertShellSafe('parent_host.snapshot_dir', parent_host.snapshot_dir);
    const snapname = snapshotNameFromOpId(opId);
...
export async function rollbackToHostSnapshot(parent_host, hostSnapshot, ctx = {}) {
  ...
  if (hostSnapshot.type === 'zfs') {
    ...
    try { assertShellSafe('host_snapshot.dataset', hostSnapshot.dataset); assertShellSafe('host_snapshot.snapname', hostSnapshot.snapname); }
    catch (e) { return { outcome: 'failure', message: e.message }; }
...
export async function deleteHostSnapshot(parent_host, hostSnapshot, ctx = {}) {
  ...
  if (hostSnapshot?.type === 'btrfs') {
    ...
    try { assertShellSafe('host_snapshot.snapshot_path', hostSnapshot.snapshot_path); }
    catch (e) { return { deleted: false, reason: e.message }; }
