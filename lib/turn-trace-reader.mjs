...
/**
 * Sum the per-span tool-router savings (full/kept/dropped/droppedBytes) across a
 * tree. Returns null when no span recorded routing (e.g. a pure fast-path turn).
 */
function rollupToolRouter(spans) {
  const trs = spans.map(s => s?.toolRouter).filter(Boolean);
  if (!trs.length) return null;
  const sum = (k) => trs.reduce((n, x) => n + (Number(x?.[k]) || 0), 0);
  return {
    full: sum('full'),
    kept: sum('kept'),
    dropped: sum('dropped'),
    droppedBytes: sum('droppedBytes'),
    recovered: trs.some(x => x.recovered === true),
  };
}

/**
 * Raw turn metas (chronological, as logged) for aggregators that need the
 * span-level detail the tree rollup flattens away: per-tool ok/ms, per-span
 * provider, span error strings. Used by lib/reliability-metrics.mjs.
 * @param {{ tail?: number, since?: number, userId?: string|null }} [opts]
 */
export function listTurns({ tail, since, userId = null } = {}) {
  const turns = loadTurns({ tail, since });
  return userId ? turns.filter(t => t.userId === userId) : turns;
}
...
export function listTurnTrees({ tail, since, userId = null, limit = 50 } = {}) {
  ...
  const trees = [...byRoot.entries()].map(([rootId, ts]) => {
    ...
    return {
      ...
      cachedTok: sumTok(spans, 'cachedTok'),
      cacheCreateTok: sumTok(spans, 'cacheCreateTok'),
      toolRouter: rollupToolRouter(spans),
