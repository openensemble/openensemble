...
export const INFRASTRUCTURE_DEFAULT_TOOL_RE =
  /^(?:coder_|skill_|node_exec$|ask_agent$|spawn_worker$|check_workers$|stop_worker$|request_tools$|dispatch_op$|read_turns$|read_logs$|claim_role$|list_roles$)/i;

export const ADMIN_DEFAULT_TOOL_RE =
  /^(?:youtube_add_channel|youtube_remove_channel|youtube_list_channels|update_watch|create_watch|delete_watch|cancel_watch|list_watch_items)$/i;
...
export const PER_CALL_ARG_NAMES = new Set([
  ...
  'phrase', 'keyword', 'term',
  'location', 'channel', 'alias', 'deliver', 'cadence', 'interval', 'repeat',
...
export function isInfrastructureDefaultTool(toolName) {
  const t = String(toolName || '');
  return INFRASTRUCTURE_DEFAULT_TOOL_RE.test(t) || ADMIN_DEFAULT_TOOL_RE.test(t);
}
...
export function isPerCallArgName(argName) {
  ...
  // Flags, binary knobs, enums, and execution modes are usually selected by the
  // agent for a specific call. Repetition is not evidence of a user preference.
  if (/^(?:is_|has_|can_|should_|use_|include_|exclude_|enable_|disable_|skip_)/i.test(a)) return true;
  if (/^(?:watch|terminal|background|foreground|recursive|force|dry_?run|confirm|verbose|debug|mode|type|format|voice_?device|skip_?smoke)$/i.test(a)) return true;
...
export function isDefaultArgNoise(toolName, argName, value) {
  if (isInfrastructureDefaultTool(toolName)) return true;
...
function normalizeAuthoredText(text) {
...
export function isUserAuthoredDefaultValue(value, userText) {
  if (!userText || value === null || value === undefined) return false;
  const t = typeof value;
  const haystack = normalizeAuthoredText(userText);
  if (!haystack) return false;
  if (t === 'string') {
    const needle = normalizeAuthoredText(value);
  ...
  if (t === 'number') {
    ...
    return new RegExp(`(^|[^0-9.])${escaped}([^0-9.]|$)`).test(String(userText));
  }
  return false;
}
