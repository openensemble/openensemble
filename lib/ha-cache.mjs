 ...
 * through Sydney → Helen with 5 gpt-5.5 turns (~28s).
 *
...
async function fetchAndIndex() {
  ...
  for (const e of data) {
    ...
    // likely to say "turn on/off/toggle" or "activate/run" against.
    if (!FAST_PATH_DOMAINS.has(domain)) continue;
    ...
    if (!idx.has(norm)) idx.set(norm, { entity_id: id, domain, friendly_name: fnRaw });
  }
  return idx;
}
