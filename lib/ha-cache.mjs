...
// Read-only side index, refreshed by the same fetchAndIndex() pass that
// builds _idx. camera/weather are deliberately NOT in FAST_PATH_DOMAINS
// (they'd pollute the "turn on X" match space — see the comment there), but
// the TV integration needs them: tv_show_camera resolves camera names
// (skills/role_tv_control/execute.mjs) and the TV dashboard shows weather
// (lib/tv-dashboard.mjs). Collecting them here means those surfaces ride the
// one /states fetch this cache already makes instead of pulling their own.
let _side = { cameras: [], weather: [] };
...
async function fetchAndIndex() {
  ...
  for (const e of data) {
    ...
    // Side index (see _side above) — collected before the FAST_PATH_DOMAINS
    // skip since neither domain is in it.
    if (domain === 'camera') {
      cameras.push({
        entity_id: id,
        friendly_name: e.attributes?.friendly_name || id.slice('camera.'.length).replace(/_/g, ' '),
      });
      continue;
    }
    if (domain === 'weather') {
      ...
      weather.push({
        ...
        temperature: temp != null ? temp : null,
      });
      continue;
    }
  ...
  // Swap the side index in the same pass so it can never be fresher or staler
  // than _idx (both come from this one /states response).
  _side = { cameras, weather };
  return idx;
}

/**
 * Cached camera entities: [{entity_id, friendly_name}]. Same freshness as
 * the fast-path index (≤5 min + stale-while-revalidate) — fine for name
 * resolution; the snapshot itself is always fetched live via /camera_proxy.
 */
export async function listCameras() {
  await ensureCache();
  return _side.cameras;
}

/**
 * Cached weather entities: [{entity_id, state, temperature}]. State/temp are
 * snapshot at index time (≤5 min stale) — acceptable for a dashboard
 * greeting card, and it makes weather display cost zero extra HA calls.
 */
export async function listWeatherStates() {
  await ensureCache();
  return _side.weather;
}
...
export async function ensureCache(force = false) {
  ...
  // Stale-while-revalidate: when HA is slow/down, awaiting the re-fetch (15s
  // timeout) used to stall every interactive "turn off the kitchen" — even
  // the alias fast-path — before the LLM fallback. Serve the stale index we
  // already hold and let the refresh land in the background; await only when
  // there's nothing cached yet (or the caller explicitly forced a refresh).
  if (_idx && !force) return _idx;
  return _inflight;
}
