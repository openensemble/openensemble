 ...
 *
 * Decision matrix:
 *   - node Auto-fix disabled or not fully onboarded → never auto. Always propose.
 *   - profile.trust_state == 'unverified'  → never auto. Always propose.
 *   - profile.trust_state == 'reviewed'    → may auto-apply low-risk only.
 *   - profile.trust_state == 'proven'      → may auto-apply low + medium risk.
 *   - high-risk fixes always propose, regardless of trust_state. Op must
 *     also exist and be `verified` (the runtime has actually exercised it
 *     successfully at least once) before auto-apply is considered.
 *
...
async function nodeAutoFixEnabled(userId, nodeId) {
  try {
    const { getNodeAutoFixEnabled } = await import('../skills/nodes/node-registry.mjs');
    const enabled = getNodeAutoFixEnabled(nodeId, userId);
    // Unit/integration tests often exercise profiles without a registered node.
    // In that case preserve the historical profile-only behavior.
...
function shouldAutoApply(profile, fix, op, autoFixEnabled) {
  if (!autoFixEnabled) return false;
  if (!op) return false;
  if (!op.verified) return false;
  if (profile.trust_state === 'unverified') return false;
  // High-risk fixes always require user confirmation, even at proven.
  if (fix.risk === 'high') return false;
  // Medium-risk fixes need proven; reviewed-only stops at low.
  if (fix.risk === 'medium' && profile.trust_state !== 'proven') return false;
  return true;
}
...
export async function proposeFix(input) {
  ...
  const autoFixOn = await nodeAutoFixEnabled(userId, nodeId);
  if (shouldAutoApply(profile, fix, op, autoFixOn)) {
  ...
  appendIncidentEvent(userId, nodeId, incidentId, {
    ...
    payload: {
      ...
      reason_not_auto: !op.verified
        ...
        : !autoFixOn
          ? 'node Auto-fix is off'
          : profile.trust_state === 'unverified'
          ? 'profile trust_state is unverified'
          : fix.risk === 'high'
            ? 'high-risk fixes always require user confirmation'
            : (fix.risk === 'medium' && profile.trust_state !== 'proven')
              ? `medium-risk fix needs trust_state=proven (current: ${profile.trust_state})`
