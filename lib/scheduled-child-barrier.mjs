...
// Give an aborted child a short window to persist its terminal journal entry,
// publish its cancellation surface, and acknowledge the barrier. Finalizing
// immediately after AbortController.abort() races those durable terminal paths;
// waiting forever recreates the hung occurrence the watchdog exists to break.
...
function getGroup(userId, scheduledCtx) {
  ...
  if (!group) {
    ...
    group = {
      ...
      timingOut: false,      // watchdog owns drain/finalization from this point
      finalized: false,
      finalizedPromise,
      resolveFinalized,
      watchdog: null,
...
function waitForSettlements(promises, timeoutMs) {
...
async function watchdogTimeout(group) {
  if (group.finalized || group.timingOut || !groups.has(group.key)) return;
  // Claim the group synchronously, before invoking arbitrary cancellation
  // callbacks. A child may acknowledge from inside cancel(); its scheduled
  // drain must not start a continuation or finalize as a success.
  group.timingOut = true;
  if (group.watchdog) { clearTimeout(group.watchdog); group.watchdog = null; }

  const pending = pendingChildren(group);
  const timeoutMessage = `timed out after ${WATCHDOG_MS / 60_000} minutes`;
  console.warn('[scheduled-barrier] watchdog cancelling overdue work', group.key, 'pending:', pending.length);

  const cancellableSettlements = [];
  for (const child of pending) {
    child.cancelRequestedAt = Date.now();
    if (typeof child.cancel !== 'function') continue;
    cancellableSettlements.push(child.settledPromise);
    try {
      // Invoke every owner before awaiting any one of them. Promise-returning
      // callbacks are intentionally not trusted as terminal acknowledgement;
      // completeScheduledChild is the durable handoff boundary.
      Promise.resolve(child.cancel(`Scheduled child ${child.childId} ${timeoutMessage}`))
        .catch((error) => { child.cancelError = error?.message || String(error); });
    } catch (error) {
      child.cancelError = error?.message || String(error);
    }
  }

  const cancellationSettled = await waitForSettlements(
    cancellableSettlements,
    CANCELLATION_GRACE_MS,
  );
  if (group.finalized || !groups.has(group.key)) return;

  // Children that cooperated already recorded their real cancellation terminal
  // result. Any owner still running after the grace is quarantined by the
  // finalized run tombstone and represented honestly as a timeout.
  for (const child of pending) {
    if (child.status !== 'running') continue;
    child.status = 'error';
    child.endedAt = Date.now();
    child.errorMsg = child.cancelError
      ? `${timeoutMessage}; cancellation failed: ${child.cancelError}`
      : `${timeoutMessage}${typeof child.cancel === 'function' && !cancellationSettled ? '; cancellation did not settle' : ''}`;
    child.completionSeq = ++group.completionSeq;
    child.resolveSettled?.({ status: 'error', timedOut: true });
  }
...
function armWatchdog(group) {
  if (group.timingOut || group.finalized) return;
  if (group.watchdog) clearTimeout(group.watchdog);
  group.watchdog = setTimeout(() => {
    watchdogTimeout(group).catch(error => {
      console.warn('[scheduled-barrier] watchdog cancellation failed:', error?.message || error);
      if (!group.finalized && groups.has(group.key)) {
        group.continuationErrors.push(error?.message || String(error));
        group.continuationFailed = true;
        finalizeGroup(group, { timedOut: true });
      }
    });
...
function scheduleDrain(group) {
  if (group.drainScheduled || group.finalized || group.timingOut) return;
...
async function maybeDrain(group) {
  if (!groups.has(group.key) || group.finalized || group.timingOut || group.draining) return;
 ...
 */
export function registerScheduledChild({ userId, scheduledCtx, childId, label = '', kind = 'background', cancel = null }) {
  pruneGroups();
  if (!keyFor(userId, scheduledCtx) || !childId) return null;
  const group = getGroup(userId, scheduledCtx);
  if (!group || group.timingOut) return null;
  if (!group.children.has(childId)) {
    ...
    if (group.watchdog) armWatchdog(group);
  } else if (!group.children.get(childId)?.cancel && typeof cancel === 'function') {
    // Idempotent registration may race owner setup; retain the first real
    // cancellation handle instead of leaving an uncancellable bookkeeping row.
    group.children.get(childId).cancel = cancel;
  }
  group.updatedAt = Date.now();
...
export function completeScheduledChild({ userId, scheduledCtx, childId, resultText = '', errorMsg = null }) {
  ...
  child.completionSeq = ++group.completionSeq;
  child.resolveSettled?.({ status: child.status, timedOut: false });
  group.updatedAt = Date.now();
  if (group.watchdog) armWatchdog(group);
  scheduleDrain(group);
...
export function getScheduledChildGroup({ userId, scheduledCtx }) {
  ...
  return {
    ...
    timingOut: group.timingOut === true,
  };
}
