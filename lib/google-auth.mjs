...
export async function ensureFreshToken(tokenPath) {
  // readEncryptedJsonFile transparently handles legacy plaintext token files;
  // writeEncryptedJsonFile always writes the encrypted envelope, so the very
  // next refresh re-saves it encrypted.
  let tokens = readEncryptedJsonFile(tokenPath);
  if (!tokens.expiry_date || Date.now() > tokens.expiry_date - 60_000) {
    ...
    writeEncryptedJsonFile(tokenPath, tokens, { mode: 0o600 });
  }
