...
function sanitizeTools(tools) {
  ...
  const toolCallId = value => redactTextForTrace(value).slice(0, MAX_TOOL_CALL_ID_CHARS);
  ...
  const identityAnomalies = Array.isArray(tools.identityAnomalies)
    ? tools.identityAnomalies.slice(0, MAX_TOOL_EVENTS).map(anomaly => ({
        kind: 'tool_result_identity_anomaly',
        name: String(anomaly?.name ?? '').slice(0, MAX_TRACE_NAME_CHARS),
        reason: String(anomaly?.reason ?? 'unknown_result_identity').slice(0, 80),
        ...(anomaly?.toolCallId != null
          ? { toolCallId: toolCallId(anomaly.toolCallId) }
          : {}),
        identityType: String(anomaly?.identityType ?? 'unknown').slice(0, 40),
        identityLength: Number.isSafeInteger(anomaly?.identityLength)
          ? anomaly.identityLength
          : null,
        ...(anomaly?.providerNative === true ? { providerNative: true } : {}),
        ...(anomaly?.delegated === true ? { delegated: true } : {}),
        resultPreview: preview(anomaly?.resultPreview),
        observedAt: Number.isFinite(anomaly?.observedAt) ? anomaly.observedAt : null,
      }))
    : [];
  return { usedNames, used, events, identityAnomalies };
}
...
export function recordRunTrace(userId, trace) {
  ...
  try {
    ...
    const entry = {
      ...
      executionTargetExplicit: trace?.executionTargetExplicit === true,
      executionEffortExplicit: trace?.executionEffortExplicit === true,
...
export function listRunTraces(userId, { limit = 50 } = {}) {
  ...
  return readRaw(userId)
    ...
    .map(t => ({
      ...
      provider: t.provider,
      model: t.model,
      executionTargetExplicit: t.executionTargetExplicit === true,
      executionEffortExplicit: t.executionEffortExplicit === true,
      source: t.source,
      ...
      routing: t.routing ? {
        ...
        matchedSkills: t.routing.matchedSkills ?? [],
