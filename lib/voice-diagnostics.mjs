// @ts-check
// Metadata by default; authenticated detail requests may include bounded transcripts.
...
export function summarizeVoiceTurn(row, { includeContent = false } = {}) {
  ...
  return {
    ...
    outcome: ['completed', 'suppressed', 'no_speech', 'stt_dropped', 'stt_failed', 'tts_failed', 'abandoned', 'evicted', 'aborted_disconnect', 'handler_error', 'wake_rejected'].includes(outcome) ? outcome : 'unknown',
    ...(includeContent ? { transcript: typeof row.transcript === 'string' ? row.transcript.slice(0, 2000) : null,
      agentId: typeof row.agentId === 'string' ? row.agentId.slice(0, 160) : null } : {}),
    wake: { slot: Number.isInteger(row.wakeSlot) ? row.wakeSlot : null, score: number(row.wakeAvgProb),
      cutoff: number(row.wakeCutoff), decision: outcome === 'wake_rejected' ? 'rejected' : row.followup ? 'followup' : row.dispatchStartedAt ? 'accepted' : 'capture only' },
...
export function buildVoiceDiagnostics({ userId, device, online, health, turns = [], links = [], now = Date.now(), includeContent = false }) {
  ...
  const recentTurns = ownTurns.sort((a, b) => b.ts - a.ts).slice(0, 10).map(row => summarizeVoiceTurn(row, { includeContent }));
  return {
    serverNow: now,
    device: { id: device.id, name: device.name, online: !!online, muted: !!device.mute_state, firmware: device.fw_version || null },
    checks,
    heartbeatAgeMs,
    room: { samples: (health?.wakeSamples || []).filter(sample => sample.userId === userId && sample.ts >= cutoff && now - sample.ts <= 10 * 60_000)
      .map(({ userId: owner, ...sample }) => sample), units: 'device microphone units' },
    connection: {
      since: cutoff,
