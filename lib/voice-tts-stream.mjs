...
import { noteTurn } from './voice-turn-journal.mjs';
...
// Common public suffixes cover spoken news/source links without mistaking
// product names such as "Node.js" for a web address.
const BARE_WEB_ADDRESS = String.raw`(?:[a-z0-9-]+\.)+(?:co\.(?:uk|jp|in|au|nz|za)|com\.(?:au|br|mx)|com|org|net|gov|edu|io|ai|co|news|info|biz|me|tv|us|uk|ca|au|de|fr|jp|in|app|dev)(?![a-z0-9-]|\.[a-z]{2,}\b)(?::\d+)?(?:[/?#]\S*)?`;

const SPOKEN_NEWS_SOURCES = new Map([
  ['apnews.com', 'AP News'],
  ['reuters.com', 'Reuters'],
  ['marketscreener.com', 'Market Screener'],
  ['bbc.com', 'BBC'],
  ['bbc.co.uk', 'BBC'],
  ['cnn.com', 'CNN'],
  ['npr.org', 'NPR'],
...
function webAddressRegex(flags = 'i') {
  return new RegExp(String.raw`(?:<?(?:https?:\/\/|www\.)\S+|(?<![@\w.])${BARE_WEB_ADDRESS})`, flags);
}

function isWebAddress(value) {
  const candidate = String(value || '').trim();
  return /^(?:<?\s*)?(?:https?:\/\/|www\.)/i.test(candidate)
    // Keep bare host labels case-sensitive so product names such as ASP.NET
    // and Socket.IO remain useful human-readable Markdown labels.
    || new RegExp(`^${BARE_WEB_ADDRESS}$`).test(candidate);
}

function isWebAddressLabel(value) {
  const label = String(value || '').trim();
  return isWebAddress(label)
    || /^(?:source|link|\d+)$/i.test(label);
}

function isMarkdownLinkDestination(value) {
  const destination = String(value || '').trim();
  return isWebAddress(destination)
    // Markdown syntax supplies the context that plain dotted prose lacks, so
    // mixed-case hostnames are safe to recognize here.
    || new RegExp(`^${BARE_WEB_ADDRESS}$`, 'i').test(destination)
    || /^(?:\/|\.{1,2}\/|#|mailto:|ftp:\/\/)/i.test(destination);
}

function looksLikeIncompleteWebDestination(value) {
  const candidate = String(value || '').trim();
  if (!candidate) return true;
  const lower = candidate.toLowerCase();
  return /^(?:https?:\/\/|www\.)/i.test(candidate)
    || /^(?:\/|\.{1,2}\/|#|mailto:|ftp:\/\/)/i.test(candidate)
    || 'http://'.startsWith(lower)
    || 'https://'.startsWith(lower)
    || 'www.'.startsWith(lower)
    || (!/\s/.test(candidate) && isWebAddress(candidate));
}

function hasIncompleteDottedContinuation(text, end) {
  return /^\.[a-z0-9-]*$/i.test(text.slice(end));
}

function spokenBareHostReplacement(text, match) {
  const raw = match[0];
  if (/(?::\d+|[/?#])/.test(raw)) return null;
  const before = text.slice(Math.max(0, match.index - 80), match.index);
  const publisher = SPOKEN_NEWS_SOURCES.get(raw.toLowerCase());
  if (publisher && /\b(?:according\s+to|reported\s+by|citing|via|source\s*:|source\s+is)\s*$/i.test(before)) {
    return publisher;
  }
  if (raw === raw.toLowerCase() && /\b(?:visit|go\s+to)\s*$/i.test(before)) {
    return publisher ? `${publisher} website` : 'the website';
  }
  return null;
}

function isBracketLabelAttached(text, offset) {
  let chainStart = offset;
  // Follow a chain back to its first bracket: matrix[0][1] is attached to an
  // identifier, while a citation chain such as "finding [1][2]" is not.
  while (chainStart > 0 && text[chainStart - 1] === ']') {
    const priorOpen = text.lastIndexOf('[', chainStart - 1);
    if (priorOpen < 0) break;
    chainStart = priorOpen;
  }
  return (chainStart > 0 && /[\w)]/.test(text[chainStart - 1]))
    || text.slice(Math.max(0, chainStart - 2), chainStart) === '?.';
}

function hasBareAddressEvidence(text, match, end) {
  const raw = match[0];
  if (/^<?(?:https?:\/\/|www\.)/i.test(raw)) return true;
  // A locator suffix is unambiguously URL-shaped even if the host is cased.
  // A hostname alone is inherently indistinguishable from a dotted product
  // or config name, so only remove one when citation punctuation confirms it.
  if (/(?::\d+|[/?#])/.test(raw)) return true;
  if (text[match.index - 1] === '(' && text[end] === ')') return true;
  if (spokenBareHostReplacement(text, match)) return true;
  return text.slice(0, match.index).trim() === '' && text.slice(end).trim() === '';
}

/**
 * Collapse complete Markdown web links before sentence/length segmentation.
 * Parenthesized links are provider-added citations and disappear entirely;
 * ordinary links retain a human-readable label but never their address.
 * Incomplete links are deliberately left untouched so the streaming splitter
 * can hold them until a later token supplies the closing parenthesis.
 */
function stripCompletedMarkdownWebLinks(input, { flushAll = false } = {}) {
  const text = String(input || '');
  let out = '';
  let cursor = 0;
  let scanAt = 0;

  while (scanAt < text.length) {
    const labelOpen = text.indexOf('[', scanAt);
    if (labelOpen < 0) break;
    const labelClose = text.indexOf(']', labelOpen + 1);
    if (labelClose < 0 || text.slice(labelOpen, labelClose).includes('\n')) break;

    let destinationOpen = labelClose + 1;
    while (destinationOpen < text.length && /[ \t]/.test(text[destinationOpen])) destinationOpen++;
    if (text[destinationOpen] !== '(') {
      scanAt = labelOpen + 1;
      continue;
    }

    // Markdown destinations may contain balanced parentheses. Walking them
    // avoids truncating links such as https://example.test/wiki/Foo_(bar).
    let depth = 1;
    let destinationClose = destinationOpen + 1;
    for (; destinationClose < text.length; destinationClose++) {
      if (text[destinationClose] === '\\') {
        destinationClose++;
        continue;
      }
      if (text[destinationClose] === '(') depth++;
      else if (text[destinationClose] === ')' && --depth === 0) break;
    }
    if (depth !== 0) {
      const destination = text.slice(destinationOpen + 1);
      let outerOpen = labelOpen - 1;
      while (outerOpen >= cursor && /[ \t]/.test(text[outerOpen])) outerOpen--;
      const tightLinkSyntax = destinationOpen === labelClose + 1;
      const looksLikeCitation = text[outerOpen] === '(';
      // "[draft] (still under review..." is ordinary prose, not a damaged
      // link. Only consume the unfinished tail when its syntax or destination
      // supplies meaningful link evidence.
      if (!tightLinkSyntax && !looksLikeCitation && !looksLikeIncompleteWebDestination(destination)) {
        scanAt = labelOpen + 1;
        continue;
      }
      // At finish, salvage the human label (or drop a domain label/citation)
      // from malformed Markdown rather than speaking brackets and a dangling
      // parenthesis. Before finish, keep waiting for the closing token.
      if (flushAll) {
        let replaceStart = labelOpen;
        const label = text.slice(labelOpen + 1, labelClose).trim();
        let replacement = isWebAddressLabel(label) ? '' : label;
        if (text[outerOpen] === '(') {
          replaceStart = outerOpen;
          replacement = '';
        } else if (text[labelOpen - 1] === '!') {
          replaceStart = labelOpen - 1;
        }
        out += text.slice(cursor, replaceStart) + replacement;
        cursor = text.length;
      }
      break;
    }

    const destination = text.slice(destinationOpen + 1, destinationClose).trim();
    const tightLinkSyntax = destinationOpen === labelClose + 1;
    if (!isMarkdownLinkDestination(destination) && !(tightLinkSyntax && !/\s/.test(destination))) {
      scanAt = destinationClose + 1;
      continue;
    }

    // Hosted web search emits citations as "([domain](https://...))". Drop
    // the wrapper with the link so an empty "()" cannot become a TTS pause.
    let outerOpen = labelOpen - 1;
    while (outerOpen >= cursor && /[ \t]/.test(text[outerOpen])) outerOpen--;
    let outerClose = destinationClose + 1;
    while (outerClose < text.length && /[ \t]/.test(text[outerClose])) outerClose++;
    // A provider delta can end after the Markdown link's close but before the
    // citation wrapper's close. Keep the whole construct until that next
    // character arrives, especially when the label itself is human-readable.
    if (text[outerOpen] === '(' && outerClose === text.length && !flushAll) break;

    let replaceStart = labelOpen;
    let replaceEnd = destinationClose + 1;
    const label = text.slice(labelOpen + 1, labelClose).trim();
    let replacement = isWebAddressLabel(label) ? '' : label;

    if (text[outerOpen] === '(' && (text[outerClose] === ')' || (flushAll && outerClose === text.length))) {
      replaceStart = outerOpen;
      replaceEnd = text[outerClose] === ')' ? outerClose + 1 : outerClose;
      replacement = '';
    } else if (text[labelOpen - 1] === '!') {
      replaceStart = labelOpen - 1;
    }
...
/**
 * Remove only URL tokens known to be complete. At stream time, a URL at the
 * end of the current buffer may receive more characters in the next delta;
 * retaining it prevents a later scheme-less tail from being spoken. finish()
 * passes flushAll=true, at which point an end-of-reply URL is safe to drop.
 */
function stripCompletedBareWebAddresses(input, { flushAll = false } = {}) {
  const text = String(input || '');
  const addressRe = webAddressRegex('gi');
  let out = '';
  let cursor = 0;
  let match;
  while ((match = addressRe.exec(text)) !== null) {
    const end = match.index + match[0].length;
    if (!hasBareAddressEvidence(text, match, end)) continue;
    if (!flushAll && (end === text.length || hasIncompleteDottedContinuation(text, end))) continue;
    const spokenReplacement = spokenBareHostReplacement(text, match);
    // Keep prose punctuation outside the removed address. Without this,
    // "...at https://example.com. Next story..." becomes one run-on phrase.
    const trailingSyntax = match[0].match(/[.,!?;:'"”’»)>\]]+$/)?.[0] ?? '';
    const trailingPunctuation = trailingSyntax.replace(/['"”’»)>\]]/g, '');
    let replaceStart = match.index;
    const addressWithoutTrailingSyntax = match[0].slice(0, match[0].length - trailingSyntax.length);
    const opener = text[replaceStart - 1];
    const matchingCloser = { '(': ')', '[': ']', '"': '"', "'": "'", '“': '”', '‘': '’', '«': '»' }[opener];
    if (matchingCloser && trailingSyntax.includes(matchingCloser)) replaceStart--;
    else if (text[replaceStart - 1] === '(' && addressWithoutTrailingSyntax.endsWith(')')) replaceStart--;
    out += text.slice(cursor, replaceStart) + (spokenReplacement || '') + trailingPunctuation;
    cursor = end;
  }
  return out + text.slice(cursor);
}

function prepareVoiceTextForSegmentation(input, { flushAll = false } = {}) {
  const linksStripped = stripCompletedMarkdownWebLinks(input, { flushAll });
  if (flushAll) return stripCompletedBareWebAddresses(linksStripped, { flushAll: true });
  const protectedStart = protectedStreamingTailStart(linksStripped);
  if (protectedStart == null) return stripCompletedBareWebAddresses(linksStripped);
  return stripCompletedBareWebAddresses(linksStripped.slice(0, protectedStart))
    + linksStripped.slice(protectedStart);
}

// If a streamed Markdown link or bare URL is incomplete, soft length
// chunking must stop before the whole construct — never in its address.
function protectedStreamingTailStart(text) {
  let start = null;
  const markdownTail = text.match(/(?:\(\s*)?!?\[[^\]\n]*(?:\]([ \t]*)(?:\(([^\n)]*))?)?$/);
  if (markdownTail) {
    const spacing = markdownTail[1];
    const destination = markdownTail[2];
    const hasOuterWrapper = /^\(\s*!?\[/.test(markdownTail[0]);
    const shouldProtect = destination == null
      || spacing.length === 0
      || hasOuterWrapper
      || looksLikeIncompleteWebDestination(destination);
    if (shouldProtect) start = markdownTail.index;
  }
  // The inner Markdown link can be complete one delta before a citation's
  // outer ")" arrives. Keep that whole wrapper candidate too, including an
  // optional Markdown title whose whitespace makes it look like a bare URL.
  const wrappedLinkTail = text.match(/\(\s*!?\[[^\]\n]*\][ \t]*\([^\n]*\)$/);
  if (wrappedLinkTail) start = start == null
    ? wrappedLinkTail.index
    : Math.min(start, wrappedLinkTail.index);

  const addressRe = webAddressRegex('gi');
  let match;
  while ((match = addressRe.exec(text)) !== null) {
    const end = match.index + match[0].length;
    if (end !== text.length && !hasIncompleteDottedContinuation(text, end)) continue;
    let urlStart = match.index;
    const prefix = text.slice(0, urlStart);
    const markdownPrefix = prefix.match(/(?:\(\s*)?!?\[[^\]\n]*\][ \t]*\($/);
    if (markdownPrefix) urlStart = markdownPrefix.index;
    start = start == null ? urlStart : Math.min(start, urlStart);
  }
  return start;
}

export function normalizeSegmentText(s) {
  return stripCompletedBareWebAddresses(
    stripCompletedMarkdownWebLinks(String(s || ''), { flushAll: true }),
    { flushAll: true },
  )
    .replace(/\r\n?/g, '\n')
    .replace(/\*\*([^*]+)\*\*/g, '$1')
    ...
    .replace(/\[([^\]\n]+)\][ \t]*\(?/g,
      (m, label, offset, whole) => {
        const attached = isBracketLabelAttached(whole, offset);
        return isWebAddressLabel(label) && !attached ? ' ' : m;
      })                                                     // malformed/incomplete citation residue
    .replace(/\[([^\]\n]+)\]/g, ' $1 ')                   // preserve array[0] as spoken "array 0"
    .replace(/[\[\]]/g, ' ')                                // truncated bracket markup
    .replace(/\?\.\s+(?=\w)/g, ' ')                      // value?.[0] → spoken "value 0"
    .replace(/\(\s*\)/g, ' ')                               // wrapper left by stripped link
    ...
    .replace(/\s+([,.;:!?])/g, '$1')                        // "2026 , 5 AM" → "2026, 5 AM"
    .replace(/[ \t]*\n+[ \t]*/g, ' ')
    .replace(/\s+/g, ' ')
    .replace(/^[,.\s]+/, '')
    .trim();
...
function endsWithNonTerminalAbbreviation(text) {
  const prefix = text.trimEnd();
  return /(?:^|[^A-Za-z])(?:[A-Za-z]\.){2,}$/.test(prefix)
    || /\b(?:Mr|Mrs|Ms|Dr|Prof|Sen|Rep|Gov|Gen|Lt|Col|Capt|Sgt|St|Mt|Jr|Sr|Inc|Corp|Co|Ltd|No|vs|etc|Jan|Feb|Mar|Apr|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)\.$/i.test(prefix);
}

function boundaryCandidates(text, { flushAll = false } = {}) {
  const out = [];
  // At stream time, punctuation at the current buffer end is ambiguous: the
  // next delta can turn "U." into "U.S." or "example." into "example.com".
  // Wait for a following whitespace character; finish() makes true terminal
  // punctuation eligible via flushAll.
  const sentenceRe = flushAll ? /[.!?]+(?=\s|$)/g : /[.!?]+(?=\s)/g;
  let m;
  while ((m = sentenceRe.exec(text)) !== null) {
    const splitEnd = m.index + m[0].length;
    // Initialisms are not sentence endings while more text may still arrive:
    // "overnight U.S. attacks" must remain one spoken thought. If an
    // initialism truly ends the reply, flushAll emits it as the final tail.
    if (/^\.+$/.test(m[0]) && endsWithNonTerminalAbbreviation(text.slice(0, splitEnd))) continue;
...
function findHardBoundary(text, { flushAll = false } = {}) {
  for (const b of boundaryCandidates(text, { flushAll })) {
...
function findSoftBoundary(text, { flushAll = false } = {}) {
  if (text.length < MAX_SEGMENT_CHARS) return null;
  const protectedStart = protectedStreamingTailStart(text);
  const limit = Math.min(MAX_SEGMENT_CHARS, protectedStart ?? text.length);
  if (limit < MIN_SENTENCE_CHARS) return null;
  ...
  // Prefer a somewhat shorter clause ending over an arbitrary split near the
  // target size. This avoids pauses such as "Command says it targeted."
  // followed by "military facilities." when a natural semicolon came earlier.
  for (let i = Math.min(SOFT_SEGMENT_CHARS - 1, limit); i >= MIN_SOFT_PUNCT_CHARS; --i) {
    const ch = text[i - 1];
    const next = text[i] || '';
    if ((ch === ',' || ch === ';' || ch === ':') && /\s/.test(next)) return { splitEnd: i, consumeEnd: consumeWhitespaceAfter(text, i), kind: 'soft-punct' };
  }
  // With no punctuation nearby, start the next segment at a clause word. A
  // pause before "as competition..." sounds intentional; one after
  // "competition over..." sounds like the speaker lost its place.
  const clauseRe = /\s+(?=(?:and|but|while|as|because|which|who|that)\s)/gi;
  let clauseBoundary = null;
  let clauseMatch;
  while ((clauseMatch = clauseRe.exec(text.slice(0, limit + 1))) !== null) {
    if (clauseMatch.index < MIN_SOFT_PUNCT_CHARS) continue;
    if (endsWithNonTerminalAbbreviation(text.slice(0, clauseMatch.index))) continue;
    clauseBoundary = {
      splitEnd: clauseMatch.index,
      consumeEnd: consumeWhitespaceAfter(text, clauseMatch.index),
      kind: 'soft-clause',
    };
  }
  if (clauseBoundary) return clauseBoundary;
  for (let i = limit; i >= SOFT_SEGMENT_CHARS; --i) {
    if (/\s/.test(text[i - 1]) && !endsWithNonTerminalAbbreviation(text.slice(0, i))) {
      return { splitEnd: i, consumeEnd: consumeWhitespaceAfter(text, i), kind: 'soft-length' };
    }
  }
  ...
  // The current token may be an address whose final ".com" has not arrived
  // yet. Never force-cut a live unbroken token; wait for whitespace to prove
  // it complete. At finish, a genuinely long word still gets bounded chunks.
  if (!flushAll && !/\s/.test(text.slice(limit))) return null;
  return { splitEnd: limit, consumeEnd: limit, kind: 'soft-length' };
}

export function splitVoiceTtsSegments(text, { flushAll = false } = {}) {
  let rest = prepareVoiceTextForSegmentation(text, { flushAll }).replace(/^\s+/, '');
  const segments = [];
  ...
  const punctuate = (seg) => {
    ...
    if (/[,;:]$/.test(seg)) return `${seg.slice(0, -1)}.`;
    return `${seg}.`;
  };

  while (rest) {
    const hardBoundary = findHardBoundary(rest, { flushAll });
    // A complete sentence can arrive in one large provider delta. Keep the
    // latency/quality bound in that case too, preferring a natural soft pause
    // within it over sending a 150–250 character synth request at once.
    const boundary = hardBoundary?.splitEnd <= MAX_SEGMENT_CHARS
      ? hardBoundary
      : findSoftBoundary(rest, { flushAll }) || hardBoundary;
    if (!boundary) break;
  ...
  if (flushAll) {
    while (rest.length >= MAX_SEGMENT_CHARS) {
      const boundary = findSoftBoundary(rest, { flushAll: true });
      if (!boundary) break;
      const segment = normalizeSegmentText(rest.slice(0, boundary.splitEnd));
...
export function createVoiceTtsStreamer({ send, isOpen, cfg, refPath, voice, log, bufferedAmount, turnId = null, beginText = null }) {
  ...
  let firstSynthesisRecorded = false, firstAudioRecorded = false;
  ...
  async function pump() {
    ...
    try {
      ...
      while (!aborted && inFlight) {
        ...
        if (!firstSynthesisRecorded) {
          firstSynthesisRecorded = true;
          noteTurn(turnId, { ttsFirstSynthesisMs: r.synthMs }, 'tts');
        } else {
          noteTurn(turnId, {}, 'tts');
        }
        ...
        for (let off = 0; off < pcm.length && !aborted && isOpen(); off += FRAME_BYTES) {
          ...
          if (!firstAudioRecorded) {
            ...
            noteTurn(turnId, { ttsFirstAudioAt: Date.now() }, 'tts');
          }
  ...
  const api = {
    ...
    get audioSent() { return firstAudioRecorded; },
    get finished() { return finished; },
    get aborted() { return aborted; },
    get closed() { return closed; },
    // True only between the device's tts_pause and its tts_resume, which it
    // sends solely to verify a barge candidate. So this reading IS "a barge
    // verify is open on this device right now" — the window in which a
    // capture that matches our own reply text is self-audio rather than a
    // user interrupting. See matchesOwnSpeech in lib/voice-policy.mjs.
    get paused() { return paused; },
    // Immutable ownership tag used by device pause/resume correlation. A
    // turnless announcement exposes null and cannot be controlled by a scoped
    // retry from an older reply.
