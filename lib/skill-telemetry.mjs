...
export function recordToolInvocations({ userId, toolsUsed }) {
  ...
  for (const t of toolsUsed) {
    ...
    eventBatch.push({ ts: now, toolName: t.name, skillId });
    changed = true;
  }
  if (changed) {
    ...
    // Phase-5: per-event timestamps for the skill_proposal outcome measurer.
    // Fire-and-forget; cumulative counters above are the canonical store, the
    // event log is only used for windowed outcome counts.
    import('./invocation-events.mjs').then(m =>
      m.appendInvocationEvents(userId, eventBatch)
    ).catch(e => console.warn('[skill-telemetry] invocation-event append failed:', e.message));
  }
}
...
export async function recordCorrection({ userId, agentId, correctionText }) {
  ...
  // Side-channel: timestamped correction log for outcome measurers, even if
  // attribution to a specific skill fails. We log per-agent always; per-skill
  // only when attribution succeeds (below). Fire-and-forget so a slow log
  // write doesn't block the cortex hot path.
  import('./correction-events.mjs').then(m =>
    m.appendCorrectionEvent(userId, { agentId, skillId: null, text: correctionText })
  ).catch(e => console.warn('[skill-telemetry] correction-event append failed:', e.message));
  ...
  // Re-emit the correction event with skillId now that attribution succeeded —
  // measurers that filter on skillId will see both the per-agent and per-skill
  // rows. Storage is bounded by the 30d retention sweep.
  import('./correction-events.mjs').then(m =>
    m.appendCorrectionEvent(userId, { agentId, skillId: last.skillId, text: correctionText })
  ).catch(() => {});
