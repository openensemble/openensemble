 ...
 */
export async function buildContextHints(userId, text, opts = {}) {
  if (!userId || typeof text !== 'string' || text.length < 4) {
    return { hints: '', resolutions: [] };
  }
  // Alias resolution may happen before a nested stream starts. Inherit the
  // authenticated verifier boundary here so indirect callers cannot persist
  // fallback aliases or delete stale ones while resolving context.
  const resolverOpts = getTurnContext()?.suppressLearning === true
    ? { ...opts, suppressLearning: true }
    : opts;
  const settled = await Promise.allSettled(RESOLVERS.map(async (r) => {
    try {
      ...
      const resolution = await resolveFn(userId, text, resolverOpts);
  ...
  try {
    ...
    const res = await fw.resolveFromMessage(userId, text, resolverOpts);
    if (res) {
