 ...
 * disambiguate:
 *   1. full sender ADDRESS  (alex@example.com)  — always recorded.
...
// Mtime-checked parse cache. suggestLabels runs once per email inside the
// sort loop — a 200-email sort re-parsed (and re-migrated) the whole store
// 200 times. Mutators here follow a synchronous load→mutate→saveStore
// sequence, and saveStore invalidates, so the cache never serves unsaved
// state across an async boundary.
const _storeCache = new Map(); // userId -> { mtimeMs, store }
function loadStore(userId) {
  try {
    const p = storePath(userId);
    let st;
    try { st = statSync(p); } catch { _storeCache.delete(userId); return emptyStore(); }
    const hit = _storeCache.get(userId);
    if (hit && hit.mtimeMs === st.mtimeMs) return hit.store;
    const s = migrateLegacy(JSON.parse(readFileSync(p, 'utf8')));
    s.keys = s.keys || {};
    s.accounts = s.accounts || {};
    s.schema = Math.max(Number(s.schema || 3), 4);
    _storeCache.set(userId, { mtimeMs: st.mtimeMs, store: s });
    return s;
...
// Multi-part public suffixes we're likely to meet in email. A full PSL
// dependency is overkill for label bleed — unknown ccTLD shapes just fall
// back to the last two labels (the old behavior). Without this,
// amazon.co.uk and barclays.co.uk both keyed to "co.uk", so counts and even
// authoritative pins for one sender applied to every .co.uk sender.
const MULTI_PART_TLDS = new Set([
  ...
  'com.mx', 'com.ar', 'com.sg', 'com.hk', 'com.tw', 'com.cn', 'com.my', 'com.ph',
  'co.kr', 'co.il', 'co.th', 'co.id', 'com.tr', 'com.co', 'com.pe', 'com.ve',
]);
...
/**
 * Read-only PIN lookup for external consumers that want ONLY the explicit,
 * user-authoritative correction — never the fuzzy "observed" guess that
 * suggestLabels() also folds in. Used by the Gmail auto-label poller
 * (gmail-autolabel.mjs) to decide whether a static rule conflicts with a
 * correction the user already made, or whether the sender is pinned to stay
 * in the inbox. Address pin wins over domain pin; subject-conditional pins
 * win over unconditional ones (see applicablePin). Pure read: never creates
 * or seeds an account bucket, never writes.
 * @param {string} userId
 * @param {string} fromHeader
 * @param {string} [subject]
 * @param {{accountId?: string}} [opts]
 * @returns {{labels:string[], keepInbox:boolean, source:'address'|'domain'}|null}
 */
export function getPin(userId, fromHeader, subject, opts = {}) {
  const store = loadStore(userId);
  const bucket = accountBucket(store, opts.accountId, { create: false });
  const keys = bucket.keys || {};
  const subjToks = tokenizeSubject(subject);
  const addr = senderAddress(fromHeader);
  const dom = senderDomain(fromHeader);
  for (const [key, kind] of [[addr, 'address'], [(dom && !FREE_PROVIDERS.has(dom)) ? dom : null, 'domain']]) {
    ...
    if (!rec) continue;
    const pin = applicablePin(rec, subjToks);
    if (pin) return { labels: pin.labels.slice(), keepInbox: !!pin.keepInbox, source: /** @type {'address'|'domain'} */ (kind) };
  }
  return null;
}
