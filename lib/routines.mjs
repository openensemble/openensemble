 ...
 *   - Lowercased, wake-word prefix stripped, trailing punctuation/please
 ...
 * @property {ActionPlayAmbient[]} [ambient]  deferred play_ambient actions; the caller starts them after the spoken reply (TTS) finishes
 */
...
export function saveRoutines(userId, routines) {
  ...
  // Device ownership: routine.device_id targets a voice device for ambient
  // playback / tts_say and takes precedence at execution (resolveRoutineDeviceId).
  // cleanRoutine only validates its FORMAT, so a crafted routine could bind
  // ANOTHER user's device and play audio on it. Blank any device_id that isn't
  // one of this user's paired devices. Looked up lazily (only when a routine
  // actually sets a device_id) so the common no-device case pays nothing.
  let ownedDeviceIds = null;
  for (const clean of cleanList) {
    if (!clean.device_id) continue;
    if (ownedDeviceIds === null) {
      try { ownedDeviceIds = new Set(listDevices(userId).map(d => d.id)); }
      catch { ownedDeviceIds = new Set(); }
    }
    if (!ownedDeviceIds.has(clean.device_id)) {
      console.warn(`[routines] blanking non-owned device_id ${clean.device_id} on routine ${clean.id} (user=${userId})`);
      clean.device_id = null;
    }
  }
  const prev = loadRoutines(userId);
  // Webhook tokens: PRESERVE only when the incoming token matches this same
  // routine's prior token; regenerate anything else. Accepting arbitrary
  // client-chosen tokens let one user save another user's known token —
  // findRoutineByWebhookToken scans all users and first match wins, so a
  // crafted routine could shadow (or be fired in place of) the victim's.
  const prevTokById = new Map(prev.routines.map(r => [r.id, r.webhook_token]));
  for (const clean of cleanList) {
    if (clean.webhook_token && clean.webhook_token !== prevTokById.get(clean.id)) {
      clean.webhook_token = randomBytes(16).toString('hex');
    }
  }
  const next = {
...
const MAX_AMBIENT_BYTES = 40 * 1024 * 1024;
...
export async function executeRoutine(routine, ctx) {
  ...
  for (const action of routine.actions) {
    // play_ambient is DEFERRED, not started inline. The caller starts it (via
    // runDeferredAmbient) only AFTER the routine's spoken reply has finished
    // playing on the device — otherwise the ambient sound starts during the
    // loop and drowns the announcement (the TTS reply is only spoken after
    // executeRoutine returns). Collect them in order here.
    if (action.type === 'play_ambient') { ambient.push(action); continue; }
    try {
  ...
  return { text, errors, followupPrompt, ambient };
}

/**
 * Start a routine's deferred play_ambient action(s). Called by the routine
 * caller AFTER the spoken reply (TTS) has finished, so the announcement is
 * heard before the ambient sound begins. Errors are logged, never thrown.
 * @param {ActionPlayAmbient[]} ambientActions  the `ambient` array from executeRoutine
 * @param {{userId?:string, deviceId?:string}} [ctx]
 */
export async function runDeferredAmbient(ambientActions, { userId, deviceId } = {}) {
  if (!Array.isArray(ambientActions) || !ambientActions.length || !deviceId) return;
  const { playAmbientOnDevice } = await import('./ambient-playback.mjs');
  for (const action of ambientActions) {
    try {
      await playAmbientOnDevice({
        userId, deviceId, file: action.file,
        loop: action.loop !== false, volume: action.volume,
      });
    } catch (e) {
      console.warn(`[routines] deferred play_ambient failed: ${e.message}`);
    }
  }
}

/**
 * Deliver a routine outside an active chat stream (REST webhook, UI Test,
 * Home Assistant event bridge). It pushes any spoken reply to the resolved
 * device, then starts deferred ambient audio after the speech finishes.
 *
 * The caller must validate deviceId ownership and resolve any precedence
 * between an explicit target and routine.device_id before passing it.
 *
 * @param {Routine} routine
 * @param {{userId:string, deviceId?:string|null, fallbackToFirstDevice?:boolean}} options
 * @returns {Promise<ExecuteRoutineResult & {deviceId:string|null, ambientScheduled:boolean}>}
 */
export async function executeRoutineOutOfBand(routine, {
  userId,
  deviceId = null,
  fallbackToFirstDevice = false,
}) {
  if (!deviceId && fallbackToFirstDevice) deviceId = listDevices(userId)[0]?.id ?? null;
  ...
  if (result.text && deviceId) {
    try {
      const { speakRoutineTts } = await import('./voice-reminder.mjs');
      ({ durationMs: ttsMs } = await speakRoutineTts({
        userId, deviceIds: [deviceId], text: result.text,
      }));
    } catch (e) {
      console.warn(`[routines] out-of-band tts push failed: ${e.message}`);
      errors.push({ type: 'tts_delivery', message: e.message });
    }
  } else if (!deviceId && routine.actions?.some(action => action.type === 'tts_say')) {
    errors.push({ type: 'tts_delivery', message: 'No paired target device' });
  }
  ...
  if (ambient.length && deviceId) {
    const timer = setTimeout(() => {
      runDeferredAmbient(ambient, { userId, deviceId })
        .catch(e => console.warn(`[routines] out-of-band ambient start failed: ${e.message}`));
    }, ttsMs > 0 ? ttsMs + 300 : 0);
    timer.unref?.();
    ambientScheduled = true;
  } else if (ambient.length) {
    errors.push({ type: 'play_ambient', message: 'No paired target device' });
  }
  return { ...result, errors, deviceId, ambientScheduled };
}
...
async function runHaScene(action) {
  ...
  // Fire-and-forget against HA: only wait briefly for transport-level errors
  // (ECONNREFUSED, DNS, 4xx auth) — those surface within milliseconds. If
  // HA accepts the TCP connection, assume the service call is queued and
  // move on so the routine's other actions don't sit through the 15-second
  // client-side timeout. HA's actual service execution can take 30+ seconds
  // for bulk operations (light.all) and we don't need to wait on that.
  const res = await _fireAndForgetHaCall(haRequest, cfg,
    `/services/${serviceDomain}/${service}`, 'POST',
    { entity_id: action.scene_id },
    `ha_scene ${serviceDomain}.${service} ${action.scene_id}`);
...
async function runHaCall(action) {
  ...
  if (!cfg) throw new Error('Home Assistant is not configured');
  const res = await _fireAndForgetHaCall(haRequest, cfg,
    `/services/${action.domain}/${action.service}`, 'POST', action.data,
    `ha_call ${action.domain}.${action.service}`);
  if (res?.__err) throw new Error(res.__err);
  return null;
}

/**
 * Race the haRequest promise against a short transport-error window. After
 * EARLY_TIMEOUT_MS we assume the request reached HA and return success.
 * The late response is still observed via .then so an eventual error is
 * logged (but doesn't fail the routine).
 *
 * "Real" transport failures (ECONNREFUSED, DNS resolution failure, TLS
 * handshake error) surface within tens of milliseconds — well below our
 * 1.5s window. HA's service-execution latency (light.all walking every
 * bulb, etc.) takes seconds and gets the queued treatment.
 */
const _EARLY_TIMEOUT_MS = 1500;
async function _fireAndForgetHaCall(haRequest, cfg, path, method, body, label) {
  const QUEUED = Symbol('queued');
  const reqP = haRequest(cfg, path, method, body);
  const winner = await Promise.race([
    reqP,
    new Promise(resolve => setTimeout(() => resolve(QUEUED), _EARLY_TIMEOUT_MS)),
  ]);
  if (winner === QUEUED) {
    // Log the eventual result without blocking the routine.
    reqP.then(res => {
      if (res?.__err && !/timeout/i.test(res.__err)) {
        console.warn(`[routines] ${label}: late HA error after fire-and-forget: ${res.__err}`);
      }
    }).catch(() => {});
    console.log(`[routines] ${label}: dispatched (fire-and-forget; not waiting for HA to ack)`);
    return null;   // optimistic success
  }
  // Request finished within the window — typically a fast ack or an immediate
  // transport error. Timeouts are still "queued" (HA may finish async).
  if (winner?.__err && /timeout/i.test(winner.__err)) {
    console.log(`[routines] ${label}: HA timed out — treating as queued`);
    return null;
  }
  return winner;
}
