/**
 * Shared GGUF download + cleanup helper.
 *
 * Called from two entry points:
 *   1. scripts/fetch-models.mjs (npm postinstall) — downloads bundled models
 *      so the server boots fully offline.
 *   2. memory/builtin-reason.mjs / scheduler/builtin-plan.mjs — at server
 *      startup, if the configured MODEL_FILE constant points at a GGUF that
 *      isn't on disk yet (typical after `git pull` lands a version bump),
 *      download it before throwing.
 *
 * Also handles cleanup of stale GGUFs from previous versions so `models/`
 * doesn't accumulate ~140MB per retrain (per the [TEST 2026-04-27]
 * version-rollout decision: monotonic integer versions, only the current
 * MODEL_FILE survives in models/).
 */
import fs from 'fs';
...
export function cleanupOldGgufs(modelsDir, family, keepFile) {
  ...
  const re = new RegExp(`^openensemble-${family}-v[\\w.\\-]+\\.q\\d+_\\w+\\.gguf$`);
