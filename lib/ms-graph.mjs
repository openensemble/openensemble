...
function loadMsAttachments(docIds, userId) {
  ...
  return docIds.map(id => {
    ...
    if (!doc) return null;
    if (doc.uploadedBy !== userId && !doc.sharedWith.includes('*') && !doc.sharedWith.includes(userId)) return null;
...
export async function composeMsMessage(userId, accountId, { to, subject, body, html_body, attachment_doc_ids }) {
  ...
  const attachments = loadMsAttachments(attachment_doc_ids, userId);
