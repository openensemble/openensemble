/**
 * Microsoft Graph API helpers for inbox access.
 * Token files: ms-token-{userId}-{accountId}.json
 */

import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { loadEmailAttachments, attachmentResolutionError } from './email-attachments.mjs';
import { readEncryptedJsonFile, writeEncryptedJsonFile } from './encrypted-file.mjs';

const BASE_DIR   = path.dirname(path.dirname(fileURLToPath(import.meta.url)));
const CREDS_PATH = path.join(BASE_DIR, 'microsoft-credentials.json');
const GRAPH_BASE = 'https://graph.microsoft.com/v1.0';

function tokenPath(userId, accountId) {
  // accountId is part of the filename — neutralize path traversal. Legit ids are
  // `acct_<hex>` so this is a no-op on them.
  const safe = String(accountId ?? '').replace(/[^a-zA-Z0-9_-]/g, '_');
  return path.join(BASE_DIR, 'users', userId, `ms-token-${safe}.json`);
}

export async function getMsAuthHeader(userId, accountId) {
  const tp = tokenPath(userId, accountId);
  if (!fs.existsSync(tp)) throw new Error('Microsoft account not connected');
  // readEncryptedJsonFile passes through legacy plaintext tokens unchanged.
  let tokens = readEncryptedJsonFile(tp);

  if (!tokens.expiry_date || Date.now() > tokens.expiry_date - 60000) {
    // Auth metadata persisted at original-grant time tells us how to refresh.
    // Built-in (PKCE public client) tokens have no client_secret to send;
    // user-configured tokens do. Fall back to legacy CREDS_PATH read for old
    // tokens that pre-date the metadata persistence (no tokens.builtin field).
    const tenant   = tokens.tenant   ?? 'common';
    const clientId = tokens.client_id;
    const isBuiltin = tokens.builtin === true;
    const body = new URLSearchParams({
      refresh_token: tokens.refresh_token,
      grant_type:    'refresh_token',
      scope:         'https://graph.microsoft.com/Mail.ReadWrite https://graph.microsoft.com/Mail.Send offline_access',
    });
    if (isBuiltin) {
      body.set('client_id', clientId);
    } else {
      // Legacy or user-configured: read creds file for client_id + secret.
      const creds = JSON.parse(fs.readFileSync(CREDS_PATH, 'utf8'));
      body.set('client_id', clientId ?? creds.client_id);
      body.set('client_secret', creds.client_secret);
    }
    const r = await fetch(`https://login.microsoftonline.com/${tenant}/oauth2/v2.0/token`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body,
    });
    const d = await r.json();
    if (!d.access_token) throw new Error(`MS token refresh failed: ${d.error_description ?? d.error}`);
    tokens.access_token = d.access_token;
    tokens.expiry_date = Date.now() + d.expires_in * 1000;
    if (d.refresh_token) tokens.refresh_token = d.refresh_token;
    writeEncryptedJsonFile(tp, tokens, { mode: 0o600 });
  }

  return { Authorization: `Bearer ${tokens.access_token}` };
}

export async function fetchMsInboxPage(userId, accountId, skipToken, max, query) {
  const headers = await getMsAuthHeader(userId, accountId);
  // Graph rejects $search combined with $orderby — drop orderby when searching.
  // $search wraps the term in quotes so callers can pass plain text.
  const base = `${GRAPH_BASE}/me/mailFolders/inbox/messages?$top=${max}&$select=id,conversationId,from,subject,receivedDateTime,bodyPreview`;
  let url = query
    ? `${base}&$search=${encodeURIComponent(`"${String(query).replace(/"/g, '\\"')}"`)}`
    : `${base}&$orderby=receivedDateTime desc`;
  if (skipToken) url += `&$skiptoken=${encodeURIComponent(skipToken)}`;

  // $search needs ConsistencyLevel: eventual on the request.
  const reqHeaders = query ? { ...headers, ConsistencyLevel: 'eventual' } : headers;
  const res = await fetch(url, { headers: reqHeaders });
  const data = await res.json();
  if (data.error) throw new Error(data.error.message);

  const emails = (data.value ?? []).map(m => ({
    id: m.id,
    threadId: m.conversationId,
    from: m.from?.emailAddress
      ? `${m.from.emailAddress.name ?? ''} <${m.from.emailAddress.address ?? ''}>`.trim()
      : '',
    subject: m.subject ?? '(no subject)',
    date: m.receivedDateTime ?? '',
    snippet: (m.bodyPreview ?? '').replace(/\s+/g, ' ').trim(),
  }));

  // Extract $skiptoken from @odata.nextLink
  let nextPageToken = null;
  if (data['@odata.nextLink']) {
    try {
      const nextUrl = new URL(data['@odata.nextLink']);
      nextPageToken = nextUrl.searchParams.get('$skiptoken');
    } catch (_) {}
  }

  return { emails, nextPageToken };
}

export async function fetchMsMessageBody(userId, accountId, msgId) {
  const headers = await getMsAuthHeader(userId, accountId);
  const res = await fetch(`${GRAPH_BASE}/me/messages/${msgId}?$select=body`, { headers });
  const data = await res.json();
  if (data.error) throw new Error(data.error.message);
  const body = data.body;
  if (!body) return '<p>No body content.</p>';
  if (body.contentType === 'html') return body.content;
  return `<pre style="font-family:sans-serif;white-space:pre-wrap">${(body.content ?? '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</pre>`;
}

export async function fetchMsThread(userId, accountId, conversationId) {
  const headers = await getMsAuthHeader(userId, accountId);
  // Graph rejects $filter on conversationId combined with $orderby ("restriction
  // or sort order is too complex"). Drop $orderby; sort client-side.
  const url = `${GRAPH_BASE}/me/messages?$filter=conversationId eq '${conversationId}'&$select=id,from,subject,receivedDateTime,bodyPreview,body`;
  const res = await fetch(url, { headers });
  const data = await res.json();
  if (data.error) throw new Error(data.error.message);
  return (data.value ?? []).sort((a, b) =>
    (a.receivedDateTime ?? '').localeCompare(b.receivedDateTime ?? '')
  ).map(m => ({
    id: m.id,
    from: m.from?.emailAddress ? `${m.from.emailAddress.name ?? ''} <${m.from.emailAddress.address}>`.trim() : '',
    subject: m.subject ?? '',
    date: m.receivedDateTime ?? '',
    body: m.body?.contentType === 'html'
      ? m.body.content.replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim().slice(0,500)
      : (m.body?.content ?? '').slice(0,500),
  }));
}

export async function replyMsMessage(userId, accountId, msgId, body, markDispatchStarted = () => {}) {
  const headers = await getMsAuthHeader(userId, accountId);
  markDispatchStarted();
  const res = await fetch(`${GRAPH_BASE}/me/messages/${msgId}/reply`, {
    method: 'POST',
    headers: { ...headers, 'Content-Type': 'application/json' },
    body: JSON.stringify({ comment: body }),
  });
  if (!res.ok) { const e = await res.json(); throw new Error(e.error?.message ?? 'Reply failed'); }
  return 'Reply sent.';
}

export async function composeMsMessage(userId, accountId, { to, subject, body, html_body, attachment_doc_ids }, markDispatchStarted = () => {}) {
  const headers = await getMsAuthHeader(userId, accountId);

  const { attachments, errors } = loadEmailAttachments(attachment_doc_ids, userId);
  const resolveErr = attachmentResolutionError(attachment_doc_ids, errors);
  if (resolveErr) return resolveErr;

  const message = {
    subject,
    body: { contentType: html_body ? 'HTML' : 'Text', content: html_body ?? body },
    toRecipients: to.split(',').map(a => ({ emailAddress: { address: a.trim() } })),
  };
  if (attachments.length) {
    message.attachments = attachments.map(att => ({
      '@odata.type': '#microsoft.graph.fileAttachment',
      name: att.filename,
      contentType: att.mimeType,
      contentBytes: att.data.toString('base64'),
    }));
  }

  markDispatchStarted();
  const res = await fetch(`${GRAPH_BASE}/me/sendMail`, {
    method: 'POST',
    headers: { ...headers, 'Content-Type': 'application/json' },
    body: JSON.stringify({ message }),
  });
  if (!res.ok) { const e = await res.json(); throw new Error(e.error?.message ?? 'Send failed'); }
  const attachNote = attachments.length
    ? ` with ${attachments.length} attachment(s): ${attachments.map(a => a.filename).join(', ')}`
    : '';
  return `Email sent${attachNote}.`;
}

// Submit Graph requests through the /$batch endpoint (20-op cap per call).
// `requests` is an array of `{ method, url, headers?, body? }`; `id` is added
// per-chunk. Returns the count of responses with a 2xx status.
async function msBatch(headers, requests) {
  let done = 0;
  const BATCH = 20;
  for (let i = 0; i < requests.length; i += BATCH) {
    const chunk = requests.slice(i, i + BATCH).map((r, idx) => ({ ...r, id: String(idx) }));
    const res = await fetch(`${GRAPH_BASE}/$batch`, {
      method: 'POST',
      headers: { ...headers, 'Content-Type': 'application/json' },
      body: JSON.stringify({ requests: chunk }),
    });
    const data = await res.json();
    if (!res.ok && !data.responses) throw new Error(data.error?.message ?? 'Graph batch failed.');
    for (const r of (data.responses ?? [])) {
      if (r.status >= 200 && r.status < 300) done++;
    }
  }
  return done;
}

export async function purgeMsSender(userId, accountId, { sender, query, permanent }) {
  // Mirrors gmail.mjs cmdPurgeSender: paginate-all-IDs then batch the move/delete.
  // Restricted to inbox so a "purge from uber" doesn't sweep Sent or Drafts.
  const searchTerm = sender ? `from:${sender}` : query;
  if (!searchTerm) throw new Error('Provide either sender or query.');

  const headers = await getMsAuthHeader(userId, accountId);
  const ids = [];
  let skipToken = null;
  while (true) {
    const base = `${GRAPH_BASE}/me/mailFolders/inbox/messages?$top=100&$select=id`;
    const escaped = `"${searchTerm.replace(/"/g, '\\"')}"`;
    let url = `${base}&$search=${encodeURIComponent(escaped)}`;
    if (skipToken) url += `&$skiptoken=${encodeURIComponent(skipToken)}`;
    const res = await fetch(url, { headers: { ...headers, ConsistencyLevel: 'eventual' } });
    const data = await res.json();
    if (data.error) throw new Error(data.error.message);
    for (const m of (data.value ?? [])) ids.push(m.id);
    if (!data['@odata.nextLink']) break;
    try {
      skipToken = new URL(data['@odata.nextLink']).searchParams.get('$skiptoken');
    } catch (_) { break; }
    if (!skipToken) break;
  }

  if (!ids.length) return 'No emails found matching query.';

  const requests = ids.map(id => permanent
    ? { method: 'POST', url: `/me/messages/${id}/permanentDelete` }
    : {
        method: 'POST',
        url: `/me/messages/${id}/move`,
        headers: { 'Content-Type': 'application/json' },
        body: { destinationId: 'deleteditems' },
      });
  const done = await msBatch(headers, requests);

  const action = permanent ? 'permanently deleted' : 'moved to trash';
  return `Done. ${done} email(s) from "${searchTerm}" ${action}.`;
}

export async function trashMsMessage(userId, accountId, msgId) {
  const headers = await getMsAuthHeader(userId, accountId);
  const res = await fetch(`${GRAPH_BASE}/me/messages/${msgId}/move`, {
    method: 'POST',
    headers: { ...headers, 'Content-Type': 'application/json' },
    body: JSON.stringify({ destinationId: 'deleteditems' }),
  });
  if (!res.ok) { const e = await res.json(); throw new Error(e.error?.message ?? 'Trash failed'); }
  return 'Message moved to trash.';
}

export async function trashMsBatch(userId, accountId, msgIds) {
  if (!msgIds.length) return 0;
  const headers = await getMsAuthHeader(userId, accountId);
  const requests = msgIds.map(id => ({
    method: 'POST',
    url: `/me/messages/${id}/move`,
    headers: { 'Content-Type': 'application/json' },
    body: { destinationId: 'deleteditems' },
  }));
  return msBatch(headers, requests);
}

export async function markMsRead(userId, accountId, msgIds, unread = false) {
  if (!msgIds.length) return 'No message IDs provided.';
  const headers = await getMsAuthHeader(userId, accountId);
  const requests = msgIds.map(id => ({
    method: 'PATCH',
    url: `/me/messages/${id}`,
    headers: { 'Content-Type': 'application/json' },
    body: { isRead: !unread },
  }));
  const done = await msBatch(headers, requests);
  return `Marked ${done} message${done !== 1 ? 's' : ''} as ${unread ? 'unread' : 'read'}.`;
}

export async function fetchMsInboxStats(userId, accountId) {
  const headers = await getMsAuthHeader(userId, accountId);
  const [unread, total] = await Promise.all([
    fetch(`${GRAPH_BASE}/me/mailFolders/inbox?$select=unreadItemCount`, { headers }).then(r => r.json()),
    fetch(`${GRAPH_BASE}/me/mailFolders/inbox?$select=totalItemCount`, { headers }).then(r => r.json()),
  ]);
  return `Inbox: ${unread.unreadItemCount ?? '?'} unread, ${total.totalItemCount ?? '?'} total.`;
}

export function msTokenPath(userId, accountId) {
  return tokenPath(userId, accountId);
}
