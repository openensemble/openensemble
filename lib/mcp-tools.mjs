...
const MCP_SKILL_ID = 'mcp';

function cacheKey(userId, agentId) {
  return `${userId}::${agentId}`;
}

/** Fail-closed runtime authorization for every MCP connection entrypoint. */
async function isMcpRuntimeAllowed(userId) {
  if (!userId) return false;
  try {
    const { isSkillRuntimeEnabledForUser } = await import('../roles.mjs');
    return isSkillRuntimeEnabledForUser(MCP_SKILL_ID, userId);
  } catch (e) {
    console.warn(`[mcp-tools] authorization lookup failed for user=${userId}:`, e.message);
    return false;
  }
}

function clearUserToolCaches(userId) {
  const prefix = `${userId}::`;
...
/**
 * A denied account must lose both its synchronous schemas and any live MCP
 * processes/sockets. Best-effort disconnect keeps a malformed server entry
 * from preventing the rest of the user's state from being revoked.
 */
async function clearDeniedMcpState(userId, servers = null) {
  clearUserToolCaches(userId);
  let configured = servers;
  if (!configured) {
    try { configured = getServersForUser(userId); } catch { configured = []; }
  }
  await Promise.all((configured ?? []).map(async cfg => {
    if (!cfg?.id) return;
    try { await disconnect(userId, cfg.id); } catch {}
  }));
}

async function ensureMcpRuntimeAllowed(userId, servers = null) {
  if (await isMcpRuntimeAllowed(userId)) return true;
  await clearDeniedMcpState(userId, servers);
  return false;
}
 ...
 */
async function refreshAuthorizedAgentMcpTools(userId, agentId) {
...
export async function refreshAgentMcpTools(userId, agentId) {
  ...
  if (!await ensureMcpRuntimeAllowed(userId)) return false;
  await refreshAuthorizedAgentMcpTools(userId, agentId);
  return true;
}
...
export async function refreshUserMcpTools(userId) {
  ...
  if (!await ensureMcpRuntimeAllowed(userId, servers)) return false;
  ...
  // Authorization was checked immediately above. Avoid repeating it per agent,
  // while keeping the exported single-agent refresher independently gated.
  await Promise.all([...agentIds].map(a => refreshAuthorizedAgentMcpTools(userId, a)));
  return true;
}
...
/**
 * Sync union used by single-agent orchestration. Parked specialists keep their
 * MCP assignments on disk, so the visible primary needs the cached schemas for
 * every still-owned agent while the roster is projected down to one. The
 * caller supplies the current owned-agent ids; this deliberately avoids stale
 * cache entries for deleted agents. Duplicate server assignments are deduped
 * by namespaced tool name in stable agent/tool order.
 */
export function getCachedMcpToolDefsForAgents(userId, agentIds) {
  if (!userId || !Array.isArray(agentIds) || agentIds.length === 0) return [];
  const seen = new Set();
  const tools = [];
  for (const agentId of agentIds) {
    for (const tool of getCachedMcpToolDefsForAgent(userId, agentId)) {
...
export async function reconnectServer(userId, serverId) {
  ...
  for (const a of (cfg.assignedToAgents ?? [])) {
    try { await refreshAuthorizedAgentMcpTools(userId, a); } catch {}
  }
  return true;
}
...
export async function warmAllUsersAtBoot() {
  ...
  await Promise.all(userDirs.map(async (uid) => {
    try {
      ...
      if (await refreshUserMcpTools(uid)) warmed++;
    } catch (e) {
