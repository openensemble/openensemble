 ...
 * are synchronous HTTP proxies, not background jobs. Callers proxying media
 * should pass `maxBytes` so an unhealthy upstream cannot grow memory without
 * bound while the response is buffered.
 *
 * @returns {Promise<{statusCode:number, contentType:string, body:Buffer}|{__err:string}>}
 */
export function haRequestBinary(haCfg, path, {
  timeoutMs = 10_000,
  maxBytes = Infinity,
  accept = null,
} = {}, _hop = 0) {
  return new Promise((resolve) => {
    ...
    const req = lib.request(opts, (res) => {
      if ([301, 302, 307, 308].includes(res.statusCode) && res.headers.location && _hop < 1) {
        ...
        const nextBase = `${redirected.protocol}//${redirected.host}`;
        const nextPath = redirected.pathname.replace(/^\/api/, '') + redirected.search;
        return resolve(haRequestBinary(
          { ...haCfg, url: nextBase },
          nextPath,
          { timeoutMs, maxBytes, accept },
          _hop + 1,
        ));
      }
      ...
      const maximum = Number.isFinite(maxBytes) && maxBytes >= 0 ? maxBytes : Infinity;
      const declared = Number(res.headers['content-length']);
      if (Number.isFinite(declared) && declared > maximum) {
        res.resume();
        return resolve({ __err: 'Response body exceeded the configured size limit' });
      }
      const chunks = [];
      let received = 0;
      let oversized = false;
      res.on('data', (c) => {
        ...
        if (received > maximum) {
          oversized = true;
          chunks.length = 0;
          resolve({ __err: 'Response body exceeded the configured size limit' });
          res.destroy();
          return;
        }
        chunks.push(c);
