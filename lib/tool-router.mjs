...
// Skills whose tools always ship on a coordinator turn. These are either
// universally useful (web, memory, delegate, telegram) or core to coordinator
// duties (coordinator, self-mgmt). Note these are SKILL IDs, not tool names —
// every tool the listed skills own becomes always-on.
const ALWAYS_INCLUDE_SKILL_IDS = new Set([
  ...
  'tasks',           // set_reminder, schedule_task, list_tasks — common
  'routines',        // routine create/list/delete — common
...
export async function trimToolsForTurn({ agent, userText, userId }) {
  ...
  // Step 2: assemble the keep-set: always-on + matched.
