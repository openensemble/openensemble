...
async function classifyOnDemandSkills(userText, userId, threshold) {
  ...
  try {
    ...
    if (top && dynamicOnDemand.has(top.skillId)) {
      const blocker = EMBED_INTENT_BLOCK_RULES.get(top.skillId);
      if (hits.has(top.skillId) || !blocker?.test(userText)) hits.add(top.skillId);
    }
  } catch (e) {
...
export const _internal = { ALWAYS_INCLUDE_SKILL_IDS, ON_DEMAND_SKILL_IDS, INITIAL_INCLUDE_THRESHOLD, DIRECT_INTENT_RULES, EMBED_INTENT_BLOCK_RULES };
