...
async function classifyOnDemandSkills(userText, userId, threshold, { fullTools = null, rosterSolo = false } = {}) {
  ...
  const routingText = routingInstructionClauses(userText).join(' ') || instructionText(userText);
  ...
  const weatherClauses = routingInstructionClauses(userText)
...
export async function scoreToolsForTurn({ tools, userText, userId, agent, source, cfg, protectedSkillIds = new Set() }) {
  ...
  for (const t of tools) {
    ...
    if (isDetachedWorkerAgent(agent)) {
      if (name === 'request_tools' || name === 'report_progress') reason = 'worker-control';
      ...
      if (name === 'request_tools'
          || name === 'spawn_worker'
          || name === 'check_workers'
          || name === 'stop_worker') reason = 'solo-control';
...
function isDetachedWorkerAgent(agent) {
  return agent?.ephemeral === true
    && String(agent?.id ?? '').startsWith('ephemeral_worker_');
}
...
export async function trimToolsForTurn({ agent, userText, userId, source = null }) {
  ...
  if (agent.skillCategory === 'coordinator') {
    ...
    const matchedMcpTools = selectMcpToolsByText(
      fullTools,
      routingInstructionClauses(userText).join(' ') || instructionText(userText),
    );
