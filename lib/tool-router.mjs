...
export async function trimToolsForTurn({ agent, userText, userId }) {
  ...
  const forceHeldRoles = agent.skillCategory === 'coordinator'
    ? new Set([...heldRoles].filter(id => id === agent.skillCategory))
    : heldRoles;
  ...
  for (const t of fullTools) {
    ...
    if (isAlwaysOnSkill(ownerId, userId) || forceHeldRoles.has(ownerId)) {
  ...
  if (forceHeldRoles.size !== heldRoles.size) notes.push(`coordinator held-role bypass suppressed: ${[...heldRoles].filter(id => !forceHeldRoles.has(id)).join(',')}`);
