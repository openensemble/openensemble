...
// Always-ship regardless of score: the recovery + delegation primitives.
// request_tools lets the LLM pull back anything the trim dropped mid-turn.
// ask_agent starts available, then the specialist terminal-action policy can
// remove it when the agent already has the right primary action tool.
const ALWAYS_TOOL_NAMES = new Set(['request_tools', 'ask_agent']);
...
export async function scoreToolsForTurn({ tools, userText, userId, cfg }) {
  ...
  let qVec = null;
  try { qVec = await embed(userText); } catch { /* embed down */ }
...
function directiveText(userText) {
...
function adjustSpecialistTerminalTools({ agent, userText, userId, kept, fullTools, decisions }) {
  ...
  if (!primaryActionTools.length) return { kept, notes: [] };
  ...
  if (terminalPrimaryAction && keptNames.delete('ask_agent')) {
