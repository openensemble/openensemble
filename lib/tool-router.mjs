...
// weather fallback. Prefer a user's narrower localweather skill when it
// matched, but retain web for installations without that skill and for
// conditions localweather does not claim to cover (pollen/air-quality/UV).
...
async function classifyOnDemandSkills(userText, userId, threshold, { fullTools = null, rosterSolo = false } = {}) {
  ...
  if (rosterSolo) {
    for (const skillId of customRoutable) {
      ...
      if (manifest?.coordinator_scope === 'exclude' && exactCustomSkillMention(routingText, manifest)) {
        hits.add(skillId);
  ...
  if (!exactCustomNamed) try {
    ...
    const unblocked = ranked.filter(hit => {
      ...
      return hits.has(hit.skillId) || !blocker?.test(routingText);
    });
  ...
  if (dynamicOnDemand.has('web')
      ...
      && (!hits.has('localweather') || EXTENDED_WEATHER_RE.test(routingText))) {
    hits.add('web');
  }
