...
async function classifyOnDemandSkills(userText, userId, threshold, { fullTools = null, rosterSolo = false } = {}) {
  ...
  if (dynamicOnDemand.has('web')
      && WEATHER_FALLBACK_RE.test(userText)
      && (!hits.has('localweather') || EXTENDED_WEATHER_RE.test(userText))) {
    hits.add('web');
  }
  ...
  if (OUTBOUND_EMAIL_DELIVERY_RE.test(userText)) hits.add('email-send');
  return hits;
}
...
export async function trimToolsForTurn({ agent, userText, userId, source = null }) {
  ...
  if (agent.skillCategory === 'coordinator') {
    ...
    const matchedMcpTools = selectMcpToolsByText(fullTools, userText);
