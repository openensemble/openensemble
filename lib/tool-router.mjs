...
import { supportsImageGeneration } from './model-capabilities.mjs';
import {
  classifyClearlySplittableWork,
  explicitlyRequestsAgentExecution,
  isParallelWorkGateLocked,
} from './parallel-work-gate.mjs';
import { normalizeDocumentRequest } from './document-artifacts.mjs';
import { isToolSuppressedForTurn } from './tool-router-context.mjs';
import { hasUsableConfiguredImageBackend } from '../skills/image_generator/execute.mjs';
...
const ON_DEMAND_SKILL_IDS = new Set([
  ...
  'documents',       // read/edit/version docs — only on typed doc intent or a normalized drawer request
]);

// These remain legacy always-on skills for multi-agent coordinators, but a
// singleton Jarvis routes them like every other domain so its stable floor is
// only recovery + worker control. Keep this separate from ON_DEMAND_SKILL_IDS:
// it is a one-agent optimization, not a new public request_tools group list.
const SOLO_ON_DEMAND_SKILL_IDS = new Set([
  'web', 'telegram', 'profile_files', 'logs', 'self-mgmt', 'user-admin',
  'coordinator',
]);

// A singleton coordinator cannot rely on the old always-visible web tool as a
// weather fallback. Prefer a user's matching manifest-declared custom skill
// when one is present, but retain web for installations without one and for
// extended conditions the matched skill may not claim (pollen/air-quality/UV).
// Keep this out of DIRECT_INTENT_RULES so an ordinary forecast does not load
// both domains merely because web is available.
const WEATHER_FALLBACK_RE = /\b(?:weather|forecast|umbrella|rain|snow|temperature|pollen|air\s+quality|uv\s+index)\b|\bhow\s+(?:hot|cold|warm|cool)\b[^.\n]{0,50}\b(?:outside|today|tomorrow)\b|\b(?:windy|humid)\b[^.\n]{0,30}\b(?:outside|today|tomorrow)\b/i;
const EXTENDED_WEATHER_RE = /\b(?:pollen|air\s+quality|uv\s+index|allerg(?:y|ies|en)|wildfire\s+smoke)\b/i;

// Full email is a mailbox control surface, not the ordinary outbound delivery
// path. Keep this action-shaped: an address or the word "email" in "email this
// report" must not load inbox search, labels, trash, account enumeration, and
// separate compose/reply tools. email_user owns those send-only requests.
const MAILBOX_EMAIL_INTENT_RE = /\b(?:gmail|inbox|mailbox)\b|\b(?:check|read|open|show|list|summari[sz]e|recap|search|find|triage|sort|organi[sz]e|clean\s+up|clear\s+out|delete|trash|purge|archive|label|unlabel|mark)\s+(?:(?:for|all|my|the|these|those|new|latest|newest|recent|live|exact|full|personal|work|an?|\w+'s)\s+){0,4}(?:e-?mails?|gmail|inbox|mailbox|mail|messages?|threads?|labels?)\b|\b(?:e-?mails?|gmail|inbox|mailbox|mail|messages?|threads?|labels?)\b[^.\n]{0,100}\b(?:unread|newest|latest|recent|search|triage|sort|organi[sz]e|delete|trash|purge|archive|label|mark\s+(?:as\s+)?(?:read|unread))\b|\b(?:repl(?:y|ies|ied)\s+(?:to|back)|send\s+(?:a\s+)?reply|written?\s+back|write\s+back|correspond\s+with|forward\s+(?:(?:the|this|that|an?)\s+)?(?:e-?mail|message)|forward\s+(?:this|that|it)\s+to)\b|\b(?:who\s+(?:e-?mailed|messaged)|what(?:'s|\s+is)\s+in\s+(?:my|the)\s+inbox|did\s+i\s+get\b[^.\n]{0,80}\b(?:e-?mail|message|mail)|has\b[^.\n]{0,50}\bwritten\s+back)\b|\b(?:list|show|check|manage|add|remove|connect|disconnect|configure|switch)\b[^.\n]{0,80}\b(?:e-?mail|gmail|imap|smtp|microsoft)\s+accounts?\b/i;
const OFFICIAL_SOURCE_LOOKUP_RE = /\b(?:find|get|check|verify|read|look\s+up)\b[^\n]{0,100}\b(?:official|primary)\b[^\n]{0,80}\b(?:website|site|web\s*page|documentation|docs?)\b/i;
// An official-source lookup can mention a runtime-like product (Node.js) and
// ask us to "write one sentence" without asking for code work. Preserve the
// coder only when an implementation verb actually governs a code artifact.
const EXPLICIT_CODE_WORK_RE = /\b(?:write|create|build|edit|modify|fix|debug|refactor|review|test|run|implement|patch)\b[^.\n]{0,100}\b(?:code|source\s+file|script|function|class|module|project|repository|repo|tests?|bug|python|javascript|typescript|java|kotlin|swift|rust|golang|[\w./-]+\.(?:[cm]?js|tsx?|py|rb|php|java|kt|swift|rs|go|c|cc|cpp|h|hpp|cs|sh|bash|zsh|sql|html|css|vue|svelte))\b/i;
const EXPLICIT_DEEP_RESEARCH_RE = /\b(?:research|investigate|deep\s+dive|literature\s+review|market\s+analysis)\b/i;
// A Home Assistant integration can still require shell work on the host that
// runs it. Keep this deliberately conjunctive: ordinary HA entity/config errors
// must not expose the remote-node surface, while an explicit CLI-shaped command
// that the user tried to execute and could not find is actionable node work.
const HOME_ASSISTANT_CLI_FAILURE_RE = /^(?=[^\n]{0,240}\bhome\s+assistant\b)(?=[^\n]{0,240}\b(?:using|run(?:ning)?|execut(?:e|ing)|invok(?:e|ing)|launch(?:ing)?|call(?:ing)?)\s+[`'"]?[a-z0-9][\w.-]*(?:-config|ctl|cli)\b)(?=[^\n]{0,240}\b(?:not\s+found|not\s+recognized|unknown\s+command|missing\s+(?:command|binary|executable)|no\s+such\s+(?:file|command))\b)/i;

const DIRECT_INTENT_RULES = [
  {
    // Typed document edit/create/version phrasing. Drawer turns are selected
    // separately from normalized documentRequest metadata; never treat
    // a user-pasted `[Document: ...]` marker as trusted routing context.
    skillId: 'documents',
    re: /\b(?:edit|rewrite|revise|reword|redraft|proofread|update|correct|tighten|shorten|expand|append\s+to|change|replace|make|improve|clean\s+up)\s+(?:this\s+|that\s+|the\s+|my\s+)?(?:document|doc|file|report|draft|resume|essay|readme|markdown|note)\b|\b(?:create|make|start|draft|write)\s+(?:me\s+)?(?:(?:a|an|the|another)\s+)?(?:new\s+)?(?:document|doc|markdown|text\s+file|note)\b|\b(?:revert|restore)\b[^.]{0,30}\bversion\b|\bversion\s+history\b|\bundo\s+the\s+(?:last\s+)?(?:change|edit)\b/i,
  },
  {
    skillId: 'desktop',
  ...
  {
    skillId: 'web',
    re: /\b(?:search|browse|look\s+up|find)\s+(?:the\s+)?(?:web|internet|online)\b|\b(?:search|look\s+up|find)\b[^.\n]{0,80}\b(?:online|on\s+the\s+web|on\s+the\s+internet)\b|\b(?:latest|current)\s+(?:news|headlines|breaking\s+news|web\s+results)\b|\b(?:fetch|read|open|summari[sz]e)\s+https?:\/\//i,
  },
  {
    skillId: 'web',
    re: OFFICIAL_SOURCE_LOOKUP_RE,
  },
  {
    skillId: 'telegram',
    re: /\btelegram\b/i,
  ...
  {
    skillId: 'profile_files',
    re: /\b(?:profile|saved|generated)\s+files?\b|\b(?:list|show|find|read|open|attach|send|email)\b[^.\n]{0,80}\b(?:saved|generated|profile)\s+(?:file|image|photo|video|document|report)\b/i,
  },
  {
    skillId: 'logs',
    re: /\b(?:server|oe|openensemble|system|application)\s+logs?\b|\b(?:read|show|scan|check|inspect|search)\b[^.\n]{0,60}\blogs?\b|\b(?:trace|inspect|show)\b[^.\n]{0,60}\b(?:last|recent|previous)\s+turn\b|\bwhy\b[^.\n]{0,80}\b(?:tool|turn|delegation)\b[^.\n]{0,40}\b(?:fail|failed|duplicate|twice|slow)\b/i,
  },
  {
    skillId: 'self-mgmt',
    re: /\b(?:remember|recall|forget)\b|\bwhat\s+do\s+you\s+remember\b|\b(?:add|remove|delete|list|show|change)\b[^.\n]{0,60}\b(?:standing\s+instructions?|skill\s+rules?|fastpath\s+phrases?)\b|\bclaim\b[^.\n]{0,40}\brole\b/i,
  },
  {
    skillId: 'user-admin',
    re: /\b(?:create|add|invite|list|show|manage|edit|delete|remove|disable)\b[^.\n]{0,60}\b(?:users?|child\s+accounts?|household\s+members?|guardians?)\b/i,
  },
  {
    skillId: 'coordinator',
    re: /\b(?:create|add|list|show|delete|remove|assign)\b[^.\n]{0,50}\b(?:agents?|roles?)\b|\bdescribe\b[^.\n]{0,40}\b(?:oe|openensemble|platform)\b/i,
  },
  {
    skillId: 'nodes',
    re: HOME_ASSISTANT_CLI_FAILURE_RE,
  },
  {
  ...
  {
    skillId: 'email',
    // Mailbox reads, replies, search, triage, labels, and account management.
    // Pure outbound delivery stays on the smaller email_user surface.
    re: MAILBOX_EMAIL_INTENT_RE,
  },
  {
    skillId: 'gcal',
  ...
  },
  {
    ...
    // Permit a short proposed/custom name before "skill" ("build a new Local
    // Weather skill"). Without this shape, the exact custom-name matcher can
    // mistake the subject being authored for a request to run the old skill.
    re: /\b(?:build|create|make|draft|add|update|edit|patch|fix|delete|remove|inspect|show|rollback|restore)\s+(?:(?:me|a|an|the|my|this|that|new|existing|custom)\s+){0,4}(?:[a-z0-9_-]+\s+){0,4}skills?\b|\bskills?\s+(?:builder|blueprint|manifest|code|tool|tools)\b/i,
  },
  {
    skillId: 'coder',
  ...
  {
    skillId: 'oe-admin',
    // A bare "provider" is common in ordinary tool instructions (for
    // example, "match the subject as closely as the provider allows"). Only
    // load OE administration for an actual provider-management shape.
    re: /\b(?:oe\s+server|restart\s+(?:the\s+)?server|tunnel|cloudflared|tailscale|update\s+(?:oe|openensemble))\b|\bopenensemble\s+(?:update|version|server|settings|config|configuration|integration|provider|tunnel)\b|\b(?:install|configure|remove|list)\s+(?:(?:an?|the)\s+)?integrations?\b|\b(?:add|configure|change|switch|remove|list|set\s+up)\s+(?:(?:the|my|an?)\s+)?(?:(?:ai|llm|model)\s+)?providers?\b|\bproviders?\s+(?:key|keys|setting|settings|config|configuration|account|accounts|model|models)\b/i,
  },
];

function instructionSourceForDocumentTurn(userText, normalizedDocumentRequest) {
  const text = String(userText ?? '');
  if (!normalizedDocumentRequest) return text;
  // The drawer appends this envelope after the user's directive. Once the
  // separate structured metadata identifies a document turn, everything from
  // that canonical boundary onward is context, not another instruction.
  let boundary = -1;
...
const EMBED_INTENT_BLOCK_RULES = new Map([
  ...
  ['role_tv_control', /\b(?:api|sdk|source\s+code|programming|developer\s+docs?|oauth)\b|\bpause\s+for\b|\b(?:scene|movie|show)\b[^.\n]{0,60}\b(?:was|were)\b/i],
  ['youtube-download', /\b(?:explain|documentation|docs?|developer|data\s+api)\b|\b(?:create|generate|make|render|animate|produce)\b[^.\n]{0,100}\b(?:video|clip|animation|timelapse)\b/i],
  ['gcal', /\b(?:weather|forecast|umbrella|rain|snow|temperature|how\s+(?:hot|cold|warm|cool)|degrees?\s+(?:outside|today|tomorrow))\b|\bmy\s+schedule\s+(?:is|looks?|seems?|feels?)\b/i],
  ...
  ['coder', OFFICIAL_SOURCE_LOOKUP_RE],
  ['deep_research', /\b(?:official|primary)\b[^\n]{0,100}\b(?:website|site|web\s*page|documentation|docs?|url)\b|\b(?:one|a)\s+concise\s+(?:sentence|answer|line)\b/i],
]);
...
const CUSTOM_AMBIGUITY_STOPWORDS = new Set([
  'about', 'active', 'add', 'also', 'another', 'anything', 'available', 'best',
  'both', 'check', 'could', 'current', 'delete', 'does', 'entry', 'fetch',
  'find', 'from', 'get', 'going', 'have', 'history', 'index', 'into',
  'inventory', 'journal', 'latest', 'like', 'list', 'log', 'monitor', 'more',
  'most', 'new', 'newest', 'next', 'note', 'only', 'other', 'over', 'ping', 'plan',
  'planner', 'progress', 'project', 'read', 'recent', 'record', 'remove',
  'same', 'schedule', 'search', 'session', 'show', 'some', 'status', 'such',
  'summary', 'than', 'that', 'their', 'them', 'then', 'there', 'these',
  'thing', 'things', 'those', 'tool', 'tools', 'track', 'tracker', 'under',
  'until', 'update', 'very', 'watch', 'what', 'when', 'where', 'whether', 'which',
  'while', 'will', 'with', 'without', 'would',
]);

// Deterministic anchors for user-authored, intent-routed skills. Compound
// instructions can dilute a short clause below the embedding threshold, so
// derive conservative domain words from repeated manifest examples. Generic
// workflow words and isolated identity qualifiers never qualify on their own.
const CUSTOM_INTENT_ANCHOR_STOPWORDS = new Set([
  ...CUSTOM_AMBIGUITY_STOPWORDS,
  'after', 'again', 'also', 'analyze', 'attach', 'before', 'book', 'buy',
  'call', 'cancel', 'classify', 'compose', 'could', 'create', 'critique',
  'download', 'draft', 'edit', 'email', 'evening', 'every', 'extract',
  'family', 'forward', 'generate', 'help', 'inside', 'local', 'make',
  'morning', 'need', 'order', 'outside', 'personal', 'play', 'please',
  'post', 'proofread', 'reply', 'report', 'research', 'rewrite', 'right',
  'save', 'send', 'set', 'should', 'something', 'summarize', 'text', 'this',
  'through', 'today', 'tomorrow', 'tonight', 'translate', 'upload', 'using',
  'want', 'would', 'write', 'your',
]);

function customIntentTokens(text) {
  return new Set(String(text ?? '')
    .toLowerCase()
    .match(/[a-z0-9]+/g)
    ?.filter(token => token.length >= 4 && !CUSTOM_INTENT_ANCHOR_STOPWORDS.has(token)) ?? []);
}

function customIntentPhrases(manifest) {
  const phrases = Array.isArray(manifest?.intent_examples)
    ? manifest.intent_examples.filter(value => typeof value === 'string' && value.trim())
    : [];
  for (const intent of (Array.isArray(manifest?.localIntents) ? manifest.localIntents : [])) {
    for (const utterance of (Array.isArray(intent?.utterances) ? intent.utterances : [])) {
      if (typeof utterance === 'string' && utterance.trim()) phrases.push(utterance);
    }
  }
  return phrases;
}

const _customIntentAnchorCache = new WeakMap();
function customIntentAnchorTokens(manifest) {
  if (!manifest || typeof manifest !== 'object') return new Set();
  const cached = _customIntentAnchorCache.get(manifest);
  if (cached) return cached;
  const anchors = new Set();
  const counts = new Map();
  for (const phrase of customIntentPhrases(manifest)) {
    // Count a word once per example; repetition inside one verbose example is
    // not evidence that the word defines the skill's domain.
    for (const token of customIntentTokens(phrase)) {
      counts.set(token, (counts.get(token) ?? 0) + 1);
    }
  }
  for (const [token, count] of counts) if (count >= 2) anchors.add(token);
  _customIntentAnchorCache.set(manifest, anchors);
  return anchors;
}

function customIntentLexicalScore(text, manifest) {
  const query = customIntentTokens(text);
  if (!query.size) return 0;
  let score = 0;
  for (const token of customIntentAnchorTokens(manifest)) {
    if (query.has(token)) score++;
  }
  return score;
}

function customIntentLexicalMatches(text, skillIds, userId) {
  return [...skillIds]
    .map(skillId => {
      const manifest = getRoleManifest(skillId, userId);
      return {
        skillId,
        score: exactCustomSkillMention(text, manifest)
          ? Number.MAX_SAFE_INTEGER
          : customIntentLexicalScore(text, manifest),
      };
    })
...
// Runtime MCP schemas do not live in role manifests, so the normal
// tool-name -> skill-owner index cannot classify them. Treat each connected
// MCP server as a dynamic pseudo-skill (`mcp:<server-id>`) and select only the
// few tools whose server/name vocabulary is present in the turn. This keeps a
// user's growing MCP catalog off the stable floor without making the tools
// unreachable: request_tools can recover a narrow reason match or the whole
// server via an explicit `mcp:<server-id>` group.
const MCP_GROUP_PREFIX = 'mcp:';
const MAX_INITIAL_MCP_TOOLS = 8;
const MCP_ROUTING_STOPWORDS = new Set([
  ...CUSTOM_AMBIGUITY_STOPWORDS,
  'call', 'create', 'execute', 'message', 'send', 'server', 'service',
  'using', 'write',
]);

function mcpToolMeta(tool) {
  const name = toolName(tool);
  if (!name.startsWith('mcp_')) return null;
  const sep = name.indexOf('__', 4);
  if (sep <= 4 || sep >= name.length - 2) return null;
  const serverId = name.slice(4, sep);
  const bareName = name.slice(sep + 2);
  return {
    serverId,
    bareName,
    groupId: `${MCP_GROUP_PREFIX}${serverId}`,
  };
}

function mcpLexicalTokens(text) {
  return new Set(String(text ?? '')
    .toLowerCase()
    .match(/[a-z0-9]+/g)
    ?.filter(token => token.length >= 3 && !MCP_ROUTING_STOPWORDS.has(token)) ?? []);
}
...
function selectMcpToolsByText(tools, text, { limit = MAX_INITIAL_MCP_TOOLS } = {}) {
  const query = mcpLexicalTokens(text);
  if (!query.size || limit <= 0) return [];
  const ranked = [];
  for (let index = 0; index < (tools ?? []).length; index++) {
    const tool = tools[index];
    const meta = mcpToolMeta(tool);
    if (!meta) continue;
    const serverTokens = mcpLexicalTokens(meta.serverId);
    const nameTokens = mcpLexicalTokens(meta.bareName);
    const descriptionTokens = mcpLexicalTokens(tool?.function?.description ?? '');
    for (const token of serverTokens) descriptionTokens.delete(token);
    const serverHits = intersectionCount(query, serverTokens);
    const nameHits = intersectionCount(query, nameTokens);
    // Generic description overlap is too noisy (`send`, `file`, `account`)
    // and would reintroduce unrelated schemas. The stopword-filtered tool name
    // or operation description must overlap; an explicit server name boosts
    // the right connected server but is not sufficient by itself.
    const descriptionHits = intersectionCount(query, descriptionTokens);
    if (nameHits === 0 && descriptionHits === 0) continue;
    ranked.push({
      tool,
      index,
      score: (serverHits * 12) + (nameHits * 5) + descriptionHits,
    });
  }
  return ranked
...
function customEmbeddingHasLexicalAnchor(text, manifest) {
  return exactCustomSkillMention(text, manifest)
    || customAmbiguityHasLexicalAnchor(text, manifest);
}

function customSkillSuppressedByTurn(text, manifest, selectedSkills) {
  // In "build a skill for X", X describes the skill being authored; it is not
  // an instruction to run an existing custom skill that shares X's nouns or
  // exact name. Check this before the exact-name invocation escape hatch.
  if (selectedSkills?.has('skill-builder')) return true;
  if (exactCustomSkillMention(text, manifest)) return false;
  return EMBED_INTENT_BLOCK_RULES.get(manifest?.id)?.test(text) === true;
}
...
async function classifyOnDemandSkills(userText, userId, threshold, {
  ...
  const normalizedDocumentRequest = normalizeDocumentRequest(documentRequest);
  if (!userText || userText.length < 6) {
    return normalizedDocumentRequest ? new Set(['documents']) : new Set();
  }
  const hits = new Set();
  ...
  const dynamicOnDemand = new Set([
    ...ON_DEMAND_SKILL_IDS,
    ...(rosterSolo ? SOLO_ON_DEMAND_SKILL_IDS : []),
    ...customRoutable,
  ]);
  // Preserve every explicit workflow clause without scanning pasted payload
  // prose as if it were a second instruction.
  const instructionSource = instructionSourceForDocumentTurn(userText, normalizedDocumentRequest);
  const routingText = routingInstructionClauses(instructionSource).join(' ') || instructionText(instructionSource);
  for (const rule of DIRECT_INTENT_RULES) {
    if (dynamicOnDemand.has(rule.skillId) && rule.re.test(routingText)) hits.add(rule.skillId);
  }
  // Drawer routing comes from separately normalized structured metadata, not
  // from a header that arbitrary prompt or pasted document text can imitate.
  if (dynamicOnDemand.has('documents') && normalizedDocumentRequest) {
    hits.add('documents');
  }
  // A fact lookup against one named official source is ordinary web work even
  // when its product name resembles a coding runtime (for example Node.js).
  // Preserve code/deep-research domains only when explicitly requested.
  if (OFFICIAL_SOURCE_LOOKUP_RE.test(routingText)
      && !EXPLICIT_CODE_WORK_RE.test(routingText)) {
    hits.delete('coder');
    if (!EXPLICIT_DEEP_RESEARCH_RE.test(routingText)) hits.delete('deep_research');
  }
  // Preserve manifest-declared custom domains that are explicitly named in a
  // compound workflow. This remains installation-generic and examines only
  // instruction clauses, never pasted body/result prose.
  for (const skillId of customIntentLexicalMatches(routingText, customRoutable, userId)) {
    const manifest = getRoleManifest(skillId, userId);
    if (customSkillSuppressedByTurn(routingText, manifest, hits)) continue;
    hits.add(skillId);
  }
  ...
  if (rosterSolo) {
    for (const skillId of customRoutable) {
      ...
      if (manifest?.coordinator_scope === 'exclude'
          && exactCustomSkillMention(routingText, manifest)
          && !customSkillSuppressedByTurn(routingText, manifest, hits)) {
        hits.add(skillId);
  ...
  if (!exactCustomNamed) try {
    const ranked = await rankByEmbedding(routingText, userId, /* coordAgentId */ null, {
    ...
    // from Calendar ("this afternoon") to a custom forecast skill ("umbrella").
    const unblocked = ranked.filter(hit => {
      const blocker = EMBED_INTENT_BLOCK_RULES.get(hit.skillId);
      if (!hits.has(hit.skillId) && blocker?.test(routingText)) return false;
      if (customRoutable.has(hit.skillId)
          && (customSkillSuppressedByTurn(routingText, getRoleManifest(hit.skillId, userId), hits)
            || !customEmbeddingHasLexicalAnchor(routingText, getRoleManifest(hit.skillId, userId)))) return false;
      return true;
    });
        ...
        : customRoutable.has(top.skillId) && (hits.size === 0 || hits.has(top.skillId))
          ? unblocked
            .filter(hit => dynamicOnDemand.has(hit.skillId)
              ...
              && hit.sim >= threshold
              && customAmbiguityHasLexicalAnchor(routingText, getRoleManifest(hit.skillId, userId))
  ...
  const weatherClauses = routingInstructionClauses(instructionSource)
    .filter(clause => WEATHER_FALLBACK_RE.test(clause));
  const customSkillHandlesWeatherClause = weatherClauses.some(clause =>
    [...hits].some(skillId => customRoutable.has(skillId)
      && customIntentLexicalScore(clause, getRoleManifest(skillId, userId)) > 0));
  if (dynamicOnDemand.has('web')
      && weatherClauses.length > 0
      && (!customSkillHandlesWeatherClause || EXTENDED_WEATHER_RE.test(routingText))) {
    hits.add('web');
  }
  // Embeddings for "email this" naturally resemble the mailbox skill. Remove
  // that broad semantic hit unless the instruction asks for a mailbox
  // read/reply/manage operation; email_user remains separately reachable.
  if (!MAILBOX_EMAIL_INTENT_RE.test(routingText)) hits.delete('email');
  // `email-send` is always-on rather than an on-demand domain, so it is not in
  // dynamicOnDemand. Record a positive outbound request explicitly; the solo
  // tool-level gate uses this matched owner to retain email_user for the turn.
  if (OUTBOUND_EMAIL_DELIVERY_RE.test(routingText)) hits.add('email-send');
  return hits;
}
 ...
 * @property {Set<string>} [matchedSkills] Skills positively matched from this turn's instruction. Unlike skillsKept, this excludes merely always-on/shipped skills.
...
// tiny universals, genuinely unknown owners, and whatever the coordinator
// skill-gate admitted. Runtime MCP tools are handled as bounded dynamic
// pseudo-skills before this pass; they no longer inflate the stable floor.
...
// that lacked it.
const ALWAYS_TOOL_NAMES = new Set(['request_tools', 'ask_agent', 'email_user', 'web_search', 'recall_facts']);

// `email_user` is an always-on delivery primitive, but singleton routing keeps
// even always-on schemas off the neutral floor. Preserve it when the user
// actually asks for outbound delivery without conflating that request with
// mailbox-management intent (which is owned by the full `email` skill).
const OUTBOUND_EMAIL_DELIVERY_RE = /\b(?:use|call|invoke|run)\s+(?:the\s+)?email_user\b[^.\n]{0,80}\b(?:send|deliver|e-?mail)\b|\b(?:e-?mail|mail)\s+(?!(?:inbox|account|accounts|message|messages|thread|threads|settings|address|addresses|list|history|drafts?|spam|folder|folders|server|tools?|from|about|with|containing|is|was)\b)\S+|\b(?:send|deliver)\b[^.\n]{0,120}\b(?:an?\s+e-?mail|by\s+e-?mail|via\s+e-?mail|[\w.+-]+@[\w.-]+\.[a-z]{2,})\b/i;
...
export function shouldPreserveCachePrefix(agent) {
  ...
  // A singleton coordinator exists specifically to own a large, growing
  // capability surface and trim it per turn. Shipping that full surface for
  // cache stability defeats the architecture and grows without bound as the
  // household adds custom skills. Keep the experiment available only to
  // already-scoped multi-agent/specialist configurations.
  if (agent?._rosterSolo === true || agent?.ephemeral === true) return false;
...
export async function scoreToolsForTurn({ tools, userText, userId, agent, source, cfg, protectedSkillIds = new Set() }) {
  ...
  const directive = routingInstructionClauses(userText || '').join(' ')
  ...
  const explicitParallelWork = isDetachedWorkerAgent(agent)
    && !isCoordinatedChildAgent(agent)
    && classifyClearlySplittableWork(directive, {
  ...
  const explicitAgentExecution = explicitlyRequestsAgentExecution(directive);
  ...
  for (const t of tools) {
    ...
    let keep = true;
    let reason = 'default';
    // Detached workers are task-scoped copies of Jarvis, not general-purpose
    // second coordinators. Give them only recovery/progress controls plus the
    // domains selected for their self-contained task. Any missed domain stays
    // recoverable from fullTools through request_tools. This keeps a mailbox
    // worker from receiving admin, user-management, Telegram, web, memory, and
    // worker-spawning schemas merely because those are legacy coordinator
    // defaults.
    if (isDetachedWorkerAgent(agent)) {
      const coordinatedChild = isCoordinatedChildAgent(agent);
      if (name === 'request_tools'
          || name === 'report_progress'
          || (coordinatedChild && name === 'claim_work')
          || (!coordinatedChild && name === 'redirect_worker')
          || (!coordinatedChild && name === 'parallel_work' && explicitParallelWork)) {
        reason = 'worker-control';
      }
      else if (!coordinatedChild && name === 'parallel_work') {
        keep = false;
        reason = 'parallel-not-requested';
      }
      else if (!owner) reason = mcpToolMeta(t) ? 'mcp-intent' : 'unowned';
      else if (owner === primarySkill) reason = 'worker-primary-skill';
      else if (protectedSkillIds.has(owner)) reason = 'worker-skill-intent';
      else {
        keep = false;
        reason = 'worker-scope-drop';
      }
    }
    else if (agent?._rosterSolo === true) {
      if ((name === 'spawn_worker' || name === 'list_execution_targets') && !explicitAgentExecution) {
        keep = false;
        reason = 'worker-not-requested';
      }
      else if (name === 'request_tools'
          || name === 'spawn_worker'
          || name === 'list_execution_targets'
          || name === 'check_workers'
          || name === 'stop_worker'
          || name === 'redirect_worker') reason = 'solo-control';
      else if (name === 'parallel_work' || name === 'claim_work') {
        keep = false;
        reason = 'coordinator-worker-only';
      }
      // `ask_agent` earns its ALWAYS_TOOL_NAMES slot only on a roster that has
      // someone to ask. A solo coordinator has no delegation target, so
      // shipping it invites a hand-off into the void — the same reasoning that
      // already drops parallel_work/claim_work just above. The remaining
      // universal primitives stay useful to an agent working alone.
      else if (name === 'ask_agent') {
        keep = false;
        reason = 'solo-no-delegate-target';
      }
      // Per the rule documented on OUTBOUND_EMAIL_DELIVERY_RE, singleton
      // routing keeps even always-on schemas off the neutral floor, and
      // email_user is preserved only when outbound delivery is actually
      // asked for. Keeping it unconditionally here contradicted that.
      else if (name === 'email_user' && !OUTBOUND_EMAIL_DELIVERY_RE.test(directive)) {
        keep = false;
        reason = 'solo-no-outbound-request';
      }
      // Universal primitives ship on a solo coordinator for the same reasons
      // documented on ALWAYS_TOOL_NAMES (web_search also gates the model's
      // native hosted search; email_user is the always-on delivery tool). This
      // branch ran before the shared ALWAYS_TOOL_NAMES rule below and silently
      // revoked them from the one agent that must do everything itself.
      else if (ALWAYS_TOOL_NAMES.has(name)) reason = 'control';
      else if (!owner) reason = mcpToolMeta(t) ? 'mcp-intent' : 'unowned';
      else if (protectedSkillIds.has(owner)) reason = 'solo-skill-intent';
      // `include` is the historical promise that a custom skill stays on the
      // coordinator. A singleton primary is still the coordinator; applying
      // the compact solo floor must not silently revoke that opt-in. Legacy
      // custom manifests without an explicit scope retain the same behavior.
      else if (isCoordinatorIncludedCustomSkill(owner, userId)) reason = 'solo-custom-include';
      else {
        keep = false;
        reason = 'solo-scope-drop';
      }
    }
    else if (name === 'spawn_worker' && !explicitAgentExecution) {
      keep = false;
      reason = 'worker-not-requested';
    }
    else if (name === 'parallel_work' || name === 'claim_work') {
...
export function shouldUseProviderHostedImageBackend(agent, userId) {
  // This flag only selects the implementation behind an already-authorized
  // generate_image capability. The Responses adapter separately requires that
  // exact function to be present in the current turn's tool set, so this can
  // never grant image generation to a restricted account or a non-image turn.
  return agent?.provider === 'openai-oauth'
    && supportsImageGeneration('openai-oauth', agent?.model)
    && !hasUsableConfiguredImageBackend(userId);
}

function isDetachedWorkerAgent(agent) {
  return agent?.ephemeral === true
    && /^ephemeral_(?:worker|workstream)_/.test(String(agent?.id ?? ''));
}

function isCoordinatedChildAgent(agent) {
  return agent?.ephemeral === true
    && String(agent?.id ?? '').startsWith('ephemeral_workstream_');
}

function isCoordinatorIncludedCustomSkill(skillId, userId) {
  const manifest = getRoleManifest(skillId, userId);
  return manifest?.custom === true
    && manifest.coordinator_scope !== 'auto'
 ...
 * @returns {Promise<TrimResult>}
 */
export async function trimToolsForTurn({
  agent,
  userText,
  userId,
  source = null,
  ...
  if (!fullTools.length) {
    return { trimmedTools: fullTools, fullTools, initiallyIncludedSkills: new Set(), matchedSkills: new Set(), routerNotes: ['skipped: no tools'] };
  }
  ...
  if (shouldPreserveCachePrefix(agent)) {
    return { trimmedTools: fullTools, fullTools, initiallyIncludedSkills: new Set(), skillsKept: new Set(), matchedSkills: new Set(), routerNotes: [`cache-stable: trim skipped (${fullTools.length} tools)`] };
  }
  const notes = [];
  ...
  const owners = toolOwnerIndex(userId);
  const normalizedDocumentRequest = normalizeDocumentRequest(documentRequest);
  const routingUserText = instructionSourceForDocumentTurn(userText, normalizedDocumentRequest);

  if (agent.skillCategory === 'coordinator') {
    const matched = await classifyOnDemandSkills(routingUserText, userId, INITIAL_INCLUDE_THRESHOLD, {
      fullTools,
      rosterSolo: agent._rosterSolo === true,
      documentRequest: normalizedDocumentRequest,
    });
    matchedSkills = matched;
    // `email-send` carries manifest always_on so any agent can reach the user
    // (73bb836), but the newer outbound gate (5547f0c) is the narrower intent:
    // "preserve it when the user actually asks for outbound delivery without
    // conflating that request with mailbox-management intent". Left unguarded,
    // always_on short-circuits that gate and ships the send tool on every turn
    // — including "list my email accounts" — which is exactly the conflation
    // the gate exists to prevent. request_tools still reaches it on demand.
    const emailSendAlwaysOnApplies = OUTBOUND_EMAIL_DELIVERY_RE.test(String(routingUserText || ''));
    // A singleton routes SOLO_ON_DEMAND_SKILL_IDS "like every other domain",
    // but this keep loop asked isAlwaysOnSkill, which knows nothing about that
    // demotion — so `web` (and friends) were pinned on regardless, defeating
    // the custom-skill preference just above. Fall through to the `matched`
    // branch instead, which is what on-demand routing means.
    // `coordinator` is excluded: the set's own comment says the singleton's
    // stable floor IS recovery + worker control, and those (request_tools,
    // check_workers, stop_worker) are coordinator-owned. Demoting it here
    // deletes them before the tool-level solo gate can narrow within them.
    const soloDemoted = agent._rosterSolo === true
      ? new Set([...SOLO_ON_DEMAND_SKILL_IDS].filter(id => id !== 'coordinator'))
      : null;
    const alwaysOnHere = ownerId => isAlwaysOnSkill(ownerId, userId)
      && (ownerId !== 'email-send' || emailSendAlwaysOnApplies)
      && !soloDemoted?.has(ownerId);
    ...
    const matchedMcpTools = selectMcpToolsByText(
      fullTools,
      routingInstructionClauses(routingUserText).join(' ') || instructionText(routingUserText),
    );
    const matchedMcpNames = new Set(matchedMcpTools.map(toolName));
    for (const tool of matchedMcpTools) {
      const meta = mcpToolMeta(tool);
      if (meta) keepSkills.add(meta.groupId);
    }
    for (const t of fullTools) {
      const ownerId = owners[t.function?.name];
      if (!ownerId) continue;
      if (alwaysOnHere(ownerId)) keepSkills.add(ownerId);
    ...
    for (const t of fullTools) {
      const ownerId = owners[t.function?.name];
      // One small read-only memory tool is a universal capability, including
      // on a singleton coordinator. Do not ship the rest of self-management.
      if (toolName(t) === 'recall_facts') { stableSegment.push(t); continue; }
      if (!ownerId) {
        // Dynamic MCP schemas are a suffix selected for this turn; other
        // genuinely unowned runtime tools stay fail-open in the stable shell.
        if (mcpToolMeta(t)) {
          if (matchedMcpNames.has(toolName(t))) dynamicSegment.push(t);
        } else {
          stableSegment.push(t);
        }
        continue;
      }
      if (!keepSkills.has(ownerId)) continue;
      if (alwaysOnHere(ownerId) || forceHeldRoles.has(ownerId)) stableSegment.push(t);
  ...
  if (cfg.enabled) {
    const res = await scoreToolsForTurn({
      tools: candidate, userText: routingUserText, userId, agent, source, cfg,
      protectedSkillIds: matchedSkills,
    });
    if (res.applied) {
      const dropped = res.decisions.filter(d => !d.kept).map(d => d.name);
      const actualKeptSkills = new Set(res.kept
        .map(tool => owners[toolName(tool)] ?? mcpToolMeta(tool)?.groupId)
        .filter(Boolean));
      ...
      return { trimmedTools: res.kept, fullTools, initiallyIncludedSkills: new Set(), skillsKept: actualKeptSkills, matchedSkills, routerNotes: notes, toolDecisions: res.decisions };
    }
    notes.push('tool-level: not applied (disabled/empty)');
  }
  return { trimmedTools: candidate, fullTools, initiallyIncludedSkills: keepSkills, matchedSkills, routerNotes: notes };
}
 ...
 *
 * @returns {Promise<{addedToolNames: string[], addedSkills: string[], blockedBy?: string}>}
 */
export async function expandToolsByReason({ agent, fullTools, reason, groups, userId, alreadyIncludedSkills }) {
  // Defense in depth: request_tools is removed from the provider schema while
  // this lock is active, but automatic recovery and direct executor tests also
  // reach this function. No recovery path may bypass the required fan-out.
  if (isParallelWorkGateLocked(agent)) {
    return {
      addedToolNames: [],
      addedSkills: [],
      blockedBy: 'parallel-work-preflight',
    };
  }
  const owners = toolOwnerIndex(userId);
  ...
  for (const t of fullTools) {
    ...
    const meta = mcpToolMeta(t);
    if (meta) {
      recoverable.add(meta.groupId);
      if (!mcpGroups.has(meta.groupId)) mcpGroups.set(meta.groupId, []);
      mcpGroups.get(meta.groupId).push(t);
    }
  }
  ...
  const hasValidExplicitGroup = targetSkills.size > 0;
  const currentNames = new Set((agent.tools ?? []).map(t => t.function?.name));
  // Models sometimes name the exact missing tools rather than their hidden
  // skill id. Resolve only identifiers present in this turn's authorized
  // catalog; valid explicit skill groups still take precedence over a reason.
  const requestedToolNames = hasValidExplicitGroup ? new Set() : new Set([
    ...(String(reason || '').match(/[A-Za-z_][A-Za-z0-9_.:-]*/g) || []).map(token => token.replace(/[.:]+$/, '')),
    ...(Array.isArray(groups) ? groups.filter(g => typeof g === 'string').map(g => g.trim()) : []),
  ]);
  const exactTools = hasValidExplicitGroup ? [] : fullTools.filter(t => {
    const name = toolName(t);
    return requestedToolNames.has(name) && !currentNames.has(name) && !isToolSuppressedForTurn(name, agent);
  });
  const hasExactToolRequest = exactTools.length > 0;

  // Embed-match the free-form reason against recoverable skills.
  if (!hasExactToolRequest && targetSkills.size === 0 && typeof reason === 'string' && reason.trim().length >= 4) {
    ...
    if (OFFICIAL_SOURCE_LOOKUP_RE.test(reason) && !EXPLICIT_CODE_WORK_RE.test(reason)) {
      targetSkills.delete('coder');
      if (!EXPLICIT_DEEP_RESEARCH_RE.test(reason)) targetSkills.delete('deep_research');
    }
    if (agent?._rosterSolo === true) {
      for (const skillId of recoverable) {
        ...
        if (manifest?.custom === true
          ...
          && !customSkillSuppressedByTurn(reason, manifest, targetSkills)) {
    ...
    try {
      ...
      if (top && recoverable.has(top.skillId)) {
        const blocker = EMBED_INTENT_BLOCK_RULES.get(top.skillId);
        const manifest = getRoleManifest(top.skillId, userId);
        const customAllowed = manifest?.custom !== true
          || (!customSkillSuppressedByTurn(reason, manifest, targetSkills)
            && customEmbeddingHasLexicalAnchor(reason, manifest));
        if (!blocker?.test(reason) && customAllowed) targetSkills.add(top.skillId);
      }
    } catch (e) {
      log.warn('tool-router', 'expansion embed threw', { err: e.message });
    }
  }
  if (!hasValidExplicitGroup && !MAILBOX_EMAIL_INTENT_RE.test(String(reason || ''))) {
    targetSkills.delete('email');
  }
  if (!hasValidExplicitGroup
      && !hasExactToolRequest
      && OUTBOUND_EMAIL_DELIVERY_RE.test(String(reason || ''))
      && recoverable.has('email-send')) {
    targetSkills.add('email-send');
  }

  // A free-form request can recover only the MCP schemas relevant to its
  // stated operation. Explicit `groups: ['mcp:<server-id>']` remains the
  // complete-server escape hatch when a multi-step MCP workflow needs tools
  // whose names cannot be inferred before the first call.
  const reasonMatchedMcpTools = !hasValidExplicitGroup && !hasExactToolRequest && typeof reason === 'string'
    ? selectMcpToolsByText(fullTools, reason)
    : [];
  ...
  for (const skillId of targetSkills) {
    let touched = false;
    if (mcpGroups.has(skillId)) {
      // Initial MCP routing is intentionally partial. Even if the pseudo-skill
      // was marked present, an explicit server recovery must add its missing
      // remainder rather than treating one visible tool as the whole server.
      for (const t of mcpGroups.get(skillId)) {
        if (currentNames.has(t.function?.name)
            || isToolSuppressedForTurn(t.function?.name, agent)) continue;
        newTools.push(t);
  ...
  const reasonMcpSkills = new Set();
  for (const t of exactTools) {
    const name = toolName(t);
    if (!name || currentNames.has(name) || isToolSuppressedForTurn(name, agent)) continue;
    newTools.push(t);
    currentNames.add(name);
    const owner = owners[name] || mcpToolMeta(t)?.groupId;
    if (owner && !addedSkills.includes(owner)) addedSkills.push(owner);
  }
  for (const t of reasonMatchedMcpTools) {
    const name = t.function?.name;
    if (!name || currentNames.has(name) || isToolSuppressedForTurn(name, agent)) continue;
    newTools.push(t);
    currentNames.add(name);
    const meta = mcpToolMeta(t);
    if (meta) reasonMcpSkills.add(meta.groupId);
  }
  for (const skillId of reasonMcpSkills) addedSkills.push(skillId);
 ...
 *   userId?: string,
 *   documentRequest?: object|null,
 * }} [args]
 */
export function inferMissingToolSkills({
  userText = '',
  assistantText = '',
  userId,
  documentRequest = null,
  ...
  const normalizedDocumentRequest = normalizeDocumentRequest(documentRequest);
  const instructionSource = instructionSourceForDocumentTurn(userText, normalizedDocumentRequest);
  const text = `${instructionSource}\n${assistantText}`;
  const dynamicOnDemand = new Set([...ON_DEMAND_SKILL_IDS, ...getCustomAutoSkills(userId)]);
  const skills = new Set();
  if (normalizedDocumentRequest && dynamicOnDemand.has('documents')) skills.add('documents');
  for (const rule of DIRECT_INTENT_RULES) {
...
export const _internal = {
  ...
  customEmbeddingHasLexicalAnchor,
  customIntentAnchorTokens,
  customIntentLexicalScore,
  MAILBOX_EMAIL_INTENT_RE,
  OUTBOUND_EMAIL_DELIVERY_RE,
  mcpToolMeta,
  selectMcpToolsByText,
};
