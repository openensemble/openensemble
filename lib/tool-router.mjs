...
// The three trim buckets. Regex hits are the primary (deterministic) gate;
// the embedding is only a backup for phrasings the regex misses. A regex hit
...
const BUCKET_INTENT_RE = {
  tasks: /\b(?:remind|reminder|alarm|timer|task|to-?do|watch|monitor|notif|schedul|autonomy|daily|weekly|hourly|every\s+(?:day|week|hour|morning|night)|tomorrow|tonight|at\s+\d)/i,
...
// during testing doesn't need a restart. v2 has one knob: `toolBucketFloor`,
// the embedding-similarity floor a bucket tool must clear to be kept when its
// intent regex didn't fire. Everything outside the buckets ships regardless.
export function toolRouterCfg() {
  const c = loadConfig()?.toolRouter ?? {};
  const num = (v, d) => (Number.isFinite(v) ? v : d);
  return {
    enabled: c.toolLevel !== false,                // default ON for all agents
    bucketFloor: num(c.toolBucketFloor, 0.5),      // embed sim needed to keep a bucket tool without a regex hit
  };
}
...
// (original order) + per-tool decisions for logging/preview. Fail-open at
// every uncertainty: unknown owner → keep; embedder down → keep the bucket.
// A dropped bucket tool is always recoverable via request_tools.
export async function scoreToolsForTurn({ tools, userText, userId, agent, source, cfg }) {
  ...
  const tasksIntent = BUCKET_INTENT_RE.tasks.test(directive);
  const adminIntent = BUCKET_INTENT_RE.selfMgmtAdmin.test(directive);
  const desktopIntent = source === 'desktop-app' || BUCKET_INTENT_RE.desktop.test(directive);

  // The embedding backup only matters for bucket tools whose regex missed.
  const needsSim = (name, owner) => {
    if (owner === 'tasks') return !tasksIntent;
    if (owner === 'desktop') return !desktopIntent;
    if (owner === 'self-mgmt') return !adminIntent && !SELF_MGMT_MEMORY_TOOLS.has(name);
    return false;
  };
  let qVec = null;
  if (directive.trim() && tools.some(t => needsSim(toolName(t), owners[toolName(t)]))) {
    try { qVec = await embed(directive); } catch { qVec = null; }
    if (!Array.isArray(qVec) || !qVec.length) qVec = null;
  }
  ...
  for (const t of tools) {
    ...
    else if (owner === 'tasks' || owner === 'desktop' || owner === 'self-mgmt') {
      const intent = owner === 'tasks' ? tasksIntent : owner === 'desktop' ? desktopIntent : adminIntent;
      if (intent) reason = 'bucket-intent';
      else if (!qVec) reason = 'bucket-failopen';              // embedder down → keep
      else {
        const v = await getToolVec(t);
        sim = v ? Number(_toolDot(qVec, v).toFixed(3)) : null;
        keep = sim != null && sim >= cfg.bucketFloor;
        reason = keep ? 'bucket-sim' : 'bucket-drop';
        if (sim == null) { keep = true; reason = 'bucket-failopen'; }
      }
    }
    decisions.push({ name, kept: keep, reason, control: reason === 'control', ...(sim != null ? { sim } : {}) });
    if (keep) kept.push(t);
  }
