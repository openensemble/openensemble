...
const ALWAYS_INCLUDE_SKILL_IDS = new Set([
  ...
  'utility',         // grab-bag
  'browser-ext',     // browser_list/open_tab/read_page — small (3 tools) and
                     // intent-distinctive enough that the coordinator should
                     // call them directly rather than delegating to a
                     // specialist; the embed-router otherwise pulls "read
                     // the page" toward Home Assistant on cosine similarity.
]);
...
export async function trimToolsForTurn({ agent, userText, userId }) {
  ...
  // Split the kept tools into a stable prefix (always-on + held-role +
  // unknown-owner) and a dynamic suffix (router-matched on-demand). Reason:
  // OpenAI/Anthropic both cache the tools array as part of the request
  // prefix, so any reorder or insertion mid-array busts the cache for the
  // whole tools block. Keeping always-on tools at fixed leading positions
  // and appending on-demand tools at the end means the prefix stays
  // byte-identical across turns, and the cache miss is only the bytes for
  // the per-turn-added tools. We don't reorder WITHIN each segment — the
  // upstream order in fullTools is already deterministic (manifest-driven)
  // and keeping it stable avoids shuffling related tools apart.
  const matchedSet = matched;
  const stableSegment = [];
  const dynamicSegment = [];
  for (const t of fullTools) {
    const ownerId = owners[t.function?.name];
    // Unknown owner: keep, treat as stable (most are framework tools like
    // request_tools that ship on every turn anyway).
    if (!ownerId) { stableSegment.push(t); continue; }
    if (!keepSkills.has(ownerId)) continue;
    if (isAlwaysOnSkill(ownerId, userId) || heldRoles.has(ownerId)) {
      stableSegment.push(t);
    } else if (matchedSet.has(ownerId)) {
      dynamicSegment.push(t);
    } else {
      // Skill was in keepSkills but doesn't fit always-on or matched
      // (shouldn't normally happen — defensive fallback to stable).
      stableSegment.push(t);
    }
  }
  const trimmedTools = [...stableSegment, ...dynamicSegment];
  const notes = [
