...
async function deliverBrowserFieldWatchEvent(spec, helpers, message) {
  const requestedMode = spec?.deliver || helpers?.deliveryMode || null;
  // fire() is the authoritative dispatcher for non-WS delivery. Do not fall
  // back to notify after a failed email/Telegram/agent action: doing so would
  // clear pendingEvent and permanently hide the failed requested delivery.
  if (helpers?.fire && requestedMode && requestedMode !== 'notify') {
    return (await helpers.fire({
      message,
      itemKey: spec.id,
      eventKey: spec.pendingEvent?.id || null,
      ...(spec?.deliver ? { deliver: spec.deliver } : {}),
    })) === true;
  }
  if (helpers?.notify) {
    return (await helpers.notify(message, {
      from: spec.label,
...
export async function browserFieldWatchHandler(state, helpers, deps = {}) {
  ...
  const processItem = async original => {
    ...
    if (spec.pendingEvent && (helpers?.notify || helpers?.fire)) {
      ...
      const delivered = await deliverBrowserFieldWatchEvent(spec, helpers, message);
      if (delivered) {
        spec.lastNotified = {
