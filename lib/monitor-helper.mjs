...
export function buildCollectionHelpers({ userId, agentId, skillIdHint = null }) {
  async function _findOrEnsure(kind, /** @type {any} */ ensureOpts = null) {
    ...
    const explicitSkillId = ensureOpts?.skillId ?? skillIdHint ?? null;

    // 1. Exact (kind, skillId) match when we have a hint — preserves
    //    isolation when two skills share a kind on purpose.
    if (explicitSkillId !== null) {
      const exact = listAllCollections(userId, { kind, skillId: explicitSkillId })
                      .find(c => c.skillId === explicitSkillId);
      if (exact) return exact.watcherId;
    }

    // 2. Fall back to kind-alone match. The common case is "one skill owns
    //    each kind"; the skill calls ensure({skillId}) at setup but the
    //    follow-up add/update/remove don't carry skillId through. Looking
    //    by kind alone here makes those calls work without forcing every
    //    skill to thread skillId through every helper call. Disambiguation
    //    is the caller's job ONLY when multiple skills genuinely collide.
    const byKind = listAllCollections(userId, { kind });
    if (byKind.length === 1) return byKind[0].watcherId;
    if (byKind.length > 1 && explicitSkillId === null) {
      throw new Error(`ctx.collection: multiple collections for kind=${kind} (skillIds: ${byKind.map(m => m.skillId || '(null)').join(', ')}). Pass {skillId} to disambiguate.`);
    }
