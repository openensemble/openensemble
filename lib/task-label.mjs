...
Input: "in 2 hours email alex at alex@example.com about the deploy status"
TITLE: Email Alex deploy status
INSTRUCTION: email alex at alex@example.com about the deploy status`;
...
async function dispatch(agent, user, signal, userId) {
  ...
  if (p === 'lmstudio')       return callOpenAICompat(getLmstudioCompatUrl(), 'lmstudio', agent.model, user, signal);
