...
function validateCommand(step, label, credIds) {
  ...
  rejectDirectOeLifecycleCommand(step.cmd, label);
...
// Agent-authored recipes run inside the OE process. Letting one stop/restart
// OE directly would bypass restartProcess(), so no user/session continuation
// checkpoint could be written. Lifecycle changes must finish their mutation
// and then use the dedicated restart_server tool.
function rejectDirectOeLifecycleCommand(cmd, label) {
  const bin = path.basename(String(cmd[0] || '')).toLowerCase();
  const args = cmd.slice(1).map(value => String(value).toLowerCase());
  const joined = args.join(' ');
  const targetsOe = /(?:^|[\s/_.-])(?:openensemble|oe)(?:\.service)?(?:$|[\s/_.-])/.test(
    ` ${joined} `,
  );
  const lifecycle = /\b(?:restart|stop|kill|try-restart|reload-or-restart|update)\b/.test(joined);

  if (['shutdown', 'reboot', 'poweroff', 'halt'].includes(bin)) {
    throwCoded(`${label} may not stop or restart the OE host; use restart_server`, 'EINVAL');
  }
  if (bin === 'systemctl' && targetsOe && lifecycle) {
    throwCoded(`${label} may not control the OE service lifecycle; use restart_server`, 'EINVAL');
  }
  if (bin === 'oe' && lifecycle) {
    throwCoded(`${label} may not invoke OE lifecycle CLI commands; use restart_server`, 'EINVAL');
  }
  if (['sh', 'bash', 'dash', 'zsh'].includes(bin)
      && args.some(arg => arg === '-c' || arg === '--command')
      && targetsOe
      && lifecycle) {
    throwCoded(`${label} may not wrap an OE lifecycle command in a shell; use restart_server`, 'EINVAL');
  }
  if (bin === 'docker' && targetsOe && lifecycle) {
    throwCoded(`${label} may not restart the OE container directly; use restart_server`, 'EINVAL');
  }
}
