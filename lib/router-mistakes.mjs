// @ts-check
/**
 * Router-as-learner: detect explicit redirects ("@<name>", "ask coordinator",
 * "use coder") and log them as labeled router-mistake events. After threshold
...
const REDIRECT_PATTERNS = [
  /^@([a-z][a-z0-9_-]{1,30})\b/i,                       // "@<name> something"
  /^(?:no,?\s+)?(?:use|ask|have|let|get)\s+([a-z][a-z0-9_-]{1,30})\b/i,  // "use <name>", "ask coordinator"
...
export function maybePropose(userId) {
  ...
  for (const [agent, list] of byAgent) {
    ...
    // appears in both prevMessages. Reject patterns that are too short or
    // too generic — single common words like "delete", "download", "sentence"
    // would trigger on every conversation containing that word.
    const pattern = _commonPattern(best.a, best.b);
    if (!_isUsefulPattern(pattern)) continue;
...
function _commonPattern(a, b) {
  // Pick the LONGEST CONTIGUOUS phrase appearing in both messages — that's
  // far more selective than picking a single common word. Falls back to
  // the longest common single token only if no multi-word overlap exists.
  const aLow = String(a || '').toLowerCase().trim();
  const bLow = String(b || '').toLowerCase().trim();
  if (!aLow || !bLow) return '';
  // Try multi-word phrases first — slide a 3-word, 2-word window over a,
  // check substring presence in b. Captures "download the latest" not "the".
  const aWords = aLow.split(/\s+/).filter(Boolean);
  for (const wlen of [4, 3, 2]) {
    let best = '';
    for (let i = 0; i + wlen <= aWords.length; i++) {
      const phrase = aWords.slice(i, i + wlen).join(' ');
      if (phrase.length < 8) continue;          // skip "of the foo" stuff
      if (bLow.includes(phrase) && phrase.length > best.length) best = phrase;
    }
    if (best) return best;
  }
  // Fallback: longest single token >=6 chars in both
  const A = [...tokens(aLow)];
  const B = new Set(tokens(bLow));
  let bestTok = '';
  for (const t of A) {
    if (t.length >= 6 && B.has(t) && t.length > bestTok.length) bestTok = t;
  }
  return bestTok;
}
...
const GENERIC_WORDS = new Set([
  ...
  'send','email','message','what','where','when','why','how','please','sentence',
  'capital','capitals','word','words','phrase','number','file','code','image',
  'video','channel','user','users','agent','agents','task','tasks',
]);
function _isUsefulPattern(p) {
  if (!p || p.length < 8) return false;          // too short
  const words = p.trim().split(/\s+/).filter(Boolean);
  // Multi-word patterns are always useful
  if (words.length >= 2) return true;
  // Single-word patterns: reject generic command/noun terms
...
export async function detectAndLog({ userId, currentAgentId, userText }) {
  ...
  try {
    ...
    const msgs = (await loadSession(`${userId}_${currentAgentId}`, 5)) || [];
