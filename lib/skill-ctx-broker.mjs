...
import { buildRuntimeBroker } from './skill-runtime-broker.mjs';
import { buildSkillPersonalizationHelpers } from './personalization/skill-helper.mjs';

/**
 * @param {{ userId: string, agentId?: string|null, skillId: string,
 *           onEvent?: (ev: any) => void, audit?: (method: string, summary: any) => void,
 *           allowRuntime?: boolean, readOnly?: boolean }} opts
 */
export function makeCtxBroker({
  userId,
  agentId = null,
  skillId,
  onEvent = () => {},
  ...
  // Immutable reviewed/approved tool snapshots must not recover mutable live
  // binaries through parent RPC. The broker allowlist below removes the calls;
  // allowExecution is the independent defense if that list is later widened.
  const runtime = buildRuntimeBroker(userId, skillId, {
    allowPrompt: allowRuntime,
    allowExecution: allowRuntime,
  });
  const personalization = buildSkillPersonalizationHelpers({ userId, skillId });
  ...
  const handlers = {
    ...
    'personalization.confirmedPreferences': async () => {
      const rows = await personalization.confirmedPreferences();
      audit('personalization.confirmedPreferences', { count: rows.length });
      return rows;
    },
    'personalization.confirmedPreferenceDetails': async () => {
      const rows = await personalization.confirmedPreferenceDetails();
      audit('personalization.confirmedPreferenceDetails', { count: rows.length });
      return rows;
    },

    // ── external binary runtime (clamped) ────────────────────────────────────
    // Provision + run a skill-owned binary (yt-dlp etc.). buildRuntimeBroker
    // enforces bin-within-skill and writableDirs-within-the-skill's-folders.
    'runtime.ensureRuntime': async ([spec]) => {
      const p = await runtime.ensureRuntime(spec || {});
      audit('runtime.ensureRuntime', { name: spec?.name });
      return p;
    },
    'runtime.runSandboxed': async ([bin, binArgs, opts]) => {
      const r = await runtime.runSandboxed(bin, binArgs, opts || {});
      audit('runtime.runSandboxed', { code: r?.code });
      return r;
    },
    ...
    'watch': async ([opts]) => {
      const { registerWatcher, isSystemWatcherKind } = await import('../scheduler/watchers.mjs');
      const o = (opts && typeof opts === 'object') ? opts : {};
      // A sandboxed skill may only register its OWN handler kinds. A system kind (exec,
      // http_jsonpath, file_stat, …) runs in-process — refuse it so a jailed skill can't
      // plant one that would detonate in the supervisor if it were later de-sandboxed.
      if (isSystemWatcherKind(o.kind)) throw new Error(`watch: kind "${o.kind}" is reserved for the system and can't be registered by a sandboxed skill`);
      const id = registerWatcher({ ...o, userId, agentId, skillId });
      audit('watch', { kind: o.kind, id });
      return id;
    },
    'proposeMonitor': async ([opts]) => {
      const { isSystemWatcherKind } = await import('../scheduler/watchers.mjs');
      const { buildProposeMonitor } = await import('./monitor-helper.mjs');
      const o = (opts && typeof opts === 'object') ? opts : {};
      if (isSystemWatcherKind(o.kind)) throw new Error(`proposeMonitor: kind "${o.kind}" is reserved for the system and can't be registered by a sandboxed skill`);
    ...
    'unwatch': async ([watcherId]) => {
      const { getPreferenceSafeAutoContext } = await import('./personalization/safe-auto-context.mjs');
  ...
  if (!allowRuntime || readOnly) {
    delete handlers['runtime.ensureRuntime'];
    delete handlers['runtime.runSandboxed'];
  }
  if (readOnly) {
    // Dashboard widgets are polled without a chat approval turn. A manifest's
    // readOnly bit is author intent, not an enforcement boundary, so remove
    // every brokered mutation independently. Decrypted credential reads are
    // also withheld: an unattended renderer may inspect only credential
    // metadata through credentials.list(), never plaintext secret values.
    for (const method of [
      'credentials.set',
      'credentials.get',
      'credentials.delete',
