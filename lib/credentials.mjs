...
// A skill-supplied credential id must be a short safe slug BEFORE namespacing.
// Namespacing (`${skillId}__${id}`) guarantees the stored id always carries the
// skill's prefix — one skill can never read another's, even within a user.
const SKILL_CRED_ID_RE = /^[a-z0-9][a-z0-9_.-]{0,62}$/i;

/**
 * Per-skill credential accessor, namespaced + encrypted with the user master key.
 * The SAME store is used by the in-process ctx (roles.buildCtx) and the sandbox
 * broker (skill-ctx-broker), so a credential set in one path reads back in the
 * other. Values are handed to the owning skill in plaintext (v1 model); the
 * master key never leaves this process.
 * @param {string} userId @param {string} skillId
 */
export function buildSkillCredentials(userId, skillId) {
  if (!userId || !skillId) throw new Error('buildSkillCredentials: userId and skillId required');
  const prefix = `${skillId}__`;
  const ns = (id) => {
    if (typeof id !== 'string' || !SKILL_CRED_ID_RE.test(id)) throw new Error(`invalid credential id "${id}" (expected a short [a-z0-9_.-] slug)`);
    return `${skillId}__${id}`;
  };
  return {
    get:    async (id) => getCredentialValue(userId, ns(id)),
    set:    async (id, value, meta = {}) => {
      if (typeof value !== 'string' || !value.length) throw new Error('credentials.set: value must be a non-empty string');
      await storeCredential(userId, { id: ns(id), label: String(id), kind: 'skill_secret', value, meta: (meta && typeof meta === 'object') ? meta : {} });
      return true;
    },
    list:   async () => listCredentials(userId).filter(c => c.id.startsWith(prefix)).map(c => ({ ...c, id: c.id.slice(prefix.length) })),
    delete: async (id) => deleteCredential(userId, ns(id)),
  };
}
...
/**
 * Is there an unanswered consent/credential prompt for this user? Used by the
 * tool dispatcher (roles.mjs auto-bg) so a tool blocked on a user prompt is NOT
 * hurdled into the background before the user answers.
 */
export function hasPendingPrompt(userId) {
  if (!userId) return false;
  for (const e of _pending.values()) if (e.userId === userId) return true;
  return false;
}

function pendingPromptWire(id, entry, extra = {}) {
  return {
    credentialId: id,
    label: entry.label,
    ...
    ...(entry.sessionEpoch ? { session_epoch: entry.sessionEpoch } : {}),
    ...extra,
  };
}

/** Pending prompt metadata only (never the supplied value), for reconnect. */
export function getPendingCredentialPrompts(userId, agentId = null) {
  if (!userId) return [];
  const out = [];
  for (const [id, entry] of _pending) {
    if (entry.userId !== userId) continue;
    // Legacy/out-of-turn prompts are user-global and remain visible from every
    // agent; modern chat prompts are projected only into their owning chat.
    if (entry.agentId && agentId && entry.agentId !== agentId) continue;
    out.push(pendingPromptWire(id, entry));
  }
  return out;
}
...
export function requestCredential(opts) {
  ...
  const turn = getTurn();
  const promptMeta = {
    agentId: opts?.agentId ?? turn?.agentId ?? null,
    turnId: opts?.turnId ?? turn?.turnId ?? null,
    messageId: opts?.messageId ?? turn?.messageId ?? null,
    attemptId: opts?.attemptId ?? turn?.attemptId ?? turn?.turnId ?? null,
    sessionEpoch: opts?.sessionEpoch ?? turn?.sessionEpoch ?? null,
  };
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      if (entry) {
        try {
          _emit?.(userId, { type: 'credential_resolved', ...pendingPromptWire(id, entry, { cancelled: true, timedOut: true }) });
        } catch {}
      }
    }, ttlMs);
    const entry = {
      userId, kind, label, description, persist, resolve, reject, timer, expiresAt,
      ...promptMeta,
    };
...
export async function submitCredential({ credentialId, value, userId }) {
  ...
  try {
    ...
    try { _emit?.(userId, { type: 'credential_resolved', ...pendingPromptWire(credentialId, entry) }); } catch {}
    return { ok: true };
  } catch (e) {
    entry.reject(e);
    return { ok: false, error: e.message, prompt: pendingPromptWire(credentialId, entry) };
  }
}
...
export function cancelCredential({ credentialId, userId }) {
  ...
  e.code = 'CANCELLED';
  entry.reject(e);
  try { _emit?.(userId, { type: 'credential_resolved', ...pendingPromptWire(credentialId, entry, { cancelled: true }) }); } catch {}
  return { ok: true };
}

/** Cancel every pending prompt owned by a cleared/stopped chat turn. */
export function cancelPendingCredentialPrompts(userId, { agentId = null, turnId = null } = {}) {
  if (!userId) return 0;
  const ids = [];
  for (const [id, entry] of _pending) {
    if (entry.userId !== userId) continue;
  ...
  for (const credentialId of ids) {
    if (cancelCredential({ credentialId, userId }).ok) cancelled++;
  }
  return cancelled;
}
