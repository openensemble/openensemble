 ...
 *
 * Portable tiers (fast | standard | strong | reasoning) describe workload
 * shape. For named agents they tune effort without replacing the concrete
 * provider/model selected by the user. Catalog model selection is retained
 * behind an explicit opt-in for a future Auto agent mode. Custom skills never
 * need a hard-coded id table: authors can set execution_hint, otherwise
 * structure + task-shape priors fill in.
 */

export const EXECUTION_TIERS = Object.freeze(['fast', 'standard', 'strong', 'reasoning']);
export const EXECUTION_EFFORTS = Object.freeze([
  'off', 'low', 'medium', 'auto', 'high', 'xhigh', 'max', 'ultra',
]);
