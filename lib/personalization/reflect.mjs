...
- [ALREADY SET UP] lists live skills, watches, tasks, and nodes. Never invent an offer or lead that re-creates something already listed there. Prefer gaps, breakage, orphan leftovers, or genuinely new follow-ups over re-setup.
...
export function buildUserPrompt({
  existingMemories, observations, calendarEvents, sessionSummaries, now,
  timezone = null, allowPrivateCalendarDetails = true, setupInventory = null,
}) {
  let tz = timezone;
  try {
  ...
  lines.push('[ALREADY SET UP — live inventory, not evidence ids]');
  const invText = typeof setupInventory === 'string' ? setupInventory.trim()
    : (setupInventory?.text && String(setupInventory.text).trim()) || '';
  if (invText) {
    // Inventory labels come from user-owned files. Keep every line inside the
    // same JSON-string boundary used by the other untrusted prompt fields so
    // brackets/newlines cannot forge a new prompt section.
...
async function getRecentSessionSummaries(userId, daysBack = SESSION_SUMMARY_LOOKBACK_DAYS) {
  ...
  try {
    ...
    // Role assignments store the bare agent id, but session-buffer.mjs writes
    // summaries under the session-key agent id (`${userId}_${coordId}`), so a
    // bare-name-only read always saw an empty table and reflection ran with
    // zero conversation context. Read both spellings and merge.
    const sessionKey = coordId.startsWith(`${userId}_`) ? coordId : `${userId}_${coordId}`;
    const rows = [];
    for (const tableBase of new Set([sessionKey, coordId])) {
      const table = await getTable(`${tableBase}_episodes`, userId);
      // Legacy generated prose is not reliable evidence for new memories.
      rows.push(...await table.query()
        .where(`source = 'summary' AND category = 'quoted-exchanges-v1' AND forgotten = false AND created_at >= '${safeLanceVal(isoCutoff)}'`)
        .toArray());
    }
...
async function deliverAutoOfferReceipt(userId, offer) {
  ...
  if (cfg.enabled !== true || cfg.setupComplete !== true
    || isQuietEngagement(cfg) || cfg.deliveryMode !== 'immediate') return;
  try {
  ...
  let hold = !liveCfg || liveCfg.enabled !== true || liveCfg.setupComplete !== true
    || isQuietEngagement(liveCfg) || liveCfg.deliveryMode !== 'immediate';
...
async function runReflectionInner(userId, { force = false, _testCompleteJSON } = {}) {
  ...
  try {
    const { buildSetupInventory } = await import('./setup-inventory.mjs');
    const inv = buildSetupInventory(userId, { maxChars: 700 });
    if (!inv.empty) setupInventoryText = inv.text;
  } catch { /* inventory is advisory */ }
...
export async function getBriefingSection(userId) {
  ...
  let pendingReceipts = [];
  try {
    const workEnabled = ['suggest', 'prepare'].includes(config.workMode);
    const workModule = workEnabled ? await import('./work-engine.mjs') : null;
    pendingReceipts = (await listProactiveEvents(userId, { status: 'pending', limit: 100 }))
      .filter(event => (
        ...
        || (event.kind === 'prepared_work' && workModule?.workBriefingEventIsCurrent(userId, event, config)));
  } catch (e) {
