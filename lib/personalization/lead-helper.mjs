...
export function buildRegisterLead({ userId, agentId }) {
  return async function registerLead(leadPartial = /** @type {any} */ ({})) {
    try {
      ...
      // that can't happen.
      if (result?.rejected) {
        if (result.rejected === 'personalization-disabled') {
          return {
            ok: false,
            announce: "I can't set up that automatic follow-up while Personalization is off or its setup is incomplete.",
          };
        }
        if (result.rejected === 'personalization-model-off') {
          return {
            ok: false,
            announce: "I can't set up that automatic follow-up while the Personalization reflection model is set to Off.",
          };
        }
        return {
          ok: false,
      ...
      if (result?.deduped) {
        ...
        const when = describeWhen(result.existing?.nextCheckAt || leadPartial.nextCheckAt);
