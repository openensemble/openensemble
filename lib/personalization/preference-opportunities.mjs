...
function matchingPreference(preferences, recipe, skillId, {
  preferenceMemoryId = null,
  proactive = false,
  // Back-compat alias used by older tests/call sites.
  companion = false,
} = {}) {
  const openSubjectMatch = proactive || companion;
  ...
  for (const row of applicable) {
    ...
    // Proactive engagement: skill-scoped confirmed preferences (e.g. soft-
    // confirmed "chicken" from a Publix lookup) may activate this skill's
    // recipe even when the skill author never listed that subject as a
    // keyword. Global free-form preferences still require a keyword match so
    // "likes TWICE" cannot open a grocery monitor.
    if (openSubjectMatch && row?.scope === skillId && row?.tier === 'confirmed') {
      const subject = String(row?.structure?.subject || row?.subject || '').trim();
      if (subject.length >= 3) {
        const score = 0.9;
        if (score > (best?.relevance || 0)) {
          best = { row, relevance: score, matchedKeyword: subject.toLowerCase() };
        }
      }
    }
  }
...
export async function validatePreferenceActivationProposal(userId, proposal) {
  ...
  const match = matchingPreference(preferences, recipe, found.manifest.id, {
    ...
    proactive: isProactiveEngagement(cfg),
  });
  ...
  try {
    ...
    if (!active) {
      try {
        ...
        const { userInterruptionContext } = await import('./work-attention.mjs');
        occupied = (await userInterruptionContext(userId, cfg, now.getTime())).occupied;
        if (occupied) timing = 0.2;
      } catch { timing = 0.7; }
    }
    ...
    const policy = await utility.recommendOpportunityPolicy(userId, contract, {
      ...
      factors: {
        ...
        savings: 0.5,
        interruptionCost: occupied ? 0.9 : entry.recipe.activationArgs?.deliver === 'notify' ? 0.15 : 0.65,
      },
      safeAutoEligible,
      reviewedNotifyOnly,
...
async function safeAutoContext(userId, expected, { allowActive = false } = {}) {
  ...
  if (cfg.enabled !== true || cfg.setupComplete !== true
    || cfg.initiativeMode !== 'safe_auto' || isQuietEngagement(cfg)
...
export async function preferenceApprovedWatcherIsAuthorized(userId, watcher) {
  ...
  try {
    const cfg = await getConfig(userId);
    if (cfg.enabled !== true || cfg.setupComplete !== true || isQuietEngagement(cfg)) return false;
...
async function discoverPreferenceOpportunitiesUnlocked(userId, { limit = 1 } = {}) {
  if (!userId) return 0;
  const cfg = await getConfig(userId);
  if (cfg.enabled !== true || cfg.setupComplete !== true || isQuietEngagement(cfg)
  ...
  const proactive = isProactiveEngagement(cfg);
  for (const { manifest, recipe, profileRole } of await visibleCustomRecipes(userId)) {
    const match = matchingPreference(preferences, recipe, manifest.id, { proactive });
    if (!match) continue;
