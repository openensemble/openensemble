...
/** Remove receipts whose source preparation was deleted or aged out. */
export async function prunePreparedWorkEvents(userId) {
  const { readWorkState } = await import('./work-store.mjs');
  const live = new Set(readWorkState(userId).items.map(item => item.id));
  const snapshot = readFile(userId);
  if (!snapshot.events.some(event => event.kind === 'prepared_work' && !live.has(event.sourceId))) return;
  return withLock(inboxPath(userId), () => {
    const file = readFile(userId);
    const current = new Set(readWorkState(userId).items.map(item => item.id));
    file.events = file.events.filter(event => event.kind !== 'prepared_work' || current.has(event.sourceId));
    writeFile(userId, file);
  });
}
