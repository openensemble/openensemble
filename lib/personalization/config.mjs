...
// Engagement is the relationship posture (what OE is allowed to notice and
// offer). Volume presets ride along; Safe initiative stays a separate control.
export const ENGAGEMENT_MODES = Object.freeze(['quiet', 'helpful', 'proactive']);
const ENGAGEMENT_PRESETS = Object.freeze({
  quiet: { maxOffersPerRun: 1, maxUnsolicitedPingsPerDay: 0 },
  helpful: { maxOffersPerRun: 2, maxUnsolicitedPingsPerDay: 2 },
  proactive: { maxOffersPerRun: 4, maxUnsolicitedPingsPerDay: 4 },
});
// Legacy proactivity values → engagement. Old volume-only "proactive" maps to
// Helpful with higher volume so upgrades never silently opt users into the
// friendlier Proactive engagement posture. Disk values engagement:"companion"
// (brief intermediate name) migrate to "proactive".
const LEGACY_PROACTIVITY_TO_ENGAGEMENT = Object.freeze({
  quiet: 'quiet',
  balanced: 'helpful',
  proactive: 'helpful',
});
const ENGAGEMENT_TO_LEGACY_PROACTIVITY = Object.freeze({
  quiet: 'quiet',
  helpful: 'balanced',
  proactive: 'proactive',
});
const ENGAGEMENT_ALIASES = Object.freeze({ companion: 'proactive' });
...
const DEFAULTS = Object.freeze({
  ...
  // Relationship posture: quiet | helpful | proactive. "proactivity" remains
  // a derived compat alias for older callers (quiet/balanced/proactive).
  engagement: 'helpful',
  proactivity: 'balanced',
  initiativeMode: 'suggest',      // suggest | safe_auto
  workMode: 'off',                // off | suggest | prepare; separate from monitor consent
...
function normalizeEngagementMode(value) {
  if (ENGAGEMENT_MODES.includes(value)) return value;
  if (ENGAGEMENT_ALIASES[value]) return ENGAGEMENT_ALIASES[value];
  return null;
}

/**
 * Canonical engagement posture. Accepts either the new field or a legacy
 * proactivity value; unknown input fails closed to helpful (not proactive).
 * @param {any} config
 * @returns {'quiet'|'helpful'|'proactive'}
 */
export function getEngagement(config) {
  const engagement = normalizeEngagementMode(config?.engagement);
  if (engagement) return engagement;
  const legacy = LEGACY_PROACTIVITY_TO_ENGAGEMENT[config?.proactivity];
  return legacy || 'helpful';
}

/** True when unsolicited preference discovery / pings should stay off. */
export function isQuietEngagement(config) {
  return getEngagement(config) === 'quiet';
}

/** True when soft interest confirms and subject-scoped monitors may run. */
export function isProactiveEngagement(config) {
  return getEngagement(config) === 'proactive';
}

/** @deprecated use isProactiveEngagement — kept for older call sites/tests */
export function isCompanionEngagement(config) {
  return isProactiveEngagement(config);
}

function applyEngagementPreset(target, engagement, { forceVolume = true } = {}) {
  const preset = ENGAGEMENT_PRESETS[engagement] || ENGAGEMENT_PRESETS.helpful;
  target.engagement = engagement;
  target.proactivity = ENGAGEMENT_TO_LEGACY_PROACTIVITY[engagement] || 'balanced';
  if (forceVolume) {
    target.maxOffersPerRun = preset.maxOffersPerRun;
    target.maxUnsolicitedPingsPerDay = preset.maxUnsolicitedPingsPerDay;
  }
  return target;
}
...
function mergeDefaults(raw) {
  ...
  // Engagement is source of truth. Legacy proactivity still migrates so older
  // on-disk configs keep working without a write. Invalid values fail closed
  // to quiet (volume zero), never to proactive.
  const diskEngagement = Object.hasOwn(obj, 'engagement')
    ? normalizeEngagementMode(obj.engagement) : null;
  if (diskEngagement) {
    applyEngagementPreset(out, diskEngagement, {
      // Preserve explicit volume overrides stored beside engagement.
      forceVolume: !Number.isInteger(obj.maxOffersPerRun) && !Number.isInteger(obj.maxUnsolicitedPingsPerDay),
    });
    // If only one volume field was customized, still apply the other from preset.
    if (!Number.isInteger(obj.maxOffersPerRun)) {
      out.maxOffersPerRun = ENGAGEMENT_PRESETS[diskEngagement].maxOffersPerRun;
    }
    if (!Number.isInteger(obj.maxUnsolicitedPingsPerDay)) {
      out.maxUnsolicitedPingsPerDay = ENGAGEMENT_PRESETS[diskEngagement].maxUnsolicitedPingsPerDay;
    }
  } else if (Object.hasOwn(obj, 'proactivity')) {
    if (['quiet', 'balanced', 'proactive'].includes(obj.proactivity)) {
      const engagement = LEGACY_PROACTIVITY_TO_ENGAGEMENT[obj.proactivity] || 'helpful';
      applyEngagementPreset(out, engagement, {
        forceVolume: !Number.isInteger(obj.maxOffersPerRun) && !Number.isInteger(obj.maxUnsolicitedPingsPerDay),
      });
      // Old "proactive" users keep their higher volume under Helpful.
      if (obj.proactivity === 'proactive') {
        if (!Number.isInteger(obj.maxOffersPerRun)) out.maxOffersPerRun = 4;
        if (!Number.isInteger(obj.maxUnsolicitedPingsPerDay)) out.maxUnsolicitedPingsPerDay = 4;
      } else if (!Number.isInteger(obj.maxOffersPerRun) || !Number.isInteger(obj.maxUnsolicitedPingsPerDay)) {
        const preset = ENGAGEMENT_PRESETS[engagement];
        if (!Number.isInteger(obj.maxOffersPerRun)) out.maxOffersPerRun = preset.maxOffersPerRun;
        if (!Number.isInteger(obj.maxUnsolicitedPingsPerDay)) out.maxUnsolicitedPingsPerDay = preset.maxUnsolicitedPingsPerDay;
      }
    } else {
      applyEngagementPreset(out, 'quiet', { forceVolume: true });
    }
  }
  ...
  if (Object.hasOwn(obj, 'workMode')) out.workMode = ['off', 'suggest', 'prepare'].includes(obj.workMode) ? obj.workMode : 'off';
...
export async function saveConfig(userId, patch) {
  ...
  const saved = await withLock(configPath(userId), () => {
    ...
    if (['off', 'suggest', 'prepare'].includes(p.workMode)) next.workMode = p.workMode;
    ...
    // Prefer engagement when both are present. Legacy proactivity patches still
    // work for older UI/API clients. Accept "companion" as an alias for proactive.
    const patchEngagement = normalizeEngagementMode(p.engagement);
    if (patchEngagement) {
      applyEngagementPreset(next, patchEngagement, { forceVolume: false });
      const preset = ENGAGEMENT_PRESETS[patchEngagement];
    ...
    } else if (['quiet', 'balanced', 'proactive'].includes(p.proactivity)) {
      const engagement = LEGACY_PROACTIVITY_TO_ENGAGEMENT[p.proactivity] || 'helpful';
      applyEngagementPreset(next, engagement, { forceVolume: false });
      if (p.proactivity === 'proactive') {
        if (!('maxOffersPerRun' in p)) next.maxOffersPerRun = 4;
        if (!('maxUnsolicitedPingsPerDay' in p)) next.maxUnsolicitedPingsPerDay = 4;
      } else {
        const preset = ENGAGEMENT_PRESETS[engagement];
        if (!('maxOffersPerRun' in p)) next.maxOffersPerRun = preset.maxOffersPerRun;
        if (!('maxUnsolicitedPingsPerDay' in p)) next.maxUnsolicitedPingsPerDay = preset.maxUnsolicitedPingsPerDay;
