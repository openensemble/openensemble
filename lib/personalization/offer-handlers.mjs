 ...
 */
async function _runAction(action, userId, agentId, ownerKey = null) {
  const { executeToolStreaming } = await import('../../roles.mjs');
  const ownerId = `personalization:offer:${ownerKey || action.tool}`;
...
export async function runPersonalizationOffer(p, ph) {
  ...
  try {
    ...
    // Proactive soft-confirm: no tool action — write a skill-scoped confirmed
    // preference, then let preference-opportunity discovery open a watch card.
    if (p.actionContract === 'preference_soft_confirm') {
      const { acceptInterestSoftConfirm } = await import('./interest-confirm.mjs');
      const result = await acceptInterestSoftConfirm(p);
      succeeded = !!result?.ok;
      outcomeText = succeeded
        ? 'Saved — I will remember that preference.'
        : `Couldn't save it: ${result?.error || 'unknown error'}`;
    ...
    if (activationExpectation) {
      ...
      actionResult = await _runAction(
        p.action,
        p.userId,
        p.agentId,
        p.id || p.opportunityId || p.offerKind,
      );
    }
