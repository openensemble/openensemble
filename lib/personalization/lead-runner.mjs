...
async function _deliverHit(userId, leadId, line, cfg, now) {
  ...
  const quietEngagement = typeof configMod.isQuietEngagement === 'function'
    ? configMod.isQuietEngagement(cfg)
    : (cfg?.engagement === 'quiet' || cfg?.proactivity === 'quiet');
  if (quietEngagement || cfg.deliveryMode !== 'immediate') {
    await markLeadNotifyState(userId, leadId, {
  ...
  const deliveryQuiet = _leadAutomationAllowed(deliveryCfg)
    ...
    : true;
  const deliveryQuietEngagement = typeof configMod.isQuietEngagement === 'function'
    ? configMod.isQuietEngagement(deliveryCfg)
    : (deliveryCfg?.engagement === 'quiet' || deliveryCfg?.proactivity === 'quiet');
  if (!_leadAutomationAllowed(deliveryCfg)
    || deliveryQuietEngagement
