...
// LanceDB queries return list columns (the embedding) as Arrow Vector
// objects, and table.update()'s toSQL conversion only accepts plain JS
// arrays — feeding a queried vector back into an update (the rollback path)
// threw "Unsupported value type: object", and the verify comparison read the
// same Arrow object as null and reported every successful edit as failed.
// Normalize all three shapes to a plain number[] at this boundary.
function toPlainArrayValue(value) {
  if (Array.isArray(value)) return value;
  if (ArrayBuffer.isView(value) && !(value instanceof DataView)) return Array.from(/** @type {any} */ (value));
  if (value && typeof (/** @type {any} */ (value).toArray) === 'function') return Array.from((/** @type {any} */ (value)).toArray());
  return null;
}
...
function isArrayLikeValue(value) {
  return Array.isArray(value)
    ...
    || !!(value && typeof (/** @type {any} */ (value).toArray) === 'function');
}
...
async function updateCortex(userId, memoryId, values) {
  ...
  values = Object.fromEntries(Object.entries(values).map(([k, v]) => [
    k, isArrayLikeValue(v) ? toPlainArrayValue(v) : v,
  ]));
  const { table, row, id } = await cortexRow(userId, memoryId);
  if (!row) return false;
  if (Object.values(values).some(v => Array.isArray(v))) {
    // table.update() renders arrays as SQL literals, where a round element
    // ("0") types the whole literal Int64 and any later float throws a
    // datatype-mismatch. mergeInsert encodes through Arrow with the table
    // schema instead, so it is the only safe write path for the embedding.
    // whenMatchedUpdateAll needs the full row; unmatched rows are ignored.
    const merged = { ...Object.fromEntries(Object.entries(row).map(([k, v]) => [
      k, isArrayLikeValue(v) ? toPlainArrayValue(v) : v,
    ])), ...values };
    await queuedWrite('user_facts',
      () => table.mergeInsert('id').whenMatchedUpdateAll().execute([merged]), userId);
  } else {
  ...
  return Object.entries(values).every(([k, v]) => {
    if (Array.isArray(v)) {
      const actual = toPlainArrayValue(verify[0][k]);
...
export async function correctLedgerRow(userId, memoryId, { statement = null, reason = 'edit', type = null, expectedStatement = null } = {}) {
  const id = assertId(memoryId);
  ...
  return modifyLedger(userId, async (file, registerRollback, afterCommit) => {
    ...
    const row = file.rows[idx];
    if (typeof expectedStatement === 'string' && row.statement !== expectedStatement) {
      throw new Error('Memory changed on another screen. Refresh before correcting it.');
    }
    if (reason !== 'edit') {
