...
async function declaredInterestTopics({ userId, skillId, toolName, args }) {
  ...
  try {
    ...
    for (const recipe of (Array.isArray(manifest.preferenceOpportunities)
      ...
      for (const signal of (Array.isArray(recipe?.interestSignals)
        ...
        if (!topic || seen.has(dedupe) || !interestMatchesKeywords(topic, keywords)) continue;
        seen.add(dedupe);
  ...
  Promise.resolve()
    .then(async () => {
      ...
      if (resolvedOrigin === 'interactive' && resultStatus === 'ok' && skillId) {
        const topics = await declaredInterestTopics({ userId, skillId, toolName, args });
        for (const { topic, recipeId } of topics) {
          await appendObservation(userId, {
            ...
            entities: [topic],
            metadata: {
              capturePolicy: 'declared_interest_lookup_v1',
              confidence: 0.2,
              ...(recipeId ? { recipeId } : {}),
            },
            agentId: agentId || null,
