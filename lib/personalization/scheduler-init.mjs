...
import { registerBuiltin, addTask, updateTask, findTaskById, loadTasksForOwner, scheduleNewTask } from '../../scheduler.mjs';
...
async function reflectionHandler() {
  ...
  let ran = 0, skipped = 0, failed = 0, prunedTotal = 0;
  ...
  return `Personalization reflection: ${ran} reflected, ${skipped} skipped (disabled/error), ${failed} failed, ${prunedTotal} observation(s) pruned.`;
}
...
async function leadSweepHandler() {
  ...
  let usersWithLeads = 0, checked = 0, hits = 0, expired = 0;
  for (const user of users) {
    if (!user?.id) continue;
    ...
    // Cheap pre-filter before the (potentially LLM-judged) runDueLeads call:
    // listLeads({activeOnly:true}) is just a JSON read+filter, so users with
    // no active leads never pay the heavier per-lead check cost.
    let active;
    try {
      active = await listLeads(user.id, { activeOnly: true });
    } catch (e) {
  ...
  return `Personalization lead sweep: ${usersWithLeads} user(s) with active leads, ${checked} checked, ${hits} hit, ${expired} expired.`;
}
...
async function seedPersonalizationTasks() {
  ...
  const reflectionTask = existing.find(t => t.type === 'builtin' && t.handler === 'personalizationNightly');
  if (!reflectionTask) {
  ...
  if (!existing.find(t => t.type === 'builtin' && t.handler === 'personalizationLeadSweep')) {
    try {
      const saved = await addTask({
...
export async function initPersonalization() {
  // registerBuiltin is synchronous and takes effect immediately — before the
  // `await` below ever yields — so the handlers are live even if the caller
  // doesn't await this function (matching seedSystemTasks()'s fire-and-forget
  // call style at server.mjs:888).
