...
import {
  ...
  isConnected as isXaiConnected,
  GROK_CLI_PROXY_BASE,
  GROK_CLI_HEADERS,
} from '../xai-oauth-auth.mjs';
import { getAgentsForUser, getUserCoordinatorAgentId, getUser } from '../../routes/_helpers.mjs';
...
function safeRequestFailure(error) {
  ...
  return typeof raw === 'string' && /^[A-Za-z0-9_.-]{1,80}$/.test(raw)
    ? raw
    : 'network_error';
}

function safeProviderLabel(label) {
  const clean = String(label || 'Provider').replace(/[\x00-\x1f\x7f]/g, ' ').trim().slice(0, 80);
  return clean || 'Provider';
}

// ── First-class providers (ADDENDUM F enumeration list) ─────────────────────
const FIRST_CLASS_META = {
  anthropic:      { label: 'Anthropic',             kind: 'cloud', supportsText: true },
  openrouter:     { label: 'OpenRouter',             kind: 'cloud', supportsText: true },
  'openai-oauth': { label: 'OpenAI (ChatGPT login)', kind: 'cloud', supportsText: true },
  // SuperGrok / X Premium+ login (cli-chat-proxy). Same OAuth path the
  // coordinator uses when provider is xai-oauth — without this entry,
  // "Same as coordinator" reflection permanently skips with
  // UNSUPPORTED_PROVIDER whenever the coordinator is on SuperGrok OAuth.
  'xai-oauth':    { label: 'xAI Grok (SuperGrok login)', kind: 'cloud', supportsText: true },
  ollama:         { label: 'Ollama',                 kind: 'local', supportsText: true },
  lmstudio:       { label: 'LM Studio',              kind: 'local', supportsText: true },
  ...
  fireworks:      { label: 'Fireworks AI',           kind: 'cloud', supportsText: false },
  grok:           { label: 'xAI Grok',               kind: 'cloud', supportsText: true },
};

function endpointKind(rawUrl, fallback = 'cloud') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    if (['localhost', '127.0.0.1', '0.0.0.0', '::1', '[::1]'].includes(host)) return 'local';
    return 'cloud';
  } catch { return fallback; }
}

/** Server-side enforcement for account model restrictions. */
export function isReflectionModelAllowed(userId, model) {
...
/** Reject provider/model pairs that are known to have no text-completion path. */
export function isReflectionTextModel(providerId, model) {
  if (typeof providerId !== 'string' || typeof model !== 'string'
    || !providerId || !model || providerId !== providerId.trim() || model !== model.trim()
    || providerId.length > 100 || model.length > 300
    || /[\x00-\x1f\x7f]/.test(providerId) || /[\x00-\x1f\x7f]/.test(model)) return false;
  const provider = providerId.toLowerCase();
  const modelId = model;
  if (FIRST_CLASS_META[provider]?.supportsText === false) return false;
  if ((provider === 'grok' || provider === 'xai' || provider === 'xai-oauth')
    && /^grok-imagine-(?:image|video)/i.test(modelId)) return false;
  return true;
}
...
function isFirstClassConfigured(id, userId) {
  switch (id) {
    ...
    case 'xai-oauth':      return isXaiConnected(userId);
...
export async function enumerateProviders(userId) {
  ...
  for (const [id, meta] of Object.entries(FIRST_CLASS_META)) {
    const supportsText = meta.supportsText !== false;
    const configured = supportsText && isFirstClassConfigured(id, userId);
    /** @type {string[]} */
    let models = [];
    if (id === 'ollama') models = await fetchOllamaModelNames();
    else if (id === 'lmstudio') models = await fetchLmstudioModelNames();
    const kind = id === 'ollama' ? endpointKind(getOllamaUrl(), meta.kind)
      : id === 'lmstudio' ? endpointKind(getLmstudioCompatUrl(), meta.kind)
      : meta.kind;
    const allowed = models.filter(model => isReflectionModelAllowed(userId, model));
    out.push({ id, label: meta.label, kind, configured, supportsText, models: allowed });
  }
  for (const [id, meta] of Object.entries(OPENAI_COMPAT_PROVIDERS)) {
    if (id === 'xai') continue; // covered by first-class 'grok' above
    out.push({ id, label: meta.displayName || id, kind: 'cloud', configured: !!getCompatKey(id), supportsText: true, models: [] });
  }
  return out;
}
...
export async function resolveReflectionModel(userId) {
  ...
  if (!modelSetting || modelSetting === 'coordinator') {
    ...
    if (!agent?.provider || !agent?.model || !isReflectionModelAllowed(userId, agent.model)
      || !isReflectionTextModel(agent.provider, agent.model)) return null;
    return {
      providerId: agent.provider,
      model: agent.model,
      isLocal: agent.provider === 'ollama' ? endpointKind(getOllamaUrl(), 'local') === 'local'
        : agent.provider === 'lmstudio' ? endpointKind(getLmstudioCompatUrl(), 'local') === 'local' : false,
  ...
  if (typeof modelSetting === 'object' && typeof modelSetting.provider === 'string' && typeof modelSetting.model === 'string') {
    if (!isReflectionModelAllowed(userId, modelSetting.model)
      || !isReflectionTextModel(modelSetting.provider, modelSetting.model)) return null;
    return {
      providerId: modelSetting.provider,
      model: modelSetting.model,
      isLocal: modelSetting.provider === 'ollama' ? endpointKind(getOllamaUrl(), 'local') === 'local'
        : modelSetting.provider === 'lmstudio' ? endpointKind(getLmstudioCompatUrl(), 'local') === 'local' : false,
...
async function callAnthropic(model, system, user, maxTokens) {
  ...
  try {
    ...
    throw providerError('PROVIDER_UNREACHABLE', `Anthropic request failed (${safeRequestFailure(e)})`);
  }
  if (!res.ok) {
    throw providerError(res.status === 401 || res.status === 403 ? 'PROVIDER_AUTH' : 'PROVIDER_ERROR', `Anthropic HTTP ${res.status}`);
  }
  const j = /** @type {any} */ (await res.json());
...
async function callOpenAICompat(url, key, model, system, user, maxTokens, label) {
  const safeLabel = safeProviderLabel(label);
  if (!key) throw providerError('PROVIDER_NOT_CONFIGURED', `${safeLabel}: API key not configured`);
  if (!model) throw providerError('PROVIDER_ERROR', `${safeLabel}: no model specified`);
  let res;
  try {
    res = await fetch(url, {
      method: 'POST',
    ...
    throw providerError('PROVIDER_UNREACHABLE', `${safeLabel} request failed (${safeRequestFailure(e)})`);
  }
  if (!res.ok) {
    throw providerError(res.status === 401 || res.status === 403 ? 'PROVIDER_AUTH' : 'PROVIDER_ERROR', `${safeLabel} HTTP ${res.status}`);
  }
  const j = /** @type {any} */ (await res.json());
...
async function callOllama(url, key, model, system, user, maxTokens) {
  ...
  try {
    ...
    throw providerError('PROVIDER_UNREACHABLE', `Ollama request failed (${safeRequestFailure(e)})`);
  }
  if (!res.ok) {
    throw providerError('PROVIDER_ERROR', `Ollama HTTP ${res.status}`);
  }
  const j = /** @type {any} */ (await res.json());
...
async function callOpenAIResponses(model, system, user, userId) {
  ...
  try {
    ...
    throw providerError('PROVIDER_UNREACHABLE', `openai-oauth request failed (${safeRequestFailure(e)})`);
  }
  if (!res.ok) {
    throw providerError(res.status === 401 ? 'PROVIDER_AUTH' : 'PROVIDER_ERROR', `openai-oauth HTTP ${res.status}`);
  }
  let acc = '';
...
// SuperGrok OAuth — same Responses-API stream shape as openai-oauth, but
// against cli-chat-proxy.grok.com with the Grok CLI client headers. Drain
// output_text deltas and usage from response.completed (mirrors
// callOpenAIResponses above and chat/providers/openai-responses.mjs).
async function callXaiOAuthResponses(model, system, user, userId) {
  if (!model) throw providerError('PROVIDER_ERROR', 'xai-oauth: no model specified');
  const auth = await ensureFreshXaiToken(userId).catch(() => null);
  if (!auth?.access_token) {
    throw providerError('PROVIDER_AUTH', 'xai-oauth: not connected, or token refresh failed');
  }
  const headers = {
    'Content-Type':  'application/json',
    'Accept':        'text/event-stream',
  ...
  const endpoint = `${GROK_CLI_PROXY_BASE.replace(/\/$/, '')}/responses`;
  let res;
  try {
    ...
    throw providerError('PROVIDER_UNREACHABLE', `xai-oauth request failed (${safeRequestFailure(e)})`);
  }
  if (!res.ok) {
    throw providerError(
      res.status === 401 || res.status === 403 ? 'PROVIDER_AUTH' : 'PROVIDER_ERROR',
      `xai-oauth HTTP ${res.status}`,
    );
  }
  let acc = '';
  let tokensIn = null, tokensOut = null;
...
async function dispatchProvider({ providerId, model, system, user, maxTokens, userId }) {
  ...
  if (p === 'xai-oauth')    return callXaiOAuthResponses(model, system, user, userId);
  ...
  throw providerError('UNSUPPORTED_PROVIDER', `No text-completion adapter for provider "${safeProviderLabel(p)}"`);
}
...
export async function completeJSON({ userId, providerId, model, system, user, schema, maxTokens = 1500 }) {
  if (!isReflectionTextModel(providerId, model)) {
    throw providerError('PROVIDER_ERROR', 'completeJSON requires a bounded text-capable provider/model pair');
  }
