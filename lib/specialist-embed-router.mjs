...
import { listRoles, getRoleAssignments } from '../roles.mjs';
...
export async function loadIntentEmbeddings() {
  ...
  for (const m of listRoles()) {
    if (!m.service || !Array.isArray(m.intent_examples) || m.intent_examples.length === 0) continue;
    skillCount++;
    for (const phrase of m.intent_examples) {
      if (typeof phrase !== 'string' || !phrase.trim()) continue;
      try {
        const vec = await embed(phrase);
        if (Array.isArray(vec) && vec.length) next.push({ skillId: m.id, name: m.name, phrase, vec });
      } catch (e) {
