...
// Must match RESET_DURATION_MS in proposal-salience.mjs (importing it here
// would invert that module's one-way dependency on this one).
const MANUAL_RESET_GRACE_MS = 7 * 24 * 60 * 60 * 1000;

// "Resume for 7 days" in the Learn panel writes salience-overrides.json. The
// salience gate honors it, but the low-utility-score deny below is dominated
// by the same dismissal history that caused the pause — so a resumed kind
// was still silently suppressed and the button appeared to do nothing.
// During the explicit grace window, the user's judgment overrides the score.
function inManualResetGrace(userId, kind) {
  if (!userId || !kind) return false;
  const overrides = readJsonSafe(path.join(USERS_DIR, userId, 'salience-overrides.json'));
  const resetAt = overrides?.[kind]?.resetAt;
  return Number.isFinite(resetAt) && Date.now() - resetAt < MANUAL_RESET_GRACE_MS;
}
...
export function proposalFeedbackSummary(userId, { kind = null, targetKey = null, windowMs = OUTCOME_ROLLING_WINDOW_MS } = {}) {
  ...
  for (const rec of readProposalRecords(userId)) {
    ...
    // `status` is execution state, not the user's choice. In particular,
    // personalization accepts finish as `done` / `failed`, and a generic
    // accepted action may also fail after the click. acceptProposal persists
    // acceptedAt before running any handler, so it is the durable authority
    // for "the user said yes". Policy failures have no acceptedAt and must
    // not be mistaken for accepts. An undo is a later contrary choice and is
    // kept neutral here rather than counted as either acceptance or dismissal.
    const wasDismissed = rec.status === 'dismissed';
    const acceptedAt = Number(rec.acceptedAt);
    const wasAccepted = rec.status !== 'undone' && !wasDismissed && (
      (Number.isFinite(acceptedAt) && acceptedAt > 0)
      // Legacy fallback for records written before acceptedAt was durable.
      || rec.status === 'accepted'
      || rec.status === 'done'
    );
    const ts = wasDismissed
      ? (rec.dismissedAt || rec.endedAt || rec.createdAt || 0)
      : wasAccepted
        ? (acceptedAt || rec.endedAt || rec.createdAt || 0)
        : (rec.createdAt || rec.endedAt || 0);
    if (ts && ts < cutoff) continue;
    if (wasDismissed && rec.blocked) out.blocked++;
    else if (wasDismissed) out.dismissed++;
    else if (wasAccepted) out.accepted++;
    else if (rec.status === 'pending' || rec.status === 'snoozed') out.pending++;
    else continue;
    out.total++;
  }
  return out;
...
export function scoreForProposal(record, { userId = record?.userId, evidenceCount = null, minEvidence = null } = {}) {
  ...
  // Learned intents are independent skill/intent mappings. Feedback about
  // one mapping must not lower the score of every other mapping that happens
  // to share the learned_intent proposal kind.
  const usesTargetHistory = record.kind === 'learned_intent' && !!targetHistory;
  const scoredHistory = usesTargetHistory ? targetHistory : kindHistory;
  const judged = scoredHistory.accepted + scoredHistory.dismissed + scoredHistory.blocked;
  const historyScore = judged > 0
    ? (scoredHistory.accepted + 1) / (scoredHistory.accepted + scoredHistory.dismissed + (scoredHistory.blocked * 2) + 2)
    : 0.5;

  const targetDismisses = (targetHistory?.dismissed || 0) + (targetHistory?.blocked || 0) * 2;
  // When learned-intent history is already target-scoped, applying the
  // target penalty as well would count the same dismissal twice. One dismiss
  // should cause the normal 24h cooldown; two should cross the durable target
  // threshold. The scoped history score already produces that behavior.
  const targetPenalty = usesTargetHistory ? 0 : Math.min(0.4, targetDismisses * 0.18);
...
export function evidenceRequirementForProposal(userId, record) {
  ...
  // The learned_intent outcome fallback is currently kind-wide and coarse.
  // Using it here would make unrelated skill/intent targets raise or lower
  // one another's evidence requirement. Leave the fixed safety threshold in
  // place until a target-aware intent measurer exists.
...
export function evaluateLearningProposal(record, { userId = record?.userId } = {}) {
  ...
  if (Number.isFinite(scored.score) && scored.score < MIN_UTILITY_SCORE) {
    if (inManualResetGrace(userId, kind)) {
      return allow('allowed-reset-grace', risk, {
        confidence: scored.score,
        utilityScore: scored.score,
        scoreParts: scored.parts,
        evidenceCount,
