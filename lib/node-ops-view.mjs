...
function buildQualityGates({ node, profiles, summaries, watchers, incidents }) {
  ...
  for (const p of profiles) {
    ...
    if (writeOps.length && writeOps.some(o => !o.verified)) {
      add('warn', `${label} fixes unverified`, 'Some write operations cannot auto-run until verified.', 'warn');
    }
  }
...
function buildActionItems({ node, summaries, profiles, incidents }) {
  ...
  if (node.outdated) add('outdated', 'warn', `${node.hostname} agent is outdated`, `Current ${node.version || 'unknown'}, latest ${node.latestVersion || 'unknown'}.`, 'Upgrade agent', { nodeId: node.nodeId });
...
function eventLog({ node, profiles, watchers, incidents }) {
  ...
  add(new Date(node.lastSeen || Date.now()).toISOString(), 'node', node.health || 'unknown', `Node ${node.hostname || node.nodeId}`);
