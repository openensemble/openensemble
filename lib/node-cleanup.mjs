 ...
 *   1. cancel profile_health watchers for that nodeId and any known legacy ids
 *   2. delete their on-disk node data directories (profiles/incidents/snapshots/…)
 *
...
/**
 * Return the exact node keys that belong to this cleanup. Legacy profiles and
 * watchers may be keyed by hostname, from before profile storage consistently
 * canonicalized to nodeId. Aliases are supplied by the registry rather than
 * discovered by scanning the user's nodes directory, so cleanup stays narrow.
 *
 * @param {string} nodeId
 * @param {string[]} [legacyNodeIds]
 * @returns {string[]}
 */
function cleanupNodeIds(nodeId, legacyNodeIds = []) {
  return [...new Set([nodeId, ...(Array.isArray(legacyNodeIds) ? legacyNodeIds : [])]
 ...
 */
export async function cancelNodeHealthWatchers(userId, nodeId, legacyNodeIds = []) {
  if (!userId || !nodeId) return 0;
  const nodeIds = new Set(cleanupNodeIds(nodeId, legacyNodeIds));
  try {
    const { unregisterMatchingWatchers } = await import('../scheduler/watchers.mjs');
    return unregisterMatchingWatchers(
      userId,
      w => w?.kind === 'profile_health' && nodeIds.has(w?.state?.node_id),
      'node-removed',
    );
  } catch (e) {
 ...
 * @param {string} userId
 * @param {string} nodeId
 * @param {string[]} [legacyNodeIds] exact former ids, such as the node hostname
 * @returns {Promise<{ watchersCancelled: number, dataDeleted: boolean }>}
 */
export async function purgeNodeLocalData(userId, nodeId, legacyNodeIds = []) {
  const nodeIds = cleanupNodeIds(nodeId, legacyNodeIds);
  const watchersCancelled = await cancelNodeHealthWatchers(userId, nodeId, legacyNodeIds);
  let dataDeleted = false;
  for (const id of nodeIds) {
    if (deleteNodeDataDir(userId, id).deleted) dataDeleted = true;
  }
  if (watchersCancelled || dataDeleted) {
    console.log(
      `[node-cleanup] purged ${nodeId} for ${userId}: `
      + `${watchersCancelled} watcher(s), dataDir=${dataDeleted ? 'deleted' : 'absent'}`,
    );
  }
  return { watchersCancelled, dataDeleted };
}
