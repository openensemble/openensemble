...
/**
 * Run one tool call of a custom skill in the sandbox. The child's ctx.* calls
 * arrive as RPCs over stdout and are serviced by the allowlisted ctx broker
 * (skill-ctx-broker.mjs) with the enforced userId/skillId; streamed yields arrive
 * as events and are collected (and forwarded live via onEvent if provided).
...
export function runCustomSkillSandboxed(job) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // env is deliberately empty — no provider keys / secrets leak via env into
    ...
    const handleLine = async (line) => {
      ...
      if (msg.t === 'rpc') {
        // Broker enforces userId/skillId; the child's asserted values are ignored.
        try { sendToChild({ t: 'rpc-result', id: msg.id, ok: true, value: await broker.handle(msg.method, msg.args) }); }
        catch (e) { sendToChild({ t: 'rpc-result', id: msg.id, ok: false, error: e?.message || String(e) }); }
      } else if (msg.t === 'event') {
        emit(msg.event);
      } else if (msg.t === 'result') {
        done(resolve, msg.ok
          ? { ok: true, result: msg.result ?? null, events, audit }
          : { ok: false, error: msg.error || 'skill failed', events, audit });
      }
    };
    ...
    child.on('close', (code) => {
      ...
      done(reject, new Error(`custom skill '${skillId}.${toolName}' exited (${code}) without a result. stderr: ${err.slice(0, 800)}`));
    });

    sendToChild({ t: 'job', skillExecPath: execPath, toolName, args, userId, agentId });
  });
}
