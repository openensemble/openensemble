 ...
 * Dashboard widget/readOnlyCtx jobs further downgrade every normally writable
 * bind to read-only and always disable native network access. The dedicated
 * runner protocol cannot express that downgrade yet, so those jobs fail closed
 * whenever OE_SKILL_SANDBOX_SOCKET is configured.
 *
...
/**
 * Whether this process can enforce the read-only filesystem mode required for
 * unattended dashboard-widget execution. The current dedicated-runner wire
 * protocol has no read-only-mount flag, so configured remote runners are
 * deliberately unavailable until that protocol is extended.
 */
export function readOnlySkillSandboxAvailable() {
  return !String(process.env[SKILL_SANDBOX_SOCKET_ENV] || '').trim()
    && sandboxAvailable();
}
 ...
 *           procMount?: boolean, readOnlyFs?: boolean }} opts
...
export function runSandboxedJobLocal({
  ...
  return new Promise((resolve, reject) => {
    ...
    // Dashboard polling executes unattended. A manifest's readOnly bit is not
    // an OS boundary, so remount every normally-writable user/skill directory
    // read-only for this mode; code can inspect its state but cannot change it.
    const effectiveWritableDirs = readOnlyFs ? [] : writableDirs;
    const effectiveRoDirs = readOnlyFs ? [...roDirs, ...writableDirs] : roDirs;
    ...
    const bwrapArgs = buildSandboxArgs(process.execPath, nodeArgs, {
      ...
      roDirs: reviewedSnapshotRoot
        ? effectiveRoDirs.filter(dir => path.resolve(dir) !== path.resolve(skillDir))
        : effectiveRoDirs,
    ...
    let stderrDrainTimer = null;
    let stderrDrained = child.stderr.readableEnded === true || child.stderr.destroyed === true;
    ...
    const timer = setTimeout(() => {
      ...
      const error = /** @type {Error & {stderr?: string}} */ (
        new Error(`sandboxed job (${skillId}) timed out after ${timeoutMs}ms`)
      );
    ...
    const done = (fn, v) => {
      ...
      child.stderr.off('end', onStderrDrained);
      child.stderr.off('close', onStderrDrained);
    ...
    function finishPendingResult() {
      if (settled || !pendingResult || timedOut || aborted) return;
    ...
    function onStderrDrained() {
      stderrDrained = true;
      if (timedOut || aborted) return;
      finishPendingResult();
    }
    function awaitStderrDrain(msg) {
      if (pendingResult || settled || timedOut || aborted) return;
      pendingResult = msg;
      // The host flushes and closes its diagnostic writer before publishing
      // this result. Stop the completed process group, then allow the
      // independent stderr pipe to deliver bytes already handed to the OS.
      // The first job terminal event wins, so an on-time result stops the work
      // timer; Abort remains authoritative during transport drain. An
      // inherited fd cannot hold this promise open indefinitely.
      clearTimeout(timer);
      inputSender?.close();
      try { child.stdin.end?.(); } catch {}
      killProcessGroup();
      if (stderrDrained || child.stderr.readableEnded === true || child.stderr.destroyed === true) {
        finishPendingResult();
        return;
      }
      stderrDrainTimer = setTimeout(finishPendingResult, STDERR_DRAIN_GRACE_MS);
    }
    signal?.addEventListener('abort', onAbort, { once: true });
    // Close the tiny race between the pre-spawn check and listener install.
    if (signal?.aborted) { onAbort(); return; }
    inputSender = createPlainFrameSender(child.stdin, error => {
      if (!pendingResult) done(reject, error);
    });
    ...
    const handleMessage = (msg) => {
      if (settled || pendingResult || signal?.aborted) return;
      if (msg.t === 'rpc') {
    ...
    child.stderr.on('end', onStderrDrained);
    child.stderr.on('close', onStderrDrained);
    ...
    child.on('error', (e) => {
      if (!pendingResult) done(reject, e);
    });
    child.on('close', (code) => {
      if (aborted) {
        done(reject, abortError());
...
export function runSandboxedJob(opts) {
  ...
  // The dedicated runner protocol predates read-only mount requests. Refuse
  // rather than silently executing an unattended widget with writable binds.
  if (socketPath && opts?.readOnlyFs === true) {
    return Promise.reject(new Error(
      'read-only custom-skill filesystem mode is unavailable through the configured sandbox runner',
    ));
  }
...
export async function runCustomSkillSandboxed(job) {
  const {
    ...
    timeoutMs = 60_000, onEvent = null, execSnapshotPath = null, signal = null,
    readOnlyCtx = false,
  } = job;
  ...
  const broker = makeCtxBroker({
    ...
    allowRuntime: !execSnapshotPath && !readOnlyCtx,
    readOnly: readOnlyCtx === true,
  });
  const { execPath } = customSkillBindings(userId, skillId);
  const jobPayload = {
    t: 'job', mode: 'tool', skillExecPath: execPath, toolName, args, userId, agentId,
    readOnlyCtx: readOnlyCtx === true,
  };
  try {
    const r = await runSandboxedJob({
      userId, skillId, jobPayload, handleRpc: broker.handle, onEvent: emit,
      net: readOnlyCtx ? false : net,
      timeoutMs,
      execSnapshotPath,
      signal,
      readOnlyFs: readOnlyCtx === true,
    });
