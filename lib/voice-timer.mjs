...
export async function createVoiceTimer({ userId, deviceId, seconds }) {
  ...
  try {
    const { syncScheduledDeviceAlarm, waitForScheduledDeviceAlarm } = await import('./scheduled-device-alarms.mjs');
    const armed = await syncScheduledDeviceAlarm(task, when.toISOString());
    if (armed) {
      const confirmed = await waitForScheduledDeviceAlarm(task);
      if (!confirmed || confirmed.cancelled || !confirmed.deviceIds.every(id => confirmed.armedDeviceIds.includes(id))) {
        return `Okay, ${adj} timer started. Offline ringing is not confirmed yet.`;
      }
    }
  } catch (error) {
    // The server task is already durable and scheduled. A failed device
    // confirmation must not make the caller retry creation and duplicate it.
    console.warn('[voice-timer] offline confirmation failed:', error.message);
    return `Okay, ${adj} timer started. Offline ringing is not confirmed yet.`;
  }
...
// The dispatcher needs to know a disambig question was just staged so it can
// reopen the device mic for the full TTL — regardless of conversation mode,
// or the 30s server window is unreachable without a fresh wake word.
export function hasPendingDisambig(deviceId) {
  const p = PENDING_DISAMBIG.get(deviceId);
  return !!p && Date.now() < p.expiresAt;
}

export { DISAMBIG_TTL_MS };
