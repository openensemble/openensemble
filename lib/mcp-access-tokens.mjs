 ...
 * @property {string[]} toolAllowlist exact additional agent tools permitted by a chat scope
...
function saveStore(list) {
  const target = resolveWriteTargetSync(STORE_PATH);
...
function sanitize(rec) {
  ...
  // Migration is deliberately fail-closed: tokens minted before agent-tool
  // capabilities existed become read-only-by-default without needing a store
  // rewrite. Malformed hand-edited values also collapse to no extra tools.
  const toolAllowlist = Array.isArray(rec.toolAllowlist)
    ? [...new Set(rec.toolAllowlist.filter(name => typeof name === 'string' && TOOL_NAME_RE.test(name)))]
      .slice(0, MAX_TOOL_ALLOWLIST)
    : [];
  return {
    ...rest,
    toolAllowlist,
...
function validateToolAllowlist(value, scopes) {
  if (value === undefined) return [];
  if (!Array.isArray(value)) throw new Error('toolAllowlist must be an array of exact tool names');
  const clean = [...new Set(value.map(name => String(name || '').trim()).filter(Boolean))];
  if (clean.length > MAX_TOOL_ALLOWLIST) {
    throw new Error(`toolAllowlist cannot contain more than ${MAX_TOOL_ALLOWLIST} tools`);
  }
  const invalid = clean.find(name => !TOOL_NAME_RE.test(name));
  if (invalid) throw new Error(`invalid tool name in toolAllowlist: "${invalid}"`);
  if (clean.length && !scopes.includes('chat')) {
    throw new Error('toolAllowlist requires the "chat" scope');
  }
  return clean;
}
 ...
 *
 * @param {string} userId
 * @param {{name?: string, scopes?: string[], agentId?: string|null, toolAllowlist?: string[]}} opts
 * @returns {{token: string, record: Omit<AccessTokenRecord,'sha256'>}}
 */
export function createAccessToken(userId, {
  name = '', scopes = ['chat', 'memory-read'], agentId = null, toolAllowlist = undefined,
} = {}) {
  ...
  const cleanToolAllowlist = validateToolAllowlist(toolAllowlist, cleanScopes);
  ...
  const record = {
    ...
    toolAllowlist: cleanToolAllowlist,
