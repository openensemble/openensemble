...
export async function dismissProposal(id, opts = {}) {
  ...
  if (p.kind === 'personalization_offer' && p.offerKind) {
    import('./personalization/graduation.mjs').then(m => m.recordOfferOutcome(p.userId, p.offerKind, 'dismiss'))
      .catch(e => console.warn('[personalization] recordOfferOutcome (dismiss) failed:', e.message));
  }
