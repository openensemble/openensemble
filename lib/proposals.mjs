...
import { isDestructiveText, isLearnableAliasPhrase } from './learning-safety.mjs';
...
// Terminal records (accepted/dismissed/failed/expired) older than this are
// dropped at boot load — proposals.json otherwise grows forever and every
// record in it is re-serialized on every persist and re-walked on every
// UI poll.
const TERMINAL_PRUNE_MS = 90 * 24 * 60 * 60 * 1000;
const SNOOZE_MS = 7 * 24 * 60 * 60 * 1000;
const KIND_RATE_CAP_MS = 24 * 60 * 60 * 1000;
const RATE_CAPPED_KINDS = new Set(['watch', 'recurring_task']);
...
// ── Policy gate ──────────────────────────────────────────────────────────────
//
// personalization_* kinds (personalization_offer, personalization_graduate)
// carry their own calibrated gates — per-kind dismiss-suppression and
// accept-graduation thresholds, daily ping budgets, quiet hours (see
// lib/personalization/graduation.mjs) — and evaluateLearningProposal's
// utility-score formula was never tuned for them. Routing them through it
// anyway created a self-sustaining lockout: two dismissed offers of ANY
// kind drive the generic kind-history score below MIN_UTILITY_SCORE, and
// since no further offer can ever be created to raise it back up, every
// future personalization offer silently stops being created — with no
// signal to the user or to Settings → Personalization that anything is wrong.
// This also means personalization proposals bypass the user's generic
// learning-policy master switch (setLearningPolicy(userId,{enabled:false}))
// too — that's intended, not an oversight: personalization has its own
// master switch (config.enabled in Settings → Personalization), separate
// from the generic one.
//
// Every call site that runs the generic policy against a proposal record —
// createProposal, the boot-time pending/snoozed sweep, listUserProposals'
// live sweep, and acceptProposal — must go through this helper rather than
// calling evaluateLearningProposal directly. A call site that skips it will
// retroactively flip an already-pending personalization card to 'failed'
// (or block the user's accept click) the next time that record is touched,
// even though createProposal let it through in the first place.
function policyForProposal(record) {
  if (typeof record?.kind === 'string' && record.kind.startsWith('personalization_')) {
    return { allow: true, risk: 'safe', reason: 'personalization-own-gates' };
  }
  return evaluateLearningProposal(record);
}

// Accepted proposals run without a human-attended foreground turn, but the
// proposal card/outcome record is already their durable owner. Bind nested
// tools to that identity so a 10-second generic auto-bg handoff cannot mark
// the proposal accepted before the actual mutation has finished.
function acceptedProposalTaskContext(proposal, userId, sessionKey) {
  const taskId = `proposal:${proposal.id}`;
  return {
    taskId,
    watcherId: null,
    userId,
...
function loadAllUsersFromDisk() {
  ...
  for (const entry of fs.readdirSync(USERS_DIR, { withFileTypes: true })) {
    ...
    for (const rec of (data.proposals || [])) {
      ...
      if (advanceProposalState(rec)) usersWithReap.add(uid);
      if (rec.status === 'pending' || rec.status === 'snoozed') {
        const policy = policyForProposal(rec);
        if (!policy.allow) {
          rec.status = 'failed';
          ...
          rec.policy = {
            ...
            utilityScore: policy.utilityScore ?? policy.confidence ?? null,
            scoreParts: policy.scoreParts ?? null,
      ...
      // Prune terminal records past the retention window — don't load them
      // into memory, and mark the user so the shrunken set writes back.
      if (rec.status !== 'pending' && rec.status !== 'snoozed') {
        const age = Date.now() - (rec.endedAt || rec.createdAt || 0);
        if (age > TERMINAL_PRUNE_MS) { usersWithReap.add(uid); continue; }
      }
...
function cooldownKey(record) {
  ...
  if (record?.kind === 'watch' && record.watchSourceKey) return `watch:${record.watchSourceKey}`;
  ...
  // Keep the policy category stable for graduation, but de-duplicate concrete
  // opportunities by instance.  A generic `offer:<kind>` key meant accepting
  // or seeing one deadline reminder suppressed every other deadline reminder
  // during the cooldown window, even when the event/time was different.
  if (record?.kind === 'personalization_offer' && record.opportunityId) return `offer-instance:${record.opportunityId}`;
  if (record?.kind === 'personalization_offer' && record.offerKind) return `offer:${record.offerKind}`;
  return patternKey(record?.message);
}

function countsAsKindEmission(record) {
  return ['pending', 'snoozed', 'running', 'accepted', 'dismissed'].includes(record?.status);
}

function hasRecentKindEmission(userId, kind, ignoreId = null) {
  const cutoff = Date.now() - KIND_RATE_CAP_MS;
  for (const existing of _proposals.values()) {
    if (ignoreId && existing.id === ignoreId) continue;
    if (existing.userId !== userId || existing.kind !== kind) continue;
    if (!countsAsKindEmission(existing)) continue;
    if ((existing.createdAt || 0) >= cutoff) return true;
  }
  return false;
}
...
export async function createProposal(record) {
  // Finish async gating before checking shared state. No await may separate
  // dedup/rate-limit checks from publishing the new record into the map.
  try {
    const { getKindStatus } = await import('./proposal-salience.mjs');
    const verdict = getKindStatus(record.userId, record.kind, record);
    if (!verdict.allow) {
  ...
  const policy = policyForProposal(record);
  if (!policy.allow) {
  ...
  if (record.kind !== 'skill_refine') {
    ...
    for (const existing of _proposals.values()) {
      if (existing.userId !== record.userId) continue;
      getProposal(existing.id);
      if (['pending', 'snoozed', 'running'].includes(existing.status) && cooldownKey(existing) === newKey) {
        return null;
      }
    }
  }
  ...
  if (RATE_CAPPED_KINDS.has(record.kind) && hasRecentKindEmission(record.userId, record.kind, record.id)) {
    console.log(`[proposals] skipped ${record.kind} by daily kind rate cap`);
    return null;
  }
...
/**
 * Monitorable-intent escalation path. Unlike maybePropose(), this is not
 * driven by exact repeated commands; it is called after the classifier ledger
 * has seen the user ask about the same changing external state more than once.
 */
export async function proposeMonitorableWatch({ userId, agentId, message, matched = null, source = null, evidenceCount = 2 }) {
  if (!userId || !agentId || !message) return null;
  if (isDismissedRecently(userId, message)) return null;
  if (isDestructive(message)) return null;
  const watchSource = source?.ok ? source : extractMonitorableSource(message);
  if (!watchSource?.ok) {
    console.log(`[proposals] watch suppressed (no nameable source): ${(message || '').slice(0, 80)}`);
    return null;
  }
  const targetLabel = watchSource.label;
  return createProposal({
    id: 'prop_' + randomUUID().slice(0, 12),
    userId,
    agentId,
    kind: 'watch',
    message: `You keep asking about a changing source. Set up a monitor for: ${targetLabel}`,
    matched,
    watchSource: targetLabel,
    watchSourceKey: watchSource.key,
    sourceKind: watchSource.kind || null,
    lastUserMessage: message,
    evidenceCount,
    accept_label: 'Set up the watch',
    dismiss_label: 'No, just answer',
    createdAt: Date.now(),
    status: 'pending',
  });
}
...
// default_arg proposer RETIRED 2026-07-02 (see RETIRED_PROPOSAL_KINDS in
// learning-policy.mjs). Tool args are model-authored, so "you used this value
// N times" was never a user preference. runDefaultArgPin below stays so
// previously-accepted pins keep their history rendering and revoke path.
...
function advanceProposalState(p, now = Date.now()) {
  let changed = false;
  if (p.status === 'snoozed' && p.wakeAt && now >= p.wakeAt) {
    p.status = 'pending';
    p.pendingSince = p.wakeAt;
    p.wakeAt = null;
    changed = true;
  }
  // Snooze starts a fresh response window at the scheduled return time.
  if (p.status === 'pending' && now - (p.pendingSince || p.createdAt) > PROPOSAL_TTL_MS) {
    p.status = 'expired';
    p.endedAt = now;
    changed = true;
  }
...
export function getProposal(id) {
  ...
  if (!p) return null;
  if (advanceProposalState(p)) {
    persistUser(p.userId).catch(e => console.warn('[proposals] lifecycle persist failed:', e.message));
    persistOutcome(p, p.status, p.status === 'expired' ? 'Suggestion expired.' : 'Ready for review.').catch(() => {});
  }
  return p;
}
...
export function listUserProposals(userId, status = 'pending') {
  ...
  for (const p of _proposals.values()) {
    if (p.userId === userId) getProposal(p.id);
    if (p.userId === userId && (p.status === 'pending' || p.status === 'snoozed')) {
      const policy = policyForProposal(p);
      if (!policy.allow) {
        p.status = 'failed';
...
async function failRunningProposal(proposal, error, context = 'accept') {
  console.warn(`[proposals] ${context} threw outside its handler:`, error?.message || error);
  if (proposal.status !== 'running') return;
  const reason = error?.message || 'unknown';
  proposal.status = 'failed';
  proposal.outcome = `Failed: ${reason}`;
  proposal.endedAt = Date.now();
  try {
    await persistUser(proposal.userId);
    await persistOutcome(proposal, 'failed', `Couldn’t set it up: ${reason}`);
    _wsBroadcastFn?.(proposal.userId, {
      type: 'proposal_outcome', proposalId: proposal.id, agent: proposal.agentId,
      status: 'failed', outcome: `Couldn’t set it up: ${reason}`,
    });
  } catch (persistError) {
    console.warn('[proposals] failed to persist accept failure:', persistError.message);
  }
}
...
export async function acceptProposal(id) {
  ...
  if (p.kind === 'skill_proposal') {
    runSkillProposal(p).catch(e => failRunningProposal(p, e, 'skill proposal'));
    return { ok: true, status: 'running', proposal: p };
  }
  ...
  if (p.kind === 'tool_failure') {
    runToolFailureRemedy(p).catch(e => failRunningProposal(p, e, 'tool-failure remedy'));
    return { ok: true, status: 'running', proposal: p };
  }
  ...
  if (p.kind === 'skill_refine') {
    runSkillRefine(p).catch(e => failRunningProposal(p, e, 'skill refine'));
    return { ok: true, status: 'running', proposal: p };
  }

  // personalization_offer / personalization_graduate — delegate to
  // lib/personalization/offer-handlers.mjs so this shared-file hunk stays
  // small; handlers set terminal status + persistOutcome + broadcast
  // themselves (copy runRulePromotion's shape).
  if (p.kind === 'personalization_offer') {
    const m = await import('./personalization/offer-handlers.mjs');
    m.runPersonalizationOffer(p, { persistOutcome, broadcast: _wsBroadcastFn, persistUser }).catch(e => {
      console.warn('[proposals] personalization offer threw outside its handler:', e.message);
    });
    return { ok: true, status: 'running', proposal: p };
  }
  if (p.kind === 'personalization_graduate') {
    const m = await import('./personalization/offer-handlers.mjs');
    m.runPersonalizationGraduate(p, { persistOutcome, broadcast: _wsBroadcastFn, persistUser }).catch(e => {
      console.warn('[proposals] personalization graduate threw outside its handler:', e.message);
    });
    return { ok: true, status: 'running', proposal: p };
  }

  // Other kinds (watch, recurring_task) need the agent to call its own tools.
  // Early failures (including topology admission or agent resolution) happen
  // before runAcceptedAgent's normal terminal write. Never strand the card
  // in "running" indefinitely.
  runAcceptedAgent(p).catch(e => failRunningProposal(p, e, 'accept run'));
...
/**
 * Revert an accepted proposal within 24h using its saved version. Serialize
 * repeated Undo requests so a restored value cannot be undone a second time.
 */
export async function undoProposal(id) {
  return withLock(`proposal-undo:${id}`, () => undoAcceptedProposal(id));
}

async function undoAcceptedProposal(id) {
  const p = _proposals.get(id);
  if (!p) return { ok: false, error: 'not found' };
  if (!p.producedArtifact) return { ok: false, error: 'no recorded artifact' };
  const isPersonalizationDoneArtifact = ['personalization_policy', 'preference_monitor']
    .includes(p.producedArtifact.kind);
  if (p.status !== 'accepted' && !(isPersonalizationDoneArtifact && p.status === 'done')) {
    return { ok: false, error: `cannot undo ${p.status}` };
  }
  ...
  let result;
  try {
    if (art.kind === 'personalization_policy') {
      const { revokeKindAutoApproval } = await import('./personalization/graduation.mjs');
      result = await revokeKindAutoApproval(p.userId, art.offerKind);
    } else if (art.kind === 'preference_monitor') {
      const controls = await import('./personalization/preference-opportunities.mjs');
      result = typeof p.preferenceReceiptEventId === 'string' && p.preferenceReceiptEventId
        ? await controls.controlPreferenceAutomationReceipt(
          p.userId, p.preferenceReceiptEventId, 'undo',
        )
        : await controls.controlPreferenceMonitorArtifact(p.userId, art, 'undo');
    } else if (art.kind === 'rule') {
      const { userRoleRulesPath } = await import('./paths.mjs');
      const rpath = userRoleRulesPath(p.userId, art.roleId);
      result = restoreProposalUndo(rpath, art.undo, before => atomicWriteSync(rpath, before));
    } else if (art.kind === 'alias') {
      const { loadAliases, saveAliases, normalizeAliasPhrase } = await import('./ha-aliases.mjs');
      result = restoreProposalUndo(path.join(USERS_DIR, p.userId, 'ha-aliases.json'), art.undo, before => {
        const all = loadAliases(p.userId);
        const phrase = normalizeAliasPhrase(art.phrase);
        if (before === null) delete all[phrase];
        else all[phrase] = before;
        saveAliases(p.userId, all);
      });
    } else if (art.kind === 'routine') {
      const { loadRoutines, saveRoutines } = await import('./routines.mjs');
      result = restoreProposalUndo(path.join(USERS_DIR, p.userId, 'routines.json'), art.undo, before => {
        const { routines } = loadRoutines(p.userId);
        saveRoutines(p.userId, before === null
          ? routines.filter(r => r.id !== art.routineId)
          : routines.map(r => r.id === art.routineId ? before : r));
      });
    } else if (art.kind === 'pin') {
      const { undoDefaultPin } = await import('./tool-defaults.mjs');
      result = await undoDefaultPin(p.userId, art.tool, art.arg, art.undo);
    } else if (art.kind === 'override') {
      const { undoOverride } = await import('./routing-overrides.mjs');
      result = await undoOverride(p.userId, art.overrideId, art.undo);
    } else if (art.kind === 'learned_intent') {
      const { undoLearnedIntent } = await import('./learned-intents.mjs');
      result = await undoLearnedIntent(p.userId, art.skillId, art.intentId, art.undo);
    } else {
...
async function recordPersonalizationDismissal(p, { permanent = false } = {}) {
  if (p?.kind !== 'personalization_offer' || !p.offerKind) return true;
  const [graduation, recorder] = await Promise.all([
    import('./personalization/graduation.mjs'),
    import('./personalization/recorder.mjs'),
  ]);
  await graduation.recordOfferOutcome(p.userId, p.offerKind, 'dismiss');
  if (permanent) await graduation.setKindSuppressed(p.userId, p.offerKind, true);
  await recorder.recordStructuredSignal({
      userId: p.userId,
      agentId: p.agentId || null,
      type: 'choice',
      statement: `${permanent ? 'Blocked' : 'Dismissed'} personalization offer: ${p.offerKind}`,
      source: 'personalization_offer',
      metadata: {
        choice: permanent ? 'blocked' : 'dismissed',
        offerKind: p.offerKind,
        opportunityId: p.opportunityId || null,
        proposalId: p.id || null,
        reason: p.dismissReason || null,
      },
  });
  return true;
}
...
export async function dismissProposal(id, opts = {}) {
  ...
  try { await recordPersonalizationDismissal(p); }
  catch (e) { console.warn('[personalization] dismissal signal capture failed:', e.message); }
  if (p.actionContract === 'skill_preference_activation') {
    try {
      const { recordPreferenceProposalOutcome } = await import('./personalization/preference-opportunities.mjs');
      await recordPreferenceProposalOutcome(p.userId, p, 'dismissed');
    } catch (e) {
...
export async function blockProposal(id, opts = {}) {
  ...
  try {
    await recordPersonalizationDismissal(p, { permanent: true });
  } catch (e) {
    console.warn('[personalization] permanent dismissal policy failed:', e.message);
    return { ok: false, error: 'failed to persist personalization block' };
  }
  p.status = 'dismissed';
  p.dismissedAt = Date.now();
  p.blocked = true;
...
async function withProposalTopologyLease(userId, label, fn) {
  const topology = await import('../chat-dispatch/slot-registry.mjs');
  let lease = null;
  for (let attempt = 0; attempt < 80 && !lease; attempt++) {
    lease = topology.tryAcquireUserTurnLease(userId, { label });
    if (!lease) await new Promise(resolve => setTimeout(resolve, 25));
  }
  if (!lease) throw new Error('account setup is changing; try accepting the proposal again');
  try {
    return await topology.runWithUserTopologyLease(lease, fn);
  } finally {
    lease.release();
  }
}
...
async function runAcceptedAgent(proposal) {
  ...
  return withProposalTopologyLease(userId, `proposal:${proposal.id}`, async () => {
  ...
  const watchTarget = proposal.watchSource || message;
  const taskHint = kind === 'watch'
    ? `Use create_watch to set up a monitor for: "${watchTarget}". Pick a sensible source (http_jsonpath / exec / file_stat) and a reasonable cadence.`
  ...
  const { succeeded, assistantContent: assistantBuf, lastError } = await runAgentWithRetry({
    ...
    context: 'proposals',
    rootTaskId: `proposal:${proposal.id}`,
    traceSource: 'proposal',
    taskContext: acceptedProposalTaskContext(proposal, userId, sessionKey),
  });
...
async function runRulePromotion(proposal) {
  ...
  try {
    ...
    if (!lines.includes(newLine)) {
      lines.push(newLine);
      atomicWriteSync(p, lines.join('\n') + '\n', 'utf8');
    }
    proposal.producedArtifact = { kind: 'rule', roleId, ruleText: ruleText.trim(),
      undo: recordProposalUndo(p, existing, fs.readFileSync(p, 'utf8')) };
    succeeded = true;
...
async function runAliasProposal(proposal) {
  ...
  try {
    ...
    const { setAlias, loadAliases, normalizeAliasPhrase } = await import('./ha-aliases.mjs');
    const phraseKey = normalizeAliasPhrase(phrase);
    const before = loadAliases(userId)[phraseKey] ?? null;
    setAlias(userId, phrase, entityId);
    proposal.producedArtifact = { kind: 'alias', phrase,
      undo: recordProposalUndo(path.join(USERS_DIR, userId, 'ha-aliases.json'), before, entityId) };
    succeeded = true;
...
async function runRoutingOverride(proposal) {
  ...
  try {
    ...
    if (result.ok) {
      ...
      proposal.producedArtifact = { kind: 'override', overrideId, undo: result.undo };
    } else {
...
async function runLearnedIntent(proposal) {
  ...
  let outcomeText;
  try {
    const { addLearnedUtterances } = await import('./learned-intents.mjs');
    const result = await addLearnedUtterances(userId, { skillId, intentId, tool, utterances }, { recordUndo: true });
    if (!result.ok) throw new Error(result.error);
    succeeded = true;
    proposal.producedArtifact = { kind: 'learned_intent', skillId, intentId, utterances: utterances || [], undo: result.undo };
...
async function runToolFailureRemedy(proposal) {
  return withProposalTopologyLease(
    proposal.userId,
    `proposal-remedy:${proposal.id}`,
    () => runToolFailureRemedyLeased(proposal),
  );
}

async function runToolFailureRemedyLeased(proposal) {
...
async function runDefaultArgPin(proposal) {
  ...
  try {
    ...
    const result = await pinDefault(userId, tool, arg, value, { recordUndo: true });
    if (result.ok) {
      succeeded = true;
      const display = typeof value === 'string' ? `"${value}"` : String(value);
      outcomeText = `Pinned. \`${tool}\` now defaults \`${arg} = ${display}\` when you don't specify it.`;
      proposal.producedArtifact = { kind: 'pin', tool, arg, undo: result.undo };
    } else {
...
async function runRoutineProposal(proposal) {
  ...
  try {
    ...
    const before = routines.find(r => r.id === id) ?? null;
    const next = {
      ...before,
      id,
      trigger,
    ...
    const saved = saveRoutines(userId, before
      ? routines.map(r => r.id === id ? next : r)
      : [...routines, next]);
    const after = saved.routines.find(r => r.id === id);
    if (!after) {
      throw new Error('routine failed validation on save');
    }
    proposal.producedArtifact = { kind: 'routine', routineId: id,
      undo: recordProposalUndo(path.join(USERS_DIR, userId, 'routines.json'), before, after) };
    succeeded = true;
...
async function runSkillProposal(proposal) {
  return withProposalTopologyLease(
    proposal.userId,
    `proposal-skill-create:${proposal.id}`,
    () => runSkillProposalLeased(proposal),
  );
}

async function runSkillProposalLeased(proposal) {
...
async function runSkillRefine(proposal) {
  return withProposalTopologyLease(
    proposal.userId,
    `proposal-skill-refine:${proposal.id}`,
    () => runSkillRefineLeased(proposal),
  );
}

async function runSkillRefineLeased(proposal) {
...
// Narrow test seam for the autonomous LLM paths. Keeping these runners
// private in production avoids making proposal internals part of the public
// API; tests invoke them directly so topology admission is exercised without
// relying on acceptProposal's intentionally fire-and-forget scheduling.
export const _topologyForTests = {
  runAcceptedAgent,
  runToolFailureRemedy,
  runSkillProposal,
  runSkillRefine,
};
