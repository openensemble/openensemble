 ...
 * always win.
 */
// Args that should NEVER become a default no matter how often repeated:
// per-call identifiers/handles, the tool's subject/target arg, and dates.
// Pinning any of these breaks the tool — every later call reuses a stale id or
// targets the wrong thing. (Real-world noise this kills: default tabId/extId/
// draftId, checkin_date, action="next", sender=…, domain=…, skillId=….)
function defaultArgIsNoise(arg, value) {
  const a = String(arg || '');
  if (/Id$/.test(a) || /(^|_)(id|ids|uuid|guid|token)$/i.test(a)) return true;   // identifiers/handles
  const SUBJECT = new Set(['sender', 'recipient', 'to', 'cc', 'bcc', 'domain', 'url', 'query', 'q',
    'action', 'verb', 'command', 'cmd', 'path', 'file', 'address', 'phrase', 'keyword', 'term',
    'subject', 'body', 'text', 'prompt']);
  if (SUBJECT.has(a.toLowerCase())) return true;                                  // the tool's per-call target
  if (/date/i.test(a)) return true;                                              // dates are transient
  const v = typeof value === 'string' ? value : '';
  if (/^\d{4}-\d{2}-\d{2}/.test(v)) return true;                                 // ISO date value
  if (/^[0-9a-f]{12,}$/i.test(v)) return true;                                   // opaque hex id value
  if (typeof value === 'number' && Math.abs(value) > 1e6) return true;          // big numeric runtime id
  return false;
}

export async function proposeDefaultArg({ userId, agentId, tool, arg, value, count }) {
  if (!userId || !tool || !arg) return null;
  if (defaultArgIsNoise(arg, value)) {
 ...
 */
export async function proposeLearnedIntent({ userId, agentId, skillId, intentId, tool, utterances }) {
  ...
  return createProposal({
    ...
    kind: 'learned_intent',
    message,
    skillId, intentId, tool: tool || null,
...
export async function blockProposal(id) {
  const p = _proposals.get(id);
  if (!p) return { ok: false, error: 'not found' };
  if (p.status !== 'pending') return { ok: false, error: `already ${p.status}` };
  p.status = 'dismissed';
  ...
  _blockedPatterns.get(p.userId).add(key);
