...
const RATE_CAPPED_KINDS = new Set(['default_arg', 'watch', 'recurring_task']);
 ...
 * always win.
 */
export async function proposeDefaultArg({ userId, agentId, tool, arg, value, count, userAuthored = false }) {
  ...
  const valueDisplay = typeof value === 'string'
    ...
    : String(value);
  const message =
    `I keep seeing \`${tool}\` use \`${arg} = ${valueDisplay}\` from values you typed ${count} times recently. ` +
  ...
  return createProposal({
    ...
    message,
    tool, arg, value,
    userAuthored: userAuthored === true,
    count: count || 0,
