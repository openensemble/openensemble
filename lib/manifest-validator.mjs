// @ts-check
import { validateSkillDashboardWidgets } from './dashboard-widgets.mjs';
/**
 ...
 *   5. dashboardWidgets bind only bounded declarations to exact read-only,
 *      non-destructive tools from the same skill.
 *
...
export function validateManifestCode(manifest, code) {
  ...
  const diagnostics = [
    ...
    ...validateDashboardWidgets(manifest),
  ];
...
/**
 * Dashboard widgets are automatically polled outside a chat turn, so their
 * manifest contract is deliberately narrower than an ordinary skill tool.
 * Hand-edited manifests receive the same gate as Skill Builder output.
 *
 * @param {any} manifest
 * @returns {ManifestDiagnostic[]}
 */
export function validateDashboardWidgets(manifest) {
  const checked = validateSkillDashboardWidgets(manifest?.dashboardWidgets, manifest?.tools ?? []);
  if (checked.ok === true) return [];
  return [{
    severity: 'error',
    code: 'MV301',
    message: `Invalid dashboardWidgets declaration: ${checked.error}`,
    hint: 'Bind each declarative widget to an exact same-skill tool marked readOnly:true; dashboard widgets cannot run destructive tools or inject UI code.',
  }];
}
