 ...
 *   status: 'running' | 'ok' | 'error' | 'warning' | 'skipped' | 'late'
 *
 ...
 *   firedAt?: number, status: 'running'|'ok'|'error'|'warning'|'skipped'|'late', lateByMs?: number,
...
export async function appendTaskRun(userId, row) {
  ...
  const line = JSON.stringify({
    ...
    firedAt: firedAt ?? now,
    status,
    ...(status === 'running' ? { runtimeId: RUNTIME_ID } : {}),
  ...
  try {
    await withLock(p, () => {
      ...
      const existingIndex = runId ? kept.findIndex(existing => {
        try {
      ...
      if (existingIndex !== -1) {
        // Only an in-progress entry can be replaced. Duplicate finalization
        // and delayed starts must never overwrite a terminal result.
        if (JSON.parse(kept[existingIndex]).status !== 'running' || status === 'running') return;
        kept.splice(existingIndex, 1);
      }
      kept.push(line);
...
export function loadTaskRuns(userId, taskId = null) {
  ...
  try {
    const rows = fs.readFileSync(p, 'utf8')
      ...
      .map(({ runtimeId, ...row }) => row.status === 'running' && runtimeId !== RUNTIME_ID
        ? { ...row, status: 'interrupted', error: 'OE restarted before this run finished recording. Review any completed actions before running it again.' }
        : row);
