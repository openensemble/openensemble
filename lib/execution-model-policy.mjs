...
/**
 * Rechecks a saved provider/model pair against current account policy,
 * provider configuration, connection state, and the provider's model catalog.
 */
export async function validateExecutionModelAccess(userId, provider, model, { refreshCatalog = false } = {}) {
  ...
  if (Array.isArray(user.allowedModels) && !user.allowedModels.includes(model)) {
  ...
  let catalog;
  try {
    catalog = await loadCatalog(userId, provider, cfg, { refresh: refreshCatalog });
  } catch (error) {
