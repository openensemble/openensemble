 ...
 * in chat-dispatch/local-intent-fastpath.mjs, which dispatches to the exact
 * skill that owned the matched intent.
 */

import { loadConfig } from '../routes/_helpers.mjs';
import { listRoles, executeRoleToolForSkill, agentCanFastpathSkill } from '../roles.mjs';
  ...
  for (const manifest of roles) {
    ...
    for (const intent of intents) {
      ...
      out.push({
        ...
        // The owning tool definition is the safety floor. A hand-edited or
        // stale manifest must not turn a destructive tool into a no-confirm
        // local fast-path merely by setting intent.confirm=false.
        confirm: intent.confirm === true || tool.destructive === true,
...
export function runIntent(match, userId, agentId) {
  if (!match?.skillId || !match?.tool) throw new Error('runIntent requires an exact skill and tool');
  return executeRoleToolForSkill(
    match.skillId, match.tool, match.args || {}, userId, agentId,
  );
}
