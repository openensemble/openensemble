...
export function registerCoordinatedTask({
  ...
  const task = {
    ...
    model: clean(model, 300) || null,
    reasoningEffort: clean(reasoningEffort, 40) || null,
...
export function reserveCoordinatedTasks({
  ...
  const candidates = (Array.isArray(tasks) ? tasks : []).map(item => ({
    ...
    provider: clean(item?.provider, 100) || null,
    model: clean(item?.model, 300) || null,
    reasoningEffort: clean(item?.reasoningEffort, 40) || null,
    executionTargetExplicit: item?.executionTargetExplicit === true,
    claim: {
  ...
  for (const item of candidates) {
    registerCoordinatedTask({
      ...
      executionTargetExplicit: item.executionTargetExplicit,
    });
...
export function rootCoordinationSnapshot(rootTaskId) {
  ...
  const queued = tasks.filter(task =>
    task.status === 'running' && task.phase === 'queued').length;
  ...
  return {
    coordination: {
      ...
      active: tasks.filter(task =>
        activeStatuses.has(task.status) && task.phase !== 'queued').length,
      queued,
      completed,
      failed,
    },
...
export function coordinatedTasksSnapshot(rootTaskId) {
  ...
  return [...root.tasks.values()].map(task => {
    return {
      ...
      reasoningEffort: task.reasoningEffort || null,
      executionTargetExplicit: task.executionTargetExplicit === true,
      status: task.phase === 'queued' && task.status === 'running'
        ? 'queued'
        : task.status,
