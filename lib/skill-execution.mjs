 ...
 *   2. Manifest execution_hint (portable tier/effort)
 *   3. Structural score of the skill tools/manifest → tier/effort
 ...
 * carrying a skill's tools is not enough to activate its model. A concrete
 * provider/model selected on the agent is fixed: automatic manifest/structure
 * policy may tune reasoning effort, but it cannot replace that pair. Only an
 * explicit user skill pin can override a fixed durable-agent model. An
 * explicitly targeted spawned agent locks its provider/model more strongly;
 * skill profiles may still tune that child's reasoning effort. The optional
 * allowAutomaticModelSelection flag is reserved for a future explicit Auto
 * agent mode; no current named-agent flow enables it. Provider/model always
 * travel as one atomic pair. Effort is selected independently across every
 * eligible entry (including effort-only).
 */
 ...
 * @param {boolean} [args.allowAutomaticModelSelection]
 */
export function resolveSkillExecutionForTurn({
  ...
  const modelTargetLocked = baseAgent?._executionTargetLocked === true
    || baseAgent?._executionModelLocked === true;
  const effortTargetLocked = baseAgent?._executionEffortLocked === true;
  ...
  for (let selectedIndex = 0; selectedIndex < selected.length; selectedIndex++) {
    ...
    if (rawExecution != null) {
      ...
      if (!execution) {
        ...
        const requestedModelPair = Object.hasOwn(execution, 'provider') && Object.hasOwn(execution, 'model');
        if (requestedModelPair && modelTargetLocked) {
          consider({
            skillId, selectedIndex, source: 'user',
            provider: execution.provider, model: execution.model,
            eligible: false, reason: 'agent-execution-target-locked', effortRank: -1,
          });
        }
        const hasModelPair = requestedModelPair && !modelTargetLocked;
        ...
        userEligible = eligible && (hasModelPair || hasExplicitEffort);
        if (hasModelPair || hasExplicitEffort) {
          consider({
            skillId, selectedIndex, source: 'user',
    ...
    const autoHasModelPair = typeof auto.provider === 'string' && typeof auto.model === 'string'
      && auto.provider && auto.model;
    // Automatic skill hints are advisory for a named agent. They may adjust
    // effort, but the provider/model selected in Settings → Agents stays fixed.
    // A future explicit Auto agent mode may opt into catalog model selection by
    // passing allowAutomaticModelSelection=true.
    const hasModelPair = allowAutomaticModelSelection === true
      && autoHasModelPair
      && !modelTargetLocked;
    const hasExplicitEffort = typeof auto.reasoningEffort === 'string'
    ...
    consider({
      ...
      effortRank, eligible,
      reason: eligible
        ? (!hasModelPair && autoHasModelPair
          ? `${auto.reason || 'auto-eligible'};agent-model-fixed`
  ...
  if (prior && !effortWinner && !effortTargetLocked) {
    consider({
      skillId: '_task',
...
export async function buildAutoExecutionCandidates({
  ...
  for (const skillId of selected) {
    ...
    if (allowAutomaticModelSelection === true
        && tier && tier !== 'standard' && typeof resolveTier === 'function') {
      try {
...
export async function resolveTierToModelPair(userId, baseAgent, tier, allowedModels = null) {
  ...
  for (const id of [
    ...
    'ollama', 'ollama-local', 'lmstudio', 'perplexity',
  ...
  for (const provider of providers) {
    ...
    // A tier-matched baseline is already the preferred outcome. Inherit it
    // instead of crossing provider boundaries for an equivalent model.
    if (provider === baseAgent?.provider && model === baseAgent?.model) {
      if (classifyModelTier(model) === tier) return null;
      continue;
    }
...
export async function resolveValidatedSkillExecutionForTurn(args) {
  ...
  try {
    ...
    autoCandidates = await buildAutoExecutionCandidates({
      ...
      allowAutomaticModelSelection: args?.allowAutomaticModelSelection === true,
  ...
  for (const { execution } of userExecutions) {
    if (args?.baseAgent?._executionTargetLocked === true
        || args?.baseAgent?._executionModelLocked === true) continue;
  ...
  const resolution = resolveSkillExecutionForTurn({
    ...args,
    routeText,
    autoCandidates,
    modelAccess: pairs,
  ...
  // An explicit child effort was authorized as part of its spawn target and
  // is immutable for the turn. Skill arbitration deliberately cannot replace
  // it, so do not reinterpret or weaken it here.
  if (args?.baseAgent?._executionEffortLocked === true) return resolution;

  const finalProvider = resolution.effective?.provider;
  const finalModel = resolution.effective?.model;
  const hasFinalEffort = Object.hasOwn(resolution.effective || {}, 'reasoningEffort');
  const modelChanged = finalProvider !== resolution.baseline?.provider
    || finalModel !== resolution.baseline?.model;
  const effortWasArbitrated = resolution.sourceKinds?.reasoningEffort != null;
  if (!hasFinalEffort || (!modelChanged && !effortWasArbitrated)) return resolution;

  const pairKey = `${finalProvider || ''}\0${finalModel || ''}`;
  const access = pairs.get(pairKey)
    ?? await validateExecutionModelAccess(args?.userId, finalProvider, finalModel);
  if (!access?.ok) {
    return {
      ...resolution,
      effective: { ...resolution.baseline },
      sourceSkillIds: { model: null, reasoningEffort: null },
      sourceKinds: { model: null, reasoningEffort: null },
      contenders: [
        ...(resolution.contenders || []),
        {
          skillId: resolution.sourceSkillIds?.reasoningEffort ?? '_resolved',
          source: resolution.sourceKinds?.reasoningEffort ?? 'resolved',
          provider: finalProvider,
          model: finalModel,
          reasoningEffort: resolution.effective.reasoningEffort,
          eligible: false,
          reason: access?.reason || 'model-unavailable',
        },
      ],
      reason: access?.reason || 'model-unavailable',
      applied: false,
      reasoningEffortInherited: true,
    };
  }

  const effortOptions = await listExecutionReasoningEfforts(
    args?.userId,
    finalProvider,
    finalModel,
    { refreshCatalog: false },
  );
  const supported = new Set((Array.isArray(effortOptions) ? effortOptions : [])
    .map(option => option?.value)
    .filter(Boolean));
  const requestedEffort = resolution.effective.reasoningEffort;
  if (supported.has(requestedEffort)) return resolution;

  // Model and effort winners are independent by design. If their final
  // cross-product is unsupported, retain the authorized model winner but
  // inherit the prior effort only when that exact final model advertises it;
  // otherwise use the provider/model default (`auto`).
  const baselineHasEffort = Object.hasOwn(resolution.baseline || {}, 'reasoningEffort');
  const baselineEffort = resolution.baseline?.reasoningEffort;
  const safeEffort = baselineHasEffort && supported.has(baselineEffort)
    ? baselineEffort
    : 'auto';
  const safeEffective = { ...resolution.effective, reasoningEffort: safeEffort };
  const safeEffortChanged = !baselineHasEffort || safeEffort !== baselineEffort;
  const safelyApplied = modelChanged || safeEffortChanged;
  return {
    ...resolution,
    effective: safeEffective,
    sourceSkillIds: { ...resolution.sourceSkillIds, reasoningEffort: null },
    sourceKinds: { ...resolution.sourceKinds, reasoningEffort: null },
    contenders: [
      ...(resolution.contenders || []),
      {
        skillId: resolution.sourceSkillIds?.reasoningEffort ?? '_inherited',
        source: resolution.sourceKinds?.reasoningEffort ?? 'inherited',
        provider: finalProvider,
        model: finalModel,
        reasoningEffort: requestedEffort,
        eligible: false,
        reason: 'reasoning-effort-unsupported',
      },
    ],
    reason: safelyApplied && modelChanged
      ? resolution.reason
      : 'reasoning-effort-unsupported',
    applied: safelyApplied,
    reasoningEffortInherited: true,
  };
}
