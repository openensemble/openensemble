...
// Firmware carries a normalized manifest wake-word slug in `char slug[40]`.
// Keep the server-side contract in lockstep so an accepted/uploadable manifest
// can never be truncated into a different gate identity on the device.
export const MAX_WAKEWORD_SLUG_LENGTH = 39;
...
/**
 * Convert a manifest's human-readable `wake_word` phrase to the compact
 * identity sent by firmware to the verification gate.
 *
 * This is deliberately ASCII-only and mechanical:
 *   trim → lowercase → each run of non-[a-z0-9] becomes "_" → trim "_"
 *
 * Returning an empty string means the phrase cannot be represented safely.
 * Keep this byte-for-byte equivalent to firmware's manifest normalizer.
 */
export function normalizeWakewordSlug(wakeWord) {
  if (typeof wakeWord !== 'string') return '';
  if (!wakeWord || !/^[\x20-\x7e]+$/.test(wakeWord)) return '';
  const phrase = wakeWord.trim();
  if (!phrase) return '';
  return phrase
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '_')
...
/**
 * Validate the device-bound portion of a microWakeWord manifest.
 * Returns null when valid, otherwise a user-facing error string.
 */
export function validateWakewordManifest(manifestObj) {
  if (!manifestObj || typeof manifestObj !== 'object' || Array.isArray(manifestObj)) {
    return 'manifest must be a JSON object';
  }
  if (typeof manifestObj.wake_word !== 'string' || !manifestObj.wake_word.trim()) {
    return 'manifest.wake_word must be a non-empty string';
  }
  if (!/^[\x20-\x7e]+$/.test(manifestObj.wake_word)) {
    return 'manifest.wake_word must contain printable ASCII only';
  }
  const slug = normalizeWakewordSlug(manifestObj.wake_word);
  if (!slug) {
    return 'manifest.wake_word must contain at least one ASCII letter or digit';
  }
  if (slug.length > MAX_WAKEWORD_SLUG_LENGTH) {
    return `manifest.wake_word normalizes to a ${slug.length}-character slug; firmware accepts at most ${MAX_WAKEWORD_SLUG_LENGTH}`;
  }
  if (!manifestObj.micro || typeof manifestObj.micro !== 'object' || Array.isArray(manifestObj.micro)) {
...
export function validateUpload(tfliteBuffer, manifestObj) {
  ...
  return validateWakewordManifest(manifestObj);
}
...
export function listLibraryWakewords(userId) {
  ...
  for (const f of fs.readdirSync(dir)) {
    ...
    if (validateWakewordManifest(manifest)) continue;
    try { meta = JSON.parse(fs.readFileSync(metaPath, 'utf8')); } catch {}
    const stat = fs.statSync(tflitePath);
    out.push({
      id,
      wake_word: manifest.wake_word,
      wake_word_slug: normalizeWakewordSlug(manifest.wake_word),
...
export function readLibraryWakeword(userId, wwId) {
  ...
  const manifestJson = fs.readFileSync(paths.manifestPath, 'utf8');
  try {
    if (validateWakewordManifest(JSON.parse(manifestJson))) return null;
  } catch {
    return null;
  }
  return {
