...
import { isDefaultArgNoise, isDestructiveTool, isInfrastructureDefaultTool, isUserAuthoredDefaultValue } from './learning-safety.mjs';
import { evidenceRequirementForProposal } from './learning-policy.mjs';
import { getLearningUserText } from './learning-turn-context.mjs';
 ...
 *   { proposed: true, tool, arg, value, count }      — policy threshold tripped,
 *                                                      caller should emit
 ...
 */
export async function recordToolCall(userId, toolName, args, schemaDefaults = null, opts = {}) {
  if (!userId || !toolName || !args || typeof args !== 'object') return { proposed: false };
  if (!isPinnableTool(toolName)) return { proposed: false };
  const userText = typeof opts?.userText === 'string' ? opts.userText : getLearningUserText();
  if (!userText) return { proposed: false };
  ...
  for (const [argName, value] of Object.entries(args)) {
    ...
    if (!isPinnableArg(toolName, argName, value)) continue;
    if (!isUserAuthoredDefaultValue(value, userText)) continue;
    // Skip values that already equal the tool's declared schema default —
    // pinning them is a no-op ("make the default the default"). The model
    // often echoes a documented default verbatim, which would otherwise trip
    // a useless default_arg proposal. schemaDefaults = { argName: default }.
    if (schemaDefaults && Object.prototype.hasOwnProperty.call(schemaDefaults, argName)
        && valueEquals(schemaDefaults[argName], value)) continue;
    ...
    const minEvidence = evidenceRequirementForProposal(userId, {
      kind: 'default_arg',
      userId,
      tool: toolName,
      arg: argName,
    ...
    if (!proposal && arr.length === minEvidence) {
      ...
      proposal = { proposed: true, tool: toolName, arg: argName, value, count: minEvidence, userAuthored: true };
    }
  }
