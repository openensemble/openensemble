 ...
 */
export async function pinDefault(userId, toolName, argName, value, { recordUndo = false } = {}) {
  ...
  return withLock(defaultsPath(userId), () => {
    const all = loadDefaults(userId);
    const before = all[toolName]?.[argName] ?? null;
    if (!all[toolName]) all[toolName] = {};
    all[toolName][argName] = value;
    saveDefaults(userId, all);
    return { ok: true, ...(recordUndo ? { undo: recordProposalUndo(defaultsPath(userId), before, value) } : {}) };
  });
}

export async function undoDefaultPin(userId, toolName, argName, receipt) {
  return withLock(defaultsPath(userId), () => restoreProposalUndo(defaultsPath(userId), receipt, before => {
    const all = loadDefaults(userId);
    if (before === null) {
      delete all[toolName][argName];
      if (Object.keys(all[toolName]).length === 0) delete all[toolName];
    } else {
      all[toolName][argName] = before;
    }
    saveDefaults(userId, all);
  }));
}
