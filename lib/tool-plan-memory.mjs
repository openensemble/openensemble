...
const TURN_FAILURE_RE = /\b(?:stopped:\s*same tool call repeated|tooling limitation|tool limitation|no (?:web|search|internet|tool) access|(?:could|can|was|were|did|does|do|is|are|am)(?:n'?t| not)(?: able to)? (?:complete|extract|retrieve|get|access|compile|pull|finish|produce|load|reach|attach|send|deliver)|unable to (?:complete|extract|retrieve|get|access|compile|pull|finish|produce|load|reach|attach|send|deliver)|(?:could|can|was|were|did|does|do|is|are|am|have|had)(?:n'?t| not) have (?:access|web|search|the tools?|tool access|any (?:web|search) tools?)|(?:was ?n'?t|was not|were ?n'?t|were not) provided|not provided (?:with )?(?:the )?(?:tools|web|search|access)|failed to (?:complete|extract|retrieve|get|access|compile|pull|finish|produce|load|reach|attach|send|deliver)|(?:email\s+send|image\s+generation|delivery|operation|request|tool) failed|could not attach|refusing to send|nothing to send|handed (?:this|it|the (?:extraction|task|job|compilation|work)))/i;

const EMAIL_DELIVERY_TOOLS = new Set(['email_user', 'email_compose']);
const OUTBOUND_EMAIL_REQUEST_RE = /\b(?:e-?mail|mail)\s+(?:me|us|him|her|them|my|myself|this|that|it|the|an?|[\w.+-]+@[\w.-]+\.[a-z]{2,})\b|\b(?:send|deliver)\b[^.\n]{0,120}\b(?:an?\s+e-?mail|by\s+e-?mail|via\s+e-?mail|[\w.+-]+@[\w.-]+\.[a-z]{2,})\b/i;
const GENERATED_ARTIFACT_REQUEST_RE = /\b(?:generate|create|make|draw|paint|render|produce|record|export)\b[^.\n]{0,120}\b(?:image|picture|photo|illustration|artwork|portrait|logo|poster|file|document|report|video|audio)\b/i;
const EXPLICIT_ARTIFACT_EMAIL_RE = /\b(?:attach|attachment)\b|\b(?:e-?mail|send|deliver)\b[^.\n]{0,120}\b(?:image|picture|photo|illustration|artwork|portrait|logo|poster|file|document|report|video|audio)\b|\b(?:image|picture|photo|illustration|artwork|portrait|logo|poster|file|document|report|video|audio)\b[^.\n]{0,120}\b(?:e-?mail|send|deliver|attach)\b/i;
const ATTACHMENT_CONFIRMATION_RE = /\bEmail sent with\s+[1-9]\d*\s+attachment\(s\)/i;

function looksUnsuccessful(text) {
  return TURN_FAILURE_RE.test(String(text || ''));
}

function observedWorkflowFailure(toolEvents) {
  if (!Array.isArray(toolEvents)) return null;
  for (const event of toolEvents) {
    if (!event || typeof event !== 'object') continue;
    if (event.status && event.status !== 'done') return 'tool failure';
    if (looksUnsuccessful(event.text) || looksUnsuccessful(event.preview)) return 'tool failure';
  }
  return null;
}

function successfulEmailDeliveryEvents(toolEvents) {
  return (Array.isArray(toolEvents) ? toolEvents : []).filter(event =>
    EMAIL_DELIVERY_TOOLS.has(event?.name)
      && event?.status === 'done'
      && !looksUnsuccessful(event?.text));
}

function validateRequestedDelivery(phrase, toolEvents) {
  if (!Array.isArray(toolEvents)) return null;
  const instruction = routingInstructionClauses(phrase).join(' ') || instructionText(phrase);
  if (!OUTBOUND_EMAIL_REQUEST_RE.test(instruction)) return null;
  const sends = successfulEmailDeliveryEvents(toolEvents);
  if (!sends.length) return 'unmet email delivery';

  const requestedArtifact = GENERATED_ARTIFACT_REQUEST_RE.test(instruction)
    || EXPLICIT_ARTIFACT_EMAIL_RE.test(instruction);
  if (!requestedArtifact) return null;
  const delivered = sends.some(event => {
    const ids = event?.args?.attachment_doc_ids;
    return Array.isArray(ids)
      && ids.some(id => typeof id === 'string' && id.trim())
      && ATTACHMENT_CONFIRMATION_RE.test(String(event?.text || ''));
  });
  return delivered ? null : 'unmet artifact delivery';
}
...
export function learnToolPlanFromTurn(userId, {
  ...
  const observedFailure = observedWorkflowFailure(toolEvents);
  const unmetDelivery = validateRequestedDelivery(phrase, toolEvents);
  if (succeeded === false || escalated === true || looksUnsuccessful(outcomeText) || observedFailure || unmetDelivery) {
    if (observedFailure) return { ok: false, skipped: observedFailure };
    if (unmetDelivery) return { ok: false, skipped: unmetDelivery };
