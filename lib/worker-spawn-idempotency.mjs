...
function authorizedOutcomeKey(value) {
  if (value == null) return null;
  const text = String(value).trim();
  if (!text || text.length > 2_000 || text.includes('\0')) {
    throw new Error('A valid authorizedOutcomeId is required for worker admission.');
  }
  return sha256(`authorized-outcome\0${text}`);
}
...
export async function spawnWorkerIdempotently({
  ...
  // Exact multi-outcome admission passes a server-parsed outcome id. Persist a
  // digest rather than the raw id because it can include a short excerpt of the
  // user's request. Model-authored tool arguments never reach this field.
  const outcomeKey = authorizedOutcomeKey(authorizedOutcomeId);
  ...
  return withFileLock(lockPath, async () => {
    ...
    // A browser Retry may emit several authorized outcomes in a different
    // order, especially after the first provider turn launched only part of the
    // requested work. Match new keyed records by outcome identity, not ordinal,
    // or one completed slot can consume a different still-unstarted outcome.
    // Old records have no outcomeKey, so retain their ordinal/fingerprint
    // compatibility path. Unkeyed single-worker callers keep the old behavior.
    const existing = outcomeKey
      ? (record.jobs.find(job => job.outcomeKey === outcomeKey)
        || record.jobs.find(job => !job.outcomeKey && job.ordinal === ordinal)
        || record.jobs.find(job => !job.outcomeKey && job.fingerprint === fingerprint))
      : (record.jobs.find(job => job.ordinal === ordinal)
...
export const _internal = { normalizeText, jobFingerprint, authorizedOutcomeKey };
