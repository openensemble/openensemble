...
const SWEEP_DEFAULT_ARG_THRESHOLD = 2;     // runtime: 3
...
async function _sweepDefaultArgs(userId, emitted) {
  ...
  for (const [key, valueBuckets] of Object.entries(counts)) {
    ...
    for (const [vKey, timestamps] of Object.entries(valueBuckets)) {
      ...
      // Only count timestamps within the sweep window
      ...
      try {
        const p = await proposeDefaultArg({
          ...
          tool, arg, value, count: recent.length,
        });
        if (p) emitted.push(p);
...
function _patternFromMessage(msg) {
  const tokens = String(msg || '').toLowerCase().split(/\W+/).filter(t => t.length >= 4);
