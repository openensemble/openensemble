...
// Relaxed thresholds (vs. runtime defaults in their respective modules)
const SWEEP_DEFAULT_ARG_THRESHOLD = 3;     // match runtime; lower thresholds proposed too much noise
...
async function runSweep(userId, status) {
  const emitted = [];
  try {
    await _sweepDefaultArgs(userId, emitted);
...
async function _sweepDefaultArgs(userId, emitted) {
  const countsPath = path.join(USERS_DIR, userId, 'tool-arg-counts.json');
  const counts = readJsonSafe(countsPath) || {};
  const { proposeDefaultArg } = await import('./proposals.mjs');
  const cutoff = Date.now() - SWEEP_DELAY_MS;

  // Import the per-call arg blocklist + tool blocklist via the canonical
  // pinnability checks so sweep applies the same gates as runtime.
  const { _testIsPinnable } = await import('./tool-defaults.mjs');
  for (const [key, valueBuckets] of Object.entries(counts)) {
    if (!key.includes('.') || !valueBuckets || typeof valueBuckets !== 'object') continue;
    const dot = key.indexOf('.');
    const tool = key.slice(0, dot);
    const arg = key.slice(dot + 1);

    // Build the per-value count map within the sweep window
    const valueCounts = [];
    for (const [vKey, timestamps] of Object.entries(valueBuckets)) {
      if (!Array.isArray(timestamps)) continue;
      const recent = timestamps.filter(t => t > cutoff);
      if (recent.length < SWEEP_DEFAULT_ARG_THRESHOLD) continue;
      const value = _decodeVKey(vKey);
      if (value === undefined) continue;
      valueCounts.push({ value, count: recent.length });
    }
    if (!valueCounts.length) continue;

    // Same-arg conflict consolidation: only propose the SINGLE most-frequent
    // value, and only if it's clearly dominant (>= 50% of all occurrences).
    // Without this, the sweep dumps 5 competing proposals for timeout=
    // 30/60/120/300/15 — none of which is a sensible default.
    const totalCount = valueCounts.reduce((s, x) => s + x.count, 0);
    valueCounts.sort((a, b) => b.count - a.count);
    const winner = valueCounts[0];
    if (winner.count / totalCount < 0.5) continue;   // no dominant value → skip
    // Also apply the runtime per-call-arg blocklist + tool blocklist
    if (_testIsPinnable && !_testIsPinnable(tool, arg, winner.value)) continue;

    try {
      const p = await proposeDefaultArg({
        userId, agentId: 'week1-sweep',
        tool, arg, value: winner.value, count: winner.count,
      });
      if (p) emitted.push(p);
    } catch (e) { console.warn('[week1-sweep] default_arg propose failed:', e.message); }
  }
}
