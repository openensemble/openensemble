...
//             state, deviceIds[], firedDeviceIds[], ackedDeviceIds[],
//             awaitingFireAck, ackFallbackSentAt, startedAt }]
...
if (!ISOLATED_LAB_RUNTIME) _loadAllFromDisk();
...
export function broadcastAlarmStop(userId, { ids = null } = {}) {
  ...
  for (const a of list) {
    ...
    stoppedIds.add(a.id);
    for (const d of a.deviceIds || []) deviceIds.add(d);
  }
  let n = 0;
  for (const d of deviceIds) {
    if (sendToDevice(d, { type: 'alarm_stop', ids: ids ?? Array.from(stoppedIds) })) n++;
  }
  if (stoppedIds.size) {
    const keep = list.filter(a => !stoppedIds.has(a.id));
    for (const id of stoppedIds) _ttsCache.delete(id);
    if (keep.length) _activeAlarms.set(userId, keep);
    else _activeAlarms.delete(userId);
...
export function markAlarmFired(userId, alarmId, deviceId = null) {
  const list = _activeAlarms.get(userId);
  if (!list) return false;
  const entry = list.find(a => a.id === alarmId);
  if (!entry) return false;
  if (deviceId && (!entry.deviceIds?.includes(deviceId) || entry.ackedDeviceIds?.includes(deviceId))) return false;
  entry.firedDeviceIds = Array.isArray(entry.firedDeviceIds) ? entry.firedDeviceIds : [];
  if (deviceId && !entry.firedDeviceIds.includes(deviceId)) entry.firedDeviceIds.push(deviceId);
...
// Called by ws-handler.mjs when device reports user-dismissed. Acks are
// tracked per device so a multi-device alarm remains stoppable until every
// target has acknowledged or the server explicitly broadcasts alarm_stop.
export function markAlarmAcked(userId, alarmId, deviceId = null) {
  const list = _activeAlarms.get(userId);
  if (!list) return false;
  const idx = list.findIndex(a => a.id === alarmId);
  if (idx < 0) return false;
  const entry = list[idx];
  if (deviceId && !entry.deviceIds?.includes(deviceId)) return false;
  entry.ackedDeviceIds = Array.isArray(entry.ackedDeviceIds) ? entry.ackedDeviceIds : [];
  const targets = Array.isArray(entry.deviceIds) ? entry.deviceIds.filter(Boolean) : [];
  if (deviceId) {
    if (!entry.ackedDeviceIds.includes(deviceId)) entry.ackedDeviceIds.push(deviceId);
  } else {
    for (const d of targets) if (!entry.ackedDeviceIds.includes(d)) entry.ackedDeviceIds.push(d);
  }
  entry.ackedAtMs = Date.now();
  const allAcked = targets.length === 0 || targets.every(d => entry.ackedDeviceIds.includes(d));
  if (!allAcked) {
    _saveUserAlarms(userId);
    return true;
  }
...
export function registerAlarm({
  ...
  const now = Date.now();
  const id = requestedId || genAlarmId();
  const existing = (_activeAlarms.get(userId) || []).find(alarm => alarm.id === id);
  if (existing) {
    // An early device fire can create the occurrence before the server's
    // scheduled callback. Add any legacy targets without resetting receipts,
    // firing state, or the watchdog's completed acknowledgment.
    const targets = new Set(existing.deviceIds || []);
    for (const target of deviceIds || []) targets.add(target);
    const fired = new Set(existing.firedDeviceIds || []);
    for (const target of firedDeviceIds) {
      if (targets.has(target) && !existing.ackedDeviceIds?.includes(target)) fired.add(target);
    }
    if (targets.size !== (existing.deviceIds || []).length || fired.size !== (existing.firedDeviceIds || []).length) {
      existing.deviceIds = [...targets];
      existing.firedDeviceIds = [...fired];
      if (fired.size) {
        existing.state = 'firing';
        existing.awaitingFireAck = false;
        existing.firedAtMs ??= now;
      }
      _saveUserAlarms(userId);
    }
    return id;
  ...
  const knownFired = [...new Set(firedDeviceIds)].filter(target => deviceIds?.includes(target));
  const waiting = awaitingFireAck && !knownFired.length;
  ...
  const entry = {
    ...
    triggerAtMs: trigger,
    createdAt: now,
    firedAtMs: waiting ? null : now,
    ackedAtMs: null,
    state: waiting ? 'armed' : 'firing',
    awaitingFireAck: !!waiting,
    ackFallbackSentAt: null,
    deviceIds: Array.isArray(deviceIds) ? deviceIds.slice() : [],
    firedDeviceIds: knownFired,
    ackedDeviceIds: [],
...
function ensureWatchdog() {
  if (ISOLATED_LAB_RUNTIME) return;
  if (_watchdogTimer) return;
  _watchdogTimer = setInterval(_watchdogTick, WATCHDOG_TICK_MS);
  if (_watchdogTimer.unref) _watchdogTimer.unref();
}

// An alarm 2h past its trigger is dead weight whatever its state — a device
// that never acked (offline, dropped frame) used to leave the entry active
// FOREVER: hasActiveAlarms stayed true, so every voice "stop" kept
// broadcasting alarm_stop and the registry grew without bound.
...
function _watchdogTick() {
  ...
  for (const [userId, list] of _activeAlarms) {
    for (const entry of list) {
      ...
      const targets = Array.isArray(entry.deviceIds) ? entry.deviceIds.filter(Boolean) : [];
      if (targets.some(d => isDeviceOnline(d))) {
        continue;
      }
    ...
    const keep = list.filter(e => now - e.triggerAtMs < REAP_AFTER_MS);
    if (keep.length !== list.length) {
      console.log(`[alarms] reaped ${list.length - keep.length} stale alarm(s) for ${userId}`);
      for (const e of list) if (!keep.includes(e)) _ttsCache.delete(e.id);
      if (keep.length) _activeAlarms.set(userId, keep);
      else _activeAlarms.delete(userId);
      _saveUserAlarms(userId);
    }
...
async function sendDeviceOfflineFallback(userId, entry) {
  ...
  const res = await sendUserNotification(userId, {
    ...
    // entry.id survives restart in alarms.json.  If state persistence fails
    // around provider acceptance, the same fallback cannot send twice.
    idempotencyScope: `alarm-ack-fallback:${entry.id}`,
  });
