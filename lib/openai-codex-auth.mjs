...
/**
 * Force a token refresh even when the local expires_at is still in the future.
 * Used by the provider when the upstream returns 401 token_invalidated — that
 * means ChatGPT revoked the session server-side (user logged out elsewhere,
 * security revoke, plan change) while our local clock still says the access
 * token is valid. ensureFreshToken won't refresh in that case because it only
 * checks local expiry. If the refresh_token itself is also dead, this throws
 * and the caller should surface a "please reconnect" error to the user.
 */
export async function forceRefreshToken(userId) {
  ...
  return withLock(tp, async () => {
    const current = readToken(userId);
    if (!current?.refresh_token) {
      throw new Error('OpenAI Codex token revoked and no refresh_token available; please reconnect.');
    }
    const refreshed = await refreshTokens(current.refresh_token);
    if (!refreshed.account_id && current.account_id) refreshed.account_id = current.account_id;
    writeToken(userId, refreshed);
...
/**
 * Proactively refresh every connected user's Codex token that's near expiry,
 * regardless of whether that user is active. The provider only refreshes a
 * token when that user makes an API call (ensureFreshToken), so a ChatGPT
 * account nobody chats as would let its access token — and eventually its
 * refresh_token — lapse and get revoked by ChatGPT for inactivity, forcing a
 * manual reconnect. This keep-alive (scheduled from server.mjs at boot + daily)
 * prevents that by rolling idle tokens before they expire.
 *
 * @param {{ withinMs?: number }} [opts] refresh tokens expiring within this
 *   window (default 48h — comfortably inside the ~10-day token lifetime, so
 *   every token gets rolled with margin between daily runs).
 * @returns {Promise<{checked:number, refreshed:number, failed:number, skipped:number}>}
 */
export async function refreshExpiringCodexTokens({ withinMs = 48 * 60 * 60 * 1000 } = {}) {
  ...
  for (const userId of dirs) {
    ...
    const current = readToken(userId);
    if (!current?.access_token || !current?.refresh_token) continue; // not connected / not refreshable
    summary.checked++;
    if ((current.expires_at ?? 0) - now > withinMs) { summary.skipped++; continue; } // still fresh
    try { await forceRefreshToken(userId); summary.refreshed++; }
    catch (e) { summary.failed++; console.warn(`[openai-oauth] keep-alive refresh failed for ${userId}: ${e.message}`); }
  }
  return summary;
}
