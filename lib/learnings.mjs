...
import { evaluateLearningProposal, listLearningPolicy, setLearningKindPolicy, clearLearningKindPolicy, setLearningPolicy, clearLearningPolicy } from './learning-policy.mjs';
...
function listLearningReviewQueue(userId) {
  const p = path.join(USERS_DIR, userId, 'proposals.json');
  const data = readJsonSafe(p);
  const arr = data?.proposals;
  if (!Array.isArray(arr)) return [];
  return arr
    ...
    .slice(0, 100)
    .map(r => {
      const policy = r.policy || evaluateLearningProposal(r);
      return {
        id: r.id,
        kind: r.kind,
        status: r.status,
        ...
        decision: policy.decision || policy.reason || null,
        summary: summarizeProposal(r),
        agentId: r.agentId || null,
      };
    });
...
export function readLearnings(userId) {
  ...
  return {
    ...
    reviewQueue:    listLearningReviewQueue(userId),
    outcomesByKind: summarizeByKind(userId),
    salienceStatus: getAllStatuses(userId),
    learningPolicy: listLearningPolicy(userId),
...
export async function applyLearningKindPolicy(userId, kind, patch) {
  return setLearningKindPolicy(userId, kind, patch);
}

export async function revokeLearningKindPolicy(userId, kind) {
  return clearLearningKindPolicy(userId, kind);
}

export async function applyLearningPolicy(userId, patch) {
  return setLearningPolicy(userId, patch);
}

export async function revokeLearningPolicy(userId) {
  return clearLearningPolicy(userId);
}
