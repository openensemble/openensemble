  ...
  for (const user of users) {
    ...
    for (const dev of devices) {
      ...
      if (isDeviceOnline(dev.id)) {
        if (st.alerted) {
          ...
          await notify(user.id, {
            ...
            idempotencyScope: `voice-device-recovered:${dev.id}:${st.offlineSince}`,
          });
        }
        st.seenOnline = true;
      ...
      await notify(user.id, {
        ...
        idempotencyScope: `voice-device-offline:${dev.id}:${st.offlineSince}`,
      });
    }
  }
}

async function notify(userId, { subject, body, idempotencyScope }) {
  try {
    const res = await sendUserNotification(userId, { subject, body, idempotencyScope });
