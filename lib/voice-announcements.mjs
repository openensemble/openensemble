...
/**
 * Non-consuming view of the queue, for surfaces that just want to display
 * "what's pending" without affecting delivery order (Android TV
 * dashboard_get — see lib/tv-dashboard.mjs). Returns a fresh array (oldest
 * first, same order nextVoiceAnnouncement() would drain them), each entry
 * as `{text, at}` (`at` = ISO timestamp) — never the live queue reference,
 * so a caller can't accidentally mutate delivery state.
 */
export function peekVoiceAnnouncements(deviceId) {
  const q = prune(deviceId);
  if (!q) return [];
  return q.map(e => ({ text: e.text, at: new Date(e.ts).toISOString() }));
}
