...
const DIAGNOSTICS_DRAIN_GRACE_MS = 1_000;

let diagnosticsEnding = false;
const toErr = (...args) => {
  if (diagnosticsEnding) return;
  try {
...
function drainDiagnostics() {
  ...
  return new Promise((resolve) => {
    ...
    const finish = () => {
      ...
      finished = true;
      clearTimeout(timer);
      diagnostics.off('error', finish);
      resolve(undefined);
    };
    const timer = setTimeout(() => {
      try { diagnostics.destroy(); } catch {}
      finish();
    }, DIAGNOSTICS_DRAIN_GRACE_MS);
    diagnostics.once('error', finish);
    try { diagnostics.end(finish); }
    catch { finish(); }
  });
}
async function finish(message) {
  ...
  terminalSent = true;
  // stderr and stdout are independent pipes. Seal and flush diagnostics before
  // exposing the terminal stdout frame so the parent can safely treat stderr
  // EOF as durable. The deadline prevents a blocked fd from hanging the host.
...
function makeCtx(userId, agentId, skillDir, runnerOwnsSandbox, readOnlyCtx = false) {
  const ctx = {
    userId,
    agentId,
  ...
  if (readOnlyCtx) return ctx;

  ctx.credentials.get = (id) => rpc('credentials.get', [id]);
  ctx.credentials.set = (id, value, meta) => rpc('credentials.set', [id, value, meta]);
  ctx.credentials.delete = (id) => rpc('credentials.delete', [id]);
  ctx.watch = (opts) => rpc('watch', [opts]);
  ctx.unwatch = (id) => rpc('unwatch', [id]);
  ctx.proposeMonitor = (opts) => rpc('proposeMonitor', [opts]);
  ...
  ctx.ensureRuntime = (spec) => rpc('runtime.ensureRuntime', [spec]);
  ctx.runSandboxed = runnerOwnsSandbox
...
async function runJob(job) {
  ...
  try {
    const output = await execute(
      ...
      makeCtx(userId, agentId, skillDir, runnerOwnsSandbox, job?.readOnlyCtx === true),
    );
