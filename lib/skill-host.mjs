...
async function runJob(job) {
  ...
  if (mode === 'exported_function') {
    ...
    if (!/^[A-Za-z_$][\w$]*$/.test(functionName)) {
      write({ t: 'result', ok: false, error: 'invalid exported function name' });
      process.exit(0);
    }
    const fn = mod[functionName];
    if (typeof fn !== 'function') {
      write({ t: 'result', ok: false, error: `skill has no exported function "${functionName}"` });
      process.exit(0);
    }
    try {
      const result = await fn(userId);
      write({ t: 'result', ok: true, result: result ?? null });
    } catch (e) {
      write({ t: 'result', ok: false, error: e?.message || String(e) });
    }
    process.exit(0);
