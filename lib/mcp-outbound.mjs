...
async function loadRoster(userId, boundAgentId = null) {
  ...
  const effectiveBoundAgentId = boundAgentId
    ...
    : null;
  return { agents, coordinatorId, effectiveBoundAgentId };
}
 ...
 * @param {{agents: any[], coordinatorId: string|null, effectiveBoundAgentId?: string|null}} roster
 */
function buildToolDefs(tokenRec, roster) {
  ...
  if (scopes.has('chat')) {
    ...
    if (!invalidBinding && (!bound || bound.id !== coordinator?.id)) {
      defs.push({
        name: 'ask_agent',
        description:
