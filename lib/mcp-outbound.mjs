...
async function runIsolatedTurnLeased({
  ...
  let content = '';
  try {
    await scheduledContext.run({ scheduledNote: systemNote }, () => runWithTurnContext(
      { signal: ac.signal },
    ...
    if (!invalidBinding && coordinator && (!bound || bound.id === coordinator.id)) {
      defs.push({
        ...
        description:
          ...
          `context and memory, use its tools, and ${namedDelegationEnabled ? 'delegate to specialist agents' : 'start background workers for long or parallel work'}. ` +
