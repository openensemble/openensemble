 ...
 *     Agent turns are first-party-read-only by default. A PAT may add exact
 *     agent tool names; that capability is enforced transitively at dispatch.
...
import { runWithMcpToolPolicy } from './mcp-tool-policy.mjs';
...
const PREAUTH_LIMIT_PER_MIN = 60;           // per-IP, before token validation
const _preAuthWindow = new Map();           // ip → number[] (ms stamps)
...
async function loadRoster(userId, boundAgentId = null) {
  const {
    getAgentsForUser,
    getUserCoordinatorAgentId,
  } = await import('../routes/_helpers.mjs');
  const { getOrchestrationPolicy } = await import('./orchestration-policy.mjs');
  const agents = getAgentsForUser(userId);
  const orchestrationMode = getOrchestrationPolicy(userId).mode;
  const coordinatorId = getUserCoordinatorAgentId(userId)
    ?? agents.find(a => a.skillCategory === 'coordinator')?.id
    ?? null;
  // Agent-bound access tokens are a capability boundary, not a saved runtime
  // preference. A parked/deleted/foreign binding must fail closed rather than
  // silently inheriting whichever single-mode primary is currently active.
  const effectiveBoundAgentId = boundAgentId
    ? (agents.find(agent => agent.id === boundAgentId)?.id ?? null)
    : null;
  return { agents, coordinatorId, effectiveBoundAgentId, orchestrationMode };
}

async function withOutboundTopologyLease(userId, clientSignal, label, fn) {
  const topology = await import('../chat-dispatch/slot-registry.mjs');
  const ambient = topology.getCurrentUserTopologyLease();
  if (ambient?.userId === userId && !ambient.released) return fn();

  let lease = null;
  for (let attempt = 0; attempt < 80 && !lease; attempt++) {
    if (clientSignal?.aborted) throw new Error('Client disconnected before the request started');
    lease = topology.tryAcquireUserTurnLease(userId, { label });
    if (!lease) await new Promise(resolve => setTimeout(resolve, 25));
  }
  if (!lease) throw new Error('Account setup is changing; retry this MCP call.');
  try {
 ...
 */
async function runIsolatedTurn(args) {
  const { userId, clientSignal } = args;
  return withOutboundTopologyLease(
    userId,
    clientSignal,
    'external-mcp-turn',
    () => runIsolatedTurnLeased(args),
  );
}

async function runIsolatedTurnLeased({
  agent,
  ...
  // The roster used to build this request may have been loaded immediately
  // before a mode transition. Re-resolve under the account read lease so the
  // actual turn can neither start on a parked agent nor race agent deletion.
  const runtimeAgent = strictAgentId
    ? getAgentsForUser(userId).find(candidate => candidate.id === strictAgentId) ?? null
    : resolveRuntimeAgentForUser(userId, requestedAgentId || agent?.id);
  if (!runtimeAgent) throw new Error(`Agent "${agent?.id || ''}" is no longer available for this account.`);
  agent = runtimeAgent;

  const userName = getUser(userId)?.name ?? 'the user';
  const policyBoundAgentId = boundAgentId || strictAgentId || null;
  const systemNote =
    ...
    `Agent tools are server-restricted to first-party read-only tools plus the PAT's explicit allowlist` +
    `${toolAllowlist.length ? ` (${toolAllowlist.join(', ')})` : ' (no additional tools)'}. ` +
    `${policyBoundAgentId ? `This PAT is bound to agent "${policyBoundAgentId}"; internal delegation outside that binding is blocked. ` : ''}` +
    `If the instruction explicitly requests an allowed action, perform it now — do not show drafts or wait for approval, no one can answer. ` +
  ...
  const taskId = `mcp_turn_${randomUUID()}`;
  const taskContext = {
    taskId,
    watcherId: null,
    userId,
  ...
  const timer = setTimeout(() => {
    timedOut = true;
    ac.abort(new Error(`Agent turn exceeded ${TURN_TIMEOUT_MS / 1000}s`));
  }, TURN_TIMEOUT_MS);
  const onClientAbort = () => ac.abort(new Error('Client disconnected; agent turn aborted'));
  ...
  let content = '';
  try {
    await runWithMcpToolPolicy(
      { tokenId, userId, boundAgentId: policyBoundAgentId, toolAllowlist },
      () => scheduledContext.run({ scheduledNote: systemNote }, () => runWithTurnContext(
        { signal: ac.signal },
        () => runInTaskContext(taskContext, async () => {
          for await (const event of iterateUntilAbort(
            streamChat(scopedAgent, message, ac.signal, null, userId, null, systemNote, true, null, { isolatedTaskRun: true }),
            ac.signal,
            'Agent turn aborted',
          )) {
 ...
 * @param {{agents: any[], coordinatorId: string|null, effectiveBoundAgentId?: string|null, orchestrationMode?: 'ensemble'|'single'}} roster
 */
function buildToolDefs(tokenRec, roster) {
  const scopes = new Set(tokenRec.scopes);
  // This must follow the stored policy rather than roster cardinality. An
  // ensemble account with one agent still exposes named ask_agent, while a
  // single-mode account never does even if a caller retained an old schema.
  const namedDelegationEnabled = roster.orchestrationMode !== 'single';
  const effectiveBoundId = tokenRec.agentId
    ? (Object.prototype.hasOwnProperty.call(roster, 'effectiveBoundAgentId')
        ? roster.effectiveBoundAgentId
        : (roster.agents.some(a => a.id === tokenRec.agentId) ? tokenRec.agentId : null))
    : null;
  const bound = effectiveBoundId
    ? roster.agents.find(a => a.id === effectiveBoundId) ?? null
    : null;
  const invalidBinding = Boolean(tokenRec.agentId && !bound);
  const askable = bound ? [bound] : roster.agents;
  const defs = [];

  if (scopes.has('chat')) {
    const coordinator = roster.agents.find(a => a.id === roster.coordinatorId);
    if (!invalidBinding && coordinator && (!bound || bound.id === coordinator.id)) {
      const orchestrationDescription = bound
        ? 'work only within this agent binding'
        : (namedDelegationEnabled ? 'delegate to specialist agents' : 'start background workers for long or parallel work');
      defs.push({
        ...
        description:
          ...
          `context and memory, use read-only or token-approved tools, and ${orchestrationDescription}. ` +
          `Each call is an isolated turn (no memory of prior calls) ` +
    ...
    if (namedDelegationEnabled && !invalidBinding && (!bound || bound.id !== coordinator?.id)) {
      defs.push({
        name: 'ask_agent',
        description:
...
// ── Tool dispatch ────────────────────────────────────────────────────────────
async function callTool({ tokenRec, name, args, clientSignal, onProgress }) {
  return withOutboundTopologyLease(
    tokenRec.userId,
    clientSignal,
    `external-mcp:${name || 'tool'}`,
    async () => {
      // Load and authorize against one live roster while the read lease is
      // held. The same snapshot drives memory scope, token binding, and the
      // actual agent turn, so a concurrent mode switch cannot reinterpret an
      // already-admitted request.
      const roster = await loadRoster(tokenRec.userId, tokenRec.agentId);
      return callToolLeased({ tokenRec, roster, name, args, clientSignal, onProgress });
    },
  );
}

async function callToolLeased({ tokenRec, roster, name, args, clientSignal, onProgress }) {
  ...
  const bound = tokenRec.agentId
    ? (Object.prototype.hasOwnProperty.call(roster, 'effectiveBoundAgentId')
        ? roster.effectiveBoundAgentId
        : (roster.agents.some(agent => agent.id === tokenRec.agentId) ? tokenRec.agentId : null))
    : null;

  // Schema omission is not an authorization boundary. External MCP clients
  // can cache a tools/list response across an ensemble -> single switch and
  // replay ask_agent directly, so reject that stale call before resolving a
  // target or starting an isolated agent turn.
  if (name === 'ask_agent' && roster.orchestrationMode === 'single') {
    requireScope('chat');
    throw new Error('ask_agent is unavailable while this account is in single-agent mode. Use ask_coordinator instead.');
  }

  if (name === 'list_agents') {
    requireScope('chat');
    const list = (tokenRec.agentId
      ? roster.agents.filter(a => bound && a.id === bound)
      : roster.agents)
      .map(a => ({
  ...
  if (name === 'ask_coordinator' || name === 'ask_agent') {
    requireScope('chat');
    if (tokenRec.agentId && !bound) throw new Error(`This access token's bound agent is no longer available.`);
    ...
    return runIsolatedTurn({
      agent,
      // Re-resolve the durable binding under the topology lease. If the mode
      // changed since this request loaded its roster, the turn follows the
      // binding in ensemble or the primary projection in single mode.
      requestedAgentId: tokenRec.agentId || targetId,
      strictAgentId: tokenRec.agentId || null,
      userId,
      message,
      clientSignal,
      onProgress,
      tokenId: tokenRec.id,
      boundAgentId: tokenRec.agentId,
      toolAllowlist: tokenRec.toolAllowlist ?? [],
    });
  }
...
function buildServer(tokenRec, clientMeta) {
  const server = new Server(
    { name: 'openensemble', version: '1.0.0' },
    {
      ...
      instructions:
        ...
        'agents handle within this token\'s agent-tool grants; agent tools are read-only unless an exact additional tool was granted when the token was created. ' +
        'Use recall_memory before asking the user something OE may already know. Every call is scoped to the token owner\'s account.',
    },
  );

  server.setRequestHandler(ListToolsRequestSchema, async (_request, extra) =>
    withOutboundTopologyLease(
      tokenRec.userId,
      extra?.signal,
      'external-mcp:list-tools',
      async () => ({
        tools: buildToolDefs(
          tokenRec,
          await loadRoster(tokenRec.userId, tokenRec.agentId),
        ),
      }),
    ));
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
    ...
    try {
      resultText = await callTool({ tokenRec, name, args, clientSignal: extra?.signal, onProgress });
    } catch (e) {
      ok = false;
...
export const _forTests = { buildToolDefs, callTool, loadRoster, runIsolatedTurn };
...
export async function handle(req, res) {
  ...
  // Pre-auth rate limit: /mcp sits outside the /api edge limiter, so an
  // unauthenticated caller could hammer token validation (and enumerate the
  // 8-hex id space) unmetered. Per-IP sliding window, checked BEFORE any
  // token work; authenticated traffic is governed by the per-token limits.
  {
    const ip = req.socket?.remoteAddress || 'unknown';
    const now = Date.now();
    const stamps = (_preAuthWindow.get(ip) ?? []).filter(t => now - t < 60_000);
    if (stamps.length >= PREAUTH_LIMIT_PER_MIN) {
      _preAuthWindow.set(ip, stamps);
      jsonError(res, 429, 'Too many requests.');
      return true;
    }
    stamps.push(now);
    _preAuthWindow.set(ip, stamps);
    if (_preAuthWindow.size > 4096) _preAuthWindow.clear(); // bound the map
  }
  ...
  const server = buildServer(tokenRec, clientMeta);
