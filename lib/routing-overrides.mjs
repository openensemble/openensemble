...
function saveOverrides(userId, arr) {
  const p = overridesPath(userId);
  fs.mkdirSync(path.dirname(p), { recursive: true });
  atomicWriteSync(p, JSON.stringify(arr, null, 2));
}
...
export async function addOverride(userId, { pattern, forcedAgent, mode = 'contains', addedBy = 'manual', examples = [] }, { recordUndo = false } = {}) {
  ...
  return withLock(overridesPath(userId), () => {
    const all = loadOverrides(userId);
    all.push(entry);
    saveOverrides(userId, all);
    return { ok: true, id, entry,
      ...(recordUndo ? { undo: recordProposalUndo(overridesPath(userId), null, entry) } : {}) };
  });
}

export async function undoOverride(userId, id, receipt) {
  return withLock(overridesPath(userId), () => restoreProposalUndo(overridesPath(userId), receipt, () => {
    saveOverrides(userId, loadOverrides(userId).filter(o => o.id !== id));
  }));
}
