...
describe('persisted runtime agent references', () => {
  it('redirects only owned parked agents in single mode and restores the exact target in ensemble', async () => {
    ...
    expect(resolveRuntimeAgentForUser(id, 'coordinator')?.id).toBe(primary);
    expect(resolveRuntimeAgentForUser(id, `${id}_coordinator`)?.id).toBe(primary);
    expect(resolveRuntimeAgentForUser(id, 'stale_runtime_agent')).toBeNull();
    expect(resolveRuntimeAgentForUser(id, foreign)).toBeNull();

    await setOrchestrationPolicy(id, { mode: 'single', primaryAgentId: parked });
    expect(resolveRuntimeAgentForUser(id, primary)?.id).toBe(parked);
    expect(resolveRuntimeAgentForUser(id, `${id}_${primary}`)?.id).toBe(parked);
    expect(resolveRuntimeAgentForUser(id, 'coordinator')?.id).toBe(parked);
    expect(resolveRuntimeAgentForUser(id, `${id}_coordinator`)?.id).toBe(parked);
    expect(resolveRuntimeAgentForUser(id, 'stale_runtime_agent')).toBeNull();
    expect(resolveRuntimeAgentForUser(id, foreign)).toBeNull();
