/**
 * Per-user voice references for F5-TTS zero-shot cloning.
 *
 * Storage: users/<userId>/voice-refs/<refId>.wav + .json
 *   - <refId>.wav  — reference audio (~10s of the target voice, mono WAV)
 *   - <refId>.json — { label, transcript, uploaded_at, duration_s, size_bytes }
 *
 * F5-TTS clones from a reference WAV plus the exact transcript of what's
 * said in that clip. Quality is best with 8-15s of clean speech and an
 * accurate transcript; longer or noisier samples can hurt.
 *
 * The slot_assignments[N].ttsVoice value, when ttsProvider='f5-tts', is
...
export function validateUpload(wavBuffer, label, transcript) {
  ...
  if (typeof transcript !== 'string' || !transcript.trim()) {
    return 'transcript is required (exact text of what is said in the audio)';
  }
...
export function deleteVoiceRef(userId, refId) {
  ...
  if (!r) return false;
  const dir = refsDir(userId);
  for (const f of [`${refId}.wav`, `${refId}.json`]) {
