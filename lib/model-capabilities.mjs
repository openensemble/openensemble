...
export function nativeWebSearch(provider, modelId) {
  ...
  switch (provider) {
    ...
    // xAI: NO native search on the /chat/completions endpoint OE's openai-compat
    // path uses. Verified live 2026-06-28 against grok-4.3:
    //   • `{type:'web_search'}`                       → 422 (unknown variant;
    //     the endpoint only accepts `function` or `live_search`)
    //   • `{type:'live_search', sources:[{type:'web'}]}` (a fully-formed
    //     live_search tool)                           → 410 "Live search is
    //     deprecated. Please switch to the Agent Tools API."
    // The documented `{type:'web_search'}` works only on xAI's Responses API
    // (`POST /v1/responses`, same shape as OpenAI's — live-confirmed it returns
    // a grounded dated answer), NOT on /chat/completions. Enabling grok native
    // search therefore needs a Responses adapter (generalize openai-responses.mjs
    // to xAI's base URL + bearer key). Until that exists, keep grok on Brave.
...
export function resolveNativeWebSearch(provider, modelId, tools, { disabled = false } = {}) {
  ...
  if (!desc) return noop;
  if (desc.kind === 'ollama-tool') return { ...noop, kind: 'ollama-tool' };
