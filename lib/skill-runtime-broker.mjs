 ...
 * allowExecution=false is the stronger boundary used by managed personalization
 * ticks: neither an existing mutable binary nor a newly provisioned one may run.
 */
import path from 'path';
import { getUserFilesDir, userSkillsDir } from './paths.mjs';
import { skillDeclaresNetwork } from './skill-net-policy.mjs';
 ...
 * @param {{allowPrompt?: boolean, allowExecution?: boolean}} [opts]
 */
export function buildRuntimeBroker(userId, skillId, {
  allowPrompt = true,
  allowExecution = true,
} = {}) {
  ...
  // Egress capability is the skill's, not the sub-binary's — a network-denied skill
  // can't smuggle egress through yt-dlp. (Redundant with the outer jail's --unshare-net,
  // but makes the clamp explicit and correct if the seams ever diverge.)
  const netAllowed = skillDeclaresNetwork(userId, skillId);

  return {
    ensureRuntime: async (spec = {}) => {
      if (!allowExecution) {
        throw new Error('ensureRuntime: external runtimes are disabled for this managed execution');
      }
    ...
    runSandboxed: async (bin, binArgs = [], opts = {}) => {
      if (!allowExecution) {
        throw new Error('runSandboxed: external runtimes are disabled for this managed execution');
      }
