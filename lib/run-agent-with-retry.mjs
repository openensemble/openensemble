 ...
 *
 * Two call sites need identical-shaped retry logic:
 *   • scheduler.mjs runTask        — scheduled tasks (cron / one-shot)
 *   • lib/proposals.mjs runAcceptedAgent — friction-as-proposer accepts
 *   • scheduler/watchers.mjs executeOnFire — could adopt this too if we
 *     later need retry on watcher-fired agent runs (currently single-shot).
 *
  ...
  const runAttempt = async (attempt) => {
    ...
    try {
      await scheduledContext.run({ scheduledNote: systemNote }, async () => {
        ...
        for await (const event of streamChat(scopedAgent, userText, ac.signal, null, userId, null, systemNote)) {
          if (event.type === 'error') {
