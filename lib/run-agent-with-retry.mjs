...
export async function runAgentWithRetry(opts) {
  ...
  const runAttempt = async (attempt) => {
    ...
    assistantContent = '';
    try {
      await scheduledContext.run({ scheduledNote: systemNote, originTaskId, originTaskOwnerId, originTaskAgent, runId: originTaskRunId }, async () => {
        ...
        let stallTimer = null;
        const armStall = () => { clearTimeout(stallTimer); stallTimer = setTimeout(() => { try { ac.abort(); } catch {} }, STALL_MS); };
        const rememberedPlan = matchToolPlan(userId, { agentId: scopedAgent.id, phrase: userText });
        try {
          armStall();
          for await (const event of streamChat(scopedAgent, userText, ac.signal, null, userId, null, systemNote, silent, null, { toolPlan: rememberedPlan, isolatedTaskRun })) {
            armStall();
