...
export function createHealthTracker(deps = {}) {
  ...
  function stateFor(deviceId, userId) {
    ...
    if (!st) {
      st = {
        userId,
        lastHbAt: null, tick: null, rssi: null, capSps: null, pcmDrop: null,
  ...
  function onHeartbeat(dev, st, { tick, rssi, capSps, pcmDrop }) {
    st.lastHbAt = d.now();
    if (tick !== null) st.tick = tick;
    if (rssi !== null) st.rssi = rssi;
    // Cumulative playback-samples-dropped counter (fw >= 0.2.62). Tracked
    // for the health snapshot / future device card; not an alert signal —
    // a rare skip is audible but self-explains, unlike a deaf mic.
    if (pcmDrop !== null) st.pcmDrop = pcmDrop;
    ...
    recordLine(ip, text) {
      try {
        ...
        if (text.startsWith('[hb] alive')) {
          ...
          const pcmDrop = (m = text.match(/\bpcm_drop=(\d+)/)) ? Number(m[1]) : null;
          onHeartbeat(dev, stateFor(dev.deviceId, dev.userId), { tick, rssi, capSps, pcmDrop });
        } else if (text.startsWith('[hb] wake')) {
          const dev = resolveDevice(ip);
          if (!dev) return;
          const st = stateFor(dev.deviceId, dev.userId);
          const value = name => { const match = text.match(new RegExp(`\\b${name}=(-?\\d+)`)); return match ? Number(match[1]) : null; };
          const peaks = [...text.matchAll(/\bs([0-5])=(\d+)/g)].map(match => ({ slot: Number(match[1]), score: Math.min(255, Number(match[2])) / 255 }));
          st.wakeSamples ||= [];
          st.wakeSamples.push({ ts: d.now(), userId: dev.userId, audioLevel: value('audio_lvl'), featureMax: value('feat_max'),
            agc: text.match(/\bagc=(ok|pending|capped)\b/)?.[1] || null, rmsAtGainFreeze: value('rms'), starved: value('starved'), peaks });
          st.wakeSamples = st.wakeSamples.filter(sample => sample.userId === dev.userId && d.now() - sample.ts <= 10 * 60_000).slice(-60);
