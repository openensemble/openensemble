...
import { ensureFirmwareAssets } from './firmware-assets.mjs';
...
/**
 * Re-fetch any device firmware image the pull removed or changed.
 *
 * Firmware is not tracked in git (see lib/firmware-assets.mjs), so a pull that
 * crosses the commit which untracked it DELETES those images from the working
 * tree. Without this step every existing install would silently lose OTA and
 * USB flashing on its next self-update. It also picks up a firmware publish,
 * which from git's side is only a manifest.json edit.
 *
 * Best-effort and never throws: OE boots and serves fine without firmware, and
 * an admin gets an actionable error at the point of use. Runs before the
 * restart so the new process comes up with the images already in place.
 */
async function refreshFirmwareAssets() {
  try {
    const r = await ensureFirmwareAssets({ logger: (m) => log.info('update', m, {}) });
    if (r.fetched.length) log.info('update', 'firmware re-fetched after pull', { count: r.fetched.length });
    if (!r.ok) log.warn('update', 'firmware missing after pull', { failures: r.failures });
  } catch (e) {
    log.warn('update', 'firmware refresh failed', { error: e.message });
  }
}
...
export async function forceApplyUpdate({
  ...
  try {
    ...
    // Must run on every pull, not just when package.json changed — the pull
    // itself is what removes untracked firmware from the working tree.
    await refreshFirmwareAssets();
