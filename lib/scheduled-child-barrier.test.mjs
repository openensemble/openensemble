...
describe('scheduled background child barrier', () => {
  ...
  it('cancels the real child owner and waits for terminal acknowledgement before finalizing', async () => {
    vi.useFakeTimers();
    try {
      const key = fixture();
      const order = [];
      const onFinalize = vi.fn(async () => { order.push('finalized'); });
      const owner = new AbortController();
      let ownerRunning = true;
      let lateEffect = false;
      const lateEffectTimer = setTimeout(() => { lateEffect = true; }, 31 * 60_000);
      const cancel = vi.fn(reason => owner.abort(reason));

      owner.signal.addEventListener('abort', () => {
        clearTimeout(lateEffectTimer);
        queueMicrotask(() => {
          // This represents the owner's terminal-journal/report boundary. The
          // barrier must not publish scheduled finalization before it arrives.
          order.push('owner-terminal');
          ownerRunning = false;
          completeScheduledChild({
            ...key,
            childId: 'abortable-child',
            errorMsg: 'cancelled by scheduled watchdog',
          });
        });
      }, { once: true });
      ...
      expect(ownerRunning).toBe(false);
      expect(order).toEqual(['owner-terminal', 'finalized']);
      expect(onFinalize).toHaveBeenCalledOnce();
      expect(onFinalize.mock.calls[0][1]).toMatchObject({ errorCount: 1, timedOut: true });

      // A new occurrence may start only after the old execution has stopped;
      // advancing past the would-be effect proves no old owner overlaps it.
      ...
      expect(completeScheduledChild({
        ...key,
        childId: 'abortable-child',
        resultText: 'late success must be ignored',
      })).toMatchObject({ tracked: false });
      expect(onFinalize).toHaveBeenCalledOnce();
    } finally {
      vi.useRealTimers();
    }
  ...
  it('waits only a bounded grace when a cancelled owner never acknowledges settlement', async () => {
    vi.useFakeTimers();
    try {
      const key = fixture();
      const cancel = vi.fn();
      ...
      registerScheduledChild({
        ...key,
        childId: 'unsettled-child',
        label: 'unsettled child',
        kind: 'worker',
        cancel,
      });
      completeScheduledMain({ ...key, onFinalize });
      ...
      expect(cancel).toHaveBeenCalledOnce();
      expect(onFinalize).not.toHaveBeenCalled();
      expect(getScheduledChildGroup(key)).toMatchObject({ pendingCount: 1, timingOut: true });

      await vi.advanceTimersByTimeAsync(5_000);
      await Promise.resolve();
      expect(onFinalize).toHaveBeenCalledOnce();
      expect(onFinalize.mock.calls[0][0]).toContain('cancellation did not settle');
      expect(onFinalize.mock.calls[0][1]).toMatchObject({ errorCount: 1, timedOut: true });
      expect(getScheduledChildGroup(key)).toBeNull();
    } finally {
      vi.useRealTimers();
