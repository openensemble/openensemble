...
// Legacy delivery modifiers are still recognized and stripped from prompts
// ("silently send my briefing at 10am", "every morning at 7 quietly run X",
// "schedule X at 5pm in the background"). All agent tasks now use the ledger;
// these words no longer select a separate delivery mode. Reminders still
// deliver their configured notification.
//
...
export async function interceptScheduling({ userId, agentId, text, history, force = false }) {
  ...
  const agentStr = task.type === 'reminder'
    ...
    : `will run as agent ${task.agent} (results in Tasks → Ledger, outside chat)`;
  return {
    matched: true,
    sideEffectCommitted: true,
    outcome:
      `[scheduler] ${kind} created successfully. label="${task.label}" id=${task.id} when=${whenStr} — ${agentStr}. ` +
      `Confirm this to the user in a natural sentence — include the name and the time${task.type === 'reminder' ? '' : ' and mention that results appear in Tasks → Ledger'} — and do not call any scheduling tools.`,
  };
}
