...
export function isTaskIntent(text) {
  ...
  // [TEST 2026-04-26] Negative: preference/identity statements. "I prefer
  // reminders at 9am" is a memory write, not a scheduled task. Match the
  // signal verb at the start of the message so genuine schedule requests
  // that incidentally contain "I want" mid-sentence still flow through.
...
function validateParse(parsed, userId, currentAgentId, originalText) {
  ...
  // [TEST 2026-04-26] Reject bare time/date fragments. When a clarification
  // follow-up ("tomorrow at 3pm", "12pm", "saturday at 9am") trips the regex
  // pre-filter on its own and the stitch failed to compose (e.g. session
  // write race), the intercept would otherwise create a task with no real
  // content — generic label, prompt = just the time. Bail and force the LLM
  // path. REVERT: delete this block.
  ...
  if (schedule.mode === 'recurring') {
    ...
    // [TEST 2026-04-27] Generic DOW: store the cron DOW field on the task
    // and let scheduler.mjs runTask parse + filter against today's weekday.
    // Also backfills the existing weekdaysOnly/weekendsOnly booleans for
    // any legacy code paths (fireReminder still reads them).
    const dowField = m[3];
  ...
  // [TEST 2026-04-26] Reject parses where the user gave a date but no clock
  // time. The postprocessor's "in N weeks/days/months" branch defaults the
  // hour:minute to current-time-of-day when the request had no clock anchor,
  // which silently books reminders at unintended times. Bail out and let the
  // LLM ask for clarification — the multi-turn stitch will pick up the
  // follow-up reply. REVERT: delete this block.
...
// [TEST 2026-04-26] Cancel-intent detector. Catches short phrases like
// "cancel that", "never mind", "delete the last reminder" and removes the
// user's most recently created task. Returns the deleted task or null.
// REVERT: delete this function and its call site in interceptScheduling.
...
// [TEST 2026-04-26 — reschedule] Detect "actually make it 7pm" / "change
// that to noon" / "move it to tomorrow" — patches the most recently created
// task's datetime. Returns the updated task or null.
// REVERT: delete this function and its call site in interceptScheduling.
...
// [TEST 2026-04-26 — multi-turn stitch] Detect that the current short user
// reply ("12pm") is finishing a stalled scheduling clarification from the
// previous turn. Returns the stitched text to parse, or null. Three gates:
//   (1) prior assistant turn looks like a scheduling clarification
//   (2) current user turn is a short time/date-only fragment
//   (3) the upstream user turn that triggered the clarification matched
//       isTaskIntent — i.e. there's a real stalled scheduling request
// REVERT: delete this function and the call site below.
function tryStitchClarification(text, history) {
  if (!history || !Array.isArray(history) || history.length < 2) return null;
  const cur = (text || '').trim();
  if (!cur) return null;

  // [TEST 2026-04-26] Accept compound day+time forms ("saturday at 9am",
   // "next monday 3pm", "tomorrow at noon") in addition to bare day or bare
   // time fragments. REVERT: restore the prior regex below and tighten the
   // word-count cap back to 4.
  // [TEST 2026-04-27] Added 4-digit military fragments ("0830", "1430hrs",
   // "0015 hours") so stitch fires on military-time clarifications.
...
export async function interceptScheduling({ userId, agentId, text, history, force = false }) {
  // [TEST 2026-04-26] Cancel intent — strip the user's last task before we
  // ever consult the plan model. REVERT: delete this block.
  ...
  // [TEST 2026-04-26] Reschedule intent — patch the most recent task to a
  // new time pulled from the user's phrase. REVERT: delete this block.
  ...
  // [TEST 2026-04-26] Multi-turn stitch — REVERT block to remove.
