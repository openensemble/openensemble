...
// Strip the scheduling trigger from the user's raw request so the agent at
// fire time sees just the actual job. Without this, "in 5 minutes send me
// an email…" reaches the agent verbatim and it concludes the job is to
// "schedule a delayed email" — which most email tools don't support — even
// though the trigger time has already arrived. Handles leading verbs
// ("schedule", "remind me to") and leading + trailing time qualifiers
// ("in N units", "tomorrow at HH:MM", "every morning at 7am", etc).
//
// We only strip a trailing time pattern when it actually parses as a clock
// time (digits + optional am/pm), so addresses like "send to bob at acme"
// stay untouched. The plan model has already extracted the schedule into
// task.datetime / task.time, so the trigger phrase is pure redundancy.
function stripSchedulingPhrases(text) {
  if (!text) return '';
  let s = String(text).trim();
  if (!s) return '';
  let prev;
  // Multiple passes handle compound prefixes like
  // "schedule a reminder in 5 minutes to send …".
  for (let i = 0; i < 4 && s !== prev; i++) {
    prev = s;
    // Leading scheduling verbs
    s = s.replace(/^(please\s+)?(schedule|reschedule)\s+(me\s+)?(an?\s+|the\s+)?/i, '');
    s = s.replace(/^(please\s+)?(set\s+up|set|create|make|add|arrange)\s+(an?\s+|the\s+)?(reminder|task|alarm|timer|appointment|notification)\s+(to|about|for|that)\s+/i, '');
    s = s.replace(/^remind\s+me\s+(to|about)\s+/i, '');
    // Leading time triggers
    s = s.replace(/^in\s+\d+\s*(seconds?|secs?|minutes?|mins?|hours?|hrs?|days?|weeks?|months?)(\s+from\s+now)?[\s,.;:\-]+/i, '');
    s = s.replace(/^(tomorrow|tonight|today)(\s+(morning|afternoon|evening|night))?(\s+at\s+\d{1,2}(:\d{2})?\s*(am|pm)?)?[\s,.;:\-]+/i, '');
    s = s.replace(/^this\s+(morning|afternoon|evening|night)(\s+at\s+\d{1,2}(:\d{2})?\s*(am|pm)?)?[\s,.;:\-]+/i, '');
    s = s.replace(/^at\s+\d{1,2}(:\d{2})?\s*(am|pm)?[\s,.;:\-]+/i, '');
    s = s.replace(/^next\s+(week|monday|tuesday|wednesday|thursday|friday|saturday|sunday)(\s+at\s+\d{1,2}(:\d{2})?\s*(am|pm)?)?[\s,.;:\-]+/i, '');
    s = s.replace(/^every\s+(day|morning|afternoon|evening|night|week|month|monday|tuesday|wednesday|thursday|friday|saturday|sunday|hour|\d+\s*(minutes?|hours?|days?))(\s+at\s+\d{1,2}(:\d{2})?\s*(am|pm)?)?[\s,.;:\-]+/i, '');
    s = s.replace(/^on\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday)(\s+at\s+\d{1,2}(:\d{2})?\s*(am|pm)?)?[\s,.;:\-]+/i, '');
    s = s.replace(/^(daily|weekly|hourly)[\s,.;:\-]+/i, '');
    // Trailing time qualifiers — must look like a clock time, not "at acme.com"
    s = s.replace(/[\s,.;:\-]+in\s+\d+\s*(seconds?|secs?|minutes?|mins?|hours?|hrs?|days?|weeks?|months?)(\s+from\s+now)?\s*$/i, '');
    s = s.replace(/[\s,.;:\-]+at\s+\d{1,2}(:\d{2})?\s*(am|pm)?\s*$/i, '');
    s = s.replace(/[\s,.;:\-]+(tomorrow|tonight|today)(\s+(morning|afternoon|evening|night))?\s*$/i, '');
    s = s.replace(/[\s,.;:\-]+next\s+(week|monday|tuesday|wednesday|thursday|friday|saturday|sunday)(\s+at\s+\d{1,2}(:\d{2})?\s*(am|pm)?)?\s*$/i, '');
    s = s.replace(/[\s,.;:\-]+every\s+(day|morning|afternoon|evening|night|week|month|monday|tuesday|wednesday|thursday|friday|saturday|sunday|hour)(\s+at\s+\d{1,2}(:\d{2})?\s*(am|pm)?)?\s*$/i, '');
    s = s.replace(/[\s,.;:\-]+(daily|weekly|hourly)\s*$/i, '');
    s = s.trim();
  }
...
function validateParse(parsed, userId, currentAgentId, originalText) {
  ...
  // (e.g. "SUBJECT: TEST BODY: Lets see if this works" → "test body"). Strip
  // the scheduling trigger ("in 5 minutes …") before sending: the trigger
  // already fired, and leaving it in the prompt makes the agent (or any
  // sub-agent it delegates to) interpret the job as "delay the action",
  // which most tools don't support.
  const stripped = stripSchedulingPhrases(originalText);
  const prompt = stripped || cleaned || label;
