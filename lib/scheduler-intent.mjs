 ...
 *
 * Two-stage detection:
 *   1. Request guard: leave prohibitions, examples and discussion to the
 *      agent before applying scheduling/time signals.
 *   2. Plan-model parse: extracts schedule fields from eligible requests.
 *      A parseable time alone does not authorize a scheduling side effect.
 *
...
import { addTask, scheduleNewTask, loadTasksForOwner, findTaskById, removeTask, updateTask, formatTaskCadence, parseIntervalPhrase } from '../scheduler.mjs';
import { getAgentsForUser, getUserCoordinatorAgentId, loadConfig } from '../routes/_helpers.mjs';
import { polishTask } from './task-label.mjs';

// This fast path mutates tasks before the agent can interpret the request.
// Ambiguous framing belongs in the normal agent flow, including when a form
// forces parsing or a time clarification would otherwise supply the intent.
function excludesSchedulingInterception(text) {
  const t = String(text || '').trim().toLowerCase().replace(/[‘’]/g, "'");
  // Negation can govern a whole coordinated list: "do not inspect files,
  // change settings, create reminders, or send messages". Keep notification
  // modifiers ("don't show it in chat") and "don't set that" cancellation.
  for (const clause of t.matchAll(/\b(?:do\s+not|don'?t|never|must\s+not|should\s+not|without|avoid|refrain\s+from)\b([^.!?]*)/g)) {
    if (/\b(?:reschedul\w*|schedul\w*|remind\s+me)\b/.test(clause[1]) ||
        /\b(?:creat\w*|add\w*|set\w*|mak\w*|cancel\w*|delet\w*|remov\w*|chang\w*|modif\w*)\b[^.!?]*\b(?:reminders?|tasks?|alarms?|timers?|appointments?)\b/.test(clause[1])) return true;
  }
  if (/\bno\s+(?:(?:new|additional|more|actual|real)\s+)?(?:reminders?|tasks?|alarms?|timers?|appointments?|scheduling)\b/.test(t)) return true;
  if (/\bhypothetically\b|\b(?:as|just|only|purely)\s+(?:an?\s+)?hypothetical\b/.test(t) || /(?:^|\n)\s*(?:```|>|["“'`])/.test(t)) return true;
  // Look at request framing, not words inside a reminder's payload:
  // "remind me tomorrow to explain the scheduling example" stays eligible.
  const frames = t.split(/[.!?;\n]\s*|:\s+/);
  return frames.some(frame => {
    const f = frame.trim().replace(/^(?:\[[^\]\n]+\]\s*)+/, '').replace(/^please\s+/, '');
    if (/^(?:suppose|assuming?|imagine|hypothetical|for\s+(?:example|instance)|what\s+if|if\b|(?:an?\s+)?(?:example|sample|fixture|scenario)\b)/.test(f)) return true;
    if (/^(?:(?:can|could|would|will)\s+you\s+)?(?:explain|describe|discuss|analy[sz]e|summari[sz]e|calculate|illustrate|demonstrate|simulate|compare|evaluate|state|tell\s+me|show\s+me)\b/.test(f)) return true;
    if (/^(?:what|why|how|when|where|which|does|did|is|are|was|were)\b/.test(f) ||
        /^(?:can|could|would|will|should)\s+(?!you\b)/.test(f)) return true;
    if (/^(?:an?|the|my|our|this|that)\s+(?:(?:hourly|daily|weekly|scheduled|existing|recurring)\s+)*(?:tasks?|reminders?|alarms?|timers?|schedules?)\b/.test(f)) return true;
    if (/^(?:i|we|he|she|they|(?:an?|my|our|the)\s+\w+)\s+(?:said|says|asked|asks|wrote|writes|mentioned|mentions|have|has)\b/.test(f)) return true;
    return /^(?:this\s+is\b|(?:run|perform|conduct|do|start)\b)[^.!?\n]*\bread[- ]only\s+(?:(?:integration|validation|regression)\s+)?(?:tests?|checks?|reviews?|audits?)\b/.test(f);
  });
}

// Unsupported edits must reach the agent's task tools, not become creations
// just because the request also contains "hourly" or "tomorrow at 5pm".
function isScheduleEditRequest(text) {
  const t = String(text || '').trim();
  return RESCHEDULE_INTENT_RE.test(t) || /^(?:please\s+)?(?:actually\s+)?(?:(?:can|could|would|will)\s+you\s+(?:please\s+)?)?(?:reschedule\b|(?:cancel|delete|remove|stop|change|move)\b[^.!?\n]*\b(?:tasks?|reminders?|alarms?|timers?|appointments?|schedules?|it|that)\b)/i.test(t);
}
...
export function isTaskIntent(text) {
  ...
  if (excludesSchedulingInterception(text) || isScheduleEditRequest(text)) return false;
...
function isReminderIntent(text) {
  const t = (text || '').trim().toLowerCase();
  // Match "remind me <future-particle>" anywhere, not just at start — "in 1
  // minute remind me to..." is just as valid as "remind me in 1 minute to...".
  // Restricting to future-pointing particles keeps memory-recall phrasings
  // ("remind me what we talked about", "remind me of last week") out.
  if (/\bremind me (to|at|in|on|every|tomorrow|tonight|next)\b/.test(t)) return true;
...
// Coerce known plan-model schema confusions before validation. The model can
// leak across mode/recurrence pairings the training data never showed — most
// commonly mode="recurring" with no cron BUT populated window fields, which is
// just a window task with the wrong label. Reinterpret it as a window task
...
function coerceSchedule(parsed, originalText) {
  ...
  if (s.mode === 'recurring' && hasCron && looksOneShot && !looksRecurring) {
    // A cron whose dow field already lists multiple days (e.g. "* * 1,3" or
    // "* * 1-5") is genuinely recurring — don't demote just because "on
    // monday" makes the phrase look one-shot. The demotion is for the
    // single-weekday "on tuesday at 3pm" case only.
    const dowField = String(s.recurrence).trim().split(/\s+/)[4] ?? '*';
    if (/,|-/.test(dowField)) return parsed;
...
// Adverbs the user can drop into a scheduling request to mark it silent
// ("silently send my briefing at 10am", "every morning at 7 quietly run X",
// "schedule X at 5pm in the background"). Detected here on the fast path so
// "silently" works without going through the schedule_task tool. Reminders
// don't honor this — they fire a banner regardless.
//
// `silen[a-z]*` is intentionally permissive so common typos ("silenty",
// "silentley", "silentley") still register — same root, no English false
// positives in scheduling text. "quietly" is kept strict because looser
// patterns would catch "quickly", which IS a scheduling word.
const SILENT_INTENT_RE = /\b(silen[a-z]*|quietly|quitely|in (the )?background|without (showing|notifying|chat|putting it in chat)|don'?t (show|put|display)[^.]*\bin chat\b|hide (it|this) from chat|no chat (output|notification))\b/i;
...
function validateParse(parsed, userId, currentAgentId, originalText) {
  ...
  const isSilent = !asReminder && SILENT_INTENT_RE.test(originalText || '');
  // Alarm-clock intent: the user wants something that RINGS until dismissed,
  // not a one-shot notification. server.mjs:fireReminder checks task.alarm
  // and routes through the device-side alarm ring loop. Pattern recognizes
  // explicit "alarm" phrasing and "wake me" — both imply insistent ringing.
  const isAlarm = asReminder && /\b(alarm|wake\s+me\s+up?|wake\s+me)\b/i.test(originalText || '');
  ...
  // Silent adverbs ("silently", "in the background", …) are scheduler
  // directives, not part of the action — strip them so the fallback prompt
  // doesn't tell the agent to do something silently when "silent" only ever
  // meant "don't echo to chat."
  const promptRaw = (originalText || '').trim();
  const promptStripped = isSilent ? promptRaw.replace(SILENT_INTENT_RE, '').replace(/\s{2,}/g, ' ').trim() : promptRaw;
  const prompt = promptStripped || cleaned || label;

  // Interval cadence ("every N minutes/hours", "hourly") — the scheduler fires
  // these off a fixed timer, not a clock time. Detect straight from the raw
  // text so we don't depend on the plan model emitting a parseable step-cron
  // (it reliably whiffs on "*/5 * * * *"). This wins over the recurring/one-shot
  // branches below for clearly-interval phrasing.
  const intervalMs = parseIntervalPhrase(originalText);
  if (intervalMs) {
    const base = { label, repeat: 'interval', intervalMs, ownerId: userId };
    return {
      ok: true,
      task: asReminder
        ? { ...base, type: 'reminder', handler: 'fireReminder', ...(isAlarm && { alarm: true }) }
        : { ...base, agent: agentId, prompt, ...(isSilent && { silent: true }) },
    };
  }
...
function tryCancelLastTask(userId, text) {
  ...
  const mine = loadTasksForOwner(userId);
...
async function tryRescheduleLastTask(userId, text, postprocessFn, parseClockFn) {
  ...
  const mine = loadTasksForOwner(userId).filter(t => t.enabled !== false);
  if (!mine.length) return null;
  const newest = mine
    .map(t => ({ t, ts: Number((t.id || '').match(/_(\d{10,13})/)?.[1] || 0) }))
    .sort((a, b) => b.ts - a.ts)[0]?.t;
  ...
  const fresh = findTaskById(newest.id, userId);
 ...
 * @returns {Promise<{ matched: boolean, outcome?: string, sideEffectCommitted?: boolean }>}
...
export async function interceptScheduling({ userId, agentId, text, history, force = false }) {
  // Per-install opt-out. When scheduler.useBuiltinPlan is false, route ALL
  // scheduling (create, cancel, reschedule) through the agent's tools
  // (set_reminder / schedule_task / delete_task in skills/tasks) instead
  // of the built-in plan model. The agent already has the tools wired —
  // skipping the interceptor just removes the [scheduler] outcome line so
  // the SPA gating in skills/tasks/manifest.json doesn't tell the agent
  // "task already created — don't call tools." Trade-off: agent calls cost
  // cloud tokens + ~2s extra LLM round-trip per request, but the cloud LLM
  // can handle phrasings the plan model misses (failure-mode insurance).
  const cfg = loadConfig() ?? {};
  if (cfg?.scheduler?.useBuiltinPlan === false) return { matched: false };
  if (excludesSchedulingInterception(text)) return { matched: false };

  const cancelled = tryCancelLastTask(userId, text);
  if (cancelled) {
    const whenStr = formatTaskCadence(cancelled);
    return {
      matched: true,
  ...
  if (rescheduled) {
    const whenStr = formatTaskCadence(rescheduled);
    return {
      matched: true,
      sideEffectCommitted: true,
  ...
  if (isScheduleEditRequest(text)) return { matched: false };
  ...
  if (excludesSchedulingInterception(effectiveText)) return { matched: false };
  ...
  // Interval phrasing ("every 5 minutes") is recognized from the raw text in
  // validateParse, so let it through even when the plan model emitted nothing.
  if (!hasAnySchedule && !parseIntervalPhrase(effectiveText)) return { matched: false };
  ...
  const agentStr = task.type === 'reminder'
    ...
    : `will run as agent ${task.agent}${task.silent ? ' (silent — no chat output, summary appears in tasks drawer)' : ''}`;
  return {
    matched: true,
    sideEffectCommitted: true,
    outcome:
      `[scheduler] ${kind} created successfully. label="${task.label}" id=${task.id} when=${whenStr} — ${agentStr}. ` +
      `Confirm this to the user in a natural sentence — include the name and the time${task.silent ? ' and mention it will run silently' : ''} — and do not call any scheduling tools.`,
  };
}
