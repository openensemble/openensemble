...
function atomicWriteOutcomes(file, text) {
...
export async function appendTaskOutcome(userId, outcome) {
  ...
  const row = {
    ...
    ...(outcome.provider ? { provider: String(outcome.provider).slice(0, 100) } : {}),
    ...(outcome.model ? { model: String(outcome.model).slice(0, 300) } : {}),
    ...(outcome.reasoningEffort ? { reasoningEffort: String(outcome.reasoningEffort).slice(0, 40) } : {}),
    ...(outcome.executionTargetExplicit === true ? { executionTargetExplicit: true } : {}),
      ...
      if (fs.existsSync(p)) {
        ...
        for (const ln of fileLines) {
          try {
            ...
            if (rec.ts > cutoff) {
              kept.push(ln);
              if (rec.taskId === row.taskId && rec.kind === row.kind) alreadyRecorded = true;
            }
          } catch { /* drop bad lines */ }
        }
      }
      // Terminal ownership is first-writer-wins. Restart recovery and a late
      // provider callback may both attempt the mirror, but check_workers must
      // never observe contradictory outcomes for one logical task.
      if (!alreadyRecorded) kept.push(line);
      atomicWriteOutcomes(p, kept.join('\n') + '\n');
    });
  } catch (e) {
