...
function fileLockAbortError(signal) {
  const reason = signal?.reason;
  if (reason instanceof Error) return reason;
  const error = new Error(
    typeof reason === 'string' && reason.trim()
      ? reason.trim()
      : 'File lock acquisition cancelled',
  );
  error.name = 'AbortError';
  return error;
}

function throwIfFileLockAborted(signal) {
  if (signal?.aborted) throw fileLockAbortError(signal);
}

function sleep(ms, signal = null) {
  const delay = Math.max(1, Math.floor(ms));
  if (!signal) return new Promise(resolve => setTimeout(resolve, delay));
  throwIfFileLockAborted(signal);
  return new Promise((resolve, reject) => {
    ...
    const onAbort = () => finish(reject, fileLockAbortError(signal));
    timer = setTimeout(() => finish(resolve), delay);
    signal.addEventListener('abort', onAbort, { once: true });
    // Close the check/listener race when cancellation lands between the entry
 ...
 * @param {{timeoutMs?: number, staleMs?: number, retryMinMs?: number, retryMaxMs?: number, signal?: AbortSignal|null}} [opts]
 * @returns {Promise<T>}
 */
export async function withFileLock(lockDir, fn, opts = {}) {
  ...
  while (true) {
    ...
    try {
      ...
      const jitter = retryMinMs + Math.random() * (retryMaxMs - retryMinMs);
      await sleep(Math.min(jitter, Math.max(1, deadline - Date.now())), signal);
      continue;
    }

    try {
      // mkdir succeeded, so cancellation from this point must also clean up the
      // newly-owned directory before propagating.
      throwIfFileLockAborted(signal);
    ...
    try {
      // An abort can land while owner.json is being written. Recheck at the
      // final no-await boundary so a cancelled waiter never enters fn.
      throwIfFileLockAborted(signal);
      return await fn();
    } finally {
