...
async function saveOutcomes(userId, data) {
...
export async function recordPreAcceptSnapshot(proposal) {
  ...
  try { await saveOutcomes(userId, all); } catch (e) {
...
function _applyLazyPostSnapshots(userId) {
  ...
  for (const [pid, rec] of Object.entries(all)) {
    ...
    if (measured) {
      rec.preCount  = measured.preCount;
      rec.postCount = measured.postCount;
      rec.delta     = measured.delta;
      rec.semantic  = measured.semantic || 'lower-better';
      rec.note      = measured.note || null;
      rec.measurerUsed = rec.kind;
    } else {
      ...
      rec.postCount = postCount;
      rec.delta = postCount - (rec.preCount || 0);
      rec.semantic = rec.semantic || 'lower-better';   // coarse signal: fewer proposals = less friction
      rec.note = rec.note || 'proposals to you (coarse)';
      rec.measurerUsed = null;
    }
    mutated = true;
  }
  if (mutated) {
    saveOutcomes(userId, all).catch(() => {});
  }
  return all;
}
...
export function summarizeByKind(userId) {
  ...
  for (const rec of all) {
    ...
    if (rec.postCount !== null && rec.postCount !== undefined) {
      ...
      //   lower-better:  delta <= 0 is improvement
      //   higher-better: delta >= 0 is improvement
