...
export async function drainToolWithEvents(name, args, userId, agentId, allowedTools = null) {
  ...
  try {
    for await (const chunk of executeToolStreaming(name, args, userId, agentId, allowedTools)) {
      ...
      if (chunk.type === 'tool_call')          events.push({ type: 'tool_call', name: chunk.name, args: chunk.args, ...(chunk.toolCallId ? { toolCallId: chunk.toolCallId } : {}), ...(chunk.providerNative ? { providerNative: true } : {}) });
      if (chunk.type === 'tool_progress')      events.push({ type: 'tool_progress', name: chunk.name, text: chunk.text, ...(chunk.toolCallId ? { toolCallId: chunk.toolCallId } : {}), ...(chunk.providerNative ? { providerNative: true } : {}) });
      if (chunk.type === 'tool_result')        events.push({ type: 'tool_result', name: chunk.name, text: chunk.text, preview: summarizeToolResult(chunk.name, chunk.text), ...(chunk.toolCallId ? { toolCallId: chunk.toolCallId } : {}), ...(chunk.providerNative ? { providerNative: true } : {}) });
